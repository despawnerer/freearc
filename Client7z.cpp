...
int BreakFlag = 0;
extern "C" void c_szSetBreakFlag(int flag) { BreakFlag = flag; }
...
public:
  ...
  UString Password;
  TABI_FUNCTION *cb;
  COpenCallbackConsoleZ(TABI_FUNCTION *_cb)
      : cb(_cb), PasswordIsDefined(false), PasswordWasAsked(false) {}
#endif
};
...
HRESULT COpenCallbackConsoleZ::Open_SetCompleted(const UInt64 *,
                                                 const UInt64 *) {
  return Open_CheckBreak();
}
...
HRESULT COpenCallbackConsoleZ::Open_CryptoGetTextPassword(BSTR *password) {
  ...
  if (!PasswordIsDefined) {
    ...
    cb(TABI_DYNAMAP("request", "ask_password")(
        "password_buf", (void *)password_buf)("password_size", password_size));
...
bool COpenCallbackConsoleZ::Open_WasPasswordAsked() { return PasswordWasAsked; }
...
static void szInitLibrary() {
  ...
  if (!codecs) {
    ...
    codecs = new CCodecs;
    CMyComPtr<ICompressCodecsInfo> *compressCodecsInfo =
        new CMyComPtr<ICompressCodecsInfo>(codecs);
...
extern "C" int c_szOpenArchive(TABI_ELEMENT *params) {
  try {
    ...
    COpenCallbackConsoleZ *openCallback =
        new COpenCallbackConsoleZ(p._callback("callback"));

    bool stdInMode = false;
    try {
      HRESULT result = archiveLink->Open2(codecs, formatIndices, stdInMode,
                                          NULL, archiveName, openCallback);
      if (result != S_OK)
...
extern "C" UInt32 c_szArcGetInt64Property(CArchiveLink &arc, Int32 index,
                                          PROPID propID, UInt64 *value,
                                          char *errmsg) {
  try {
    CMyComPtr<IInArchive> archive = arc.GetArchive();
    NCOM::CPropVariant prop;
    if ((index == -1 ? archive->GetArchiveProperty(propID, &prop)
                     : archive->GetProperty(index, propID, &prop)) != S_OK)
...
extern "C" UInt32 c_szArcGetBoolProperty(CArchiveLink &arc, Int32 index,
                                         PROPID propID, UInt32 *value,
                                         char *errmsg) {
  try {
    CMyComPtr<IInArchive> archive = arc.GetArchive();
    NCOM::CPropVariant prop;
    ...
    case VT_EMPTY:
      *value = 0;
      break;
    case VT_BOOL:
      *value = VARIANT_BOOLToBool(prop.boolVal) ? 1 : 0;
      break;
    default:
      throw "not an Empty or Bool value";
    }
    return 0;
  } catch (const char *msg) {
...
extern "C" UInt32 c_szArcGetStrProperty(CArchiveLink &arc, Int32 index,
                                        PROPID propID, wchar_t *value,
                                        UInt32 valueSize, char *errmsg) {
  try {
    CMyComPtr<IInArchive> archive = arc.GetArchive();
    NCOM::CPropVariant prop;
    if (index == -1 && propID == kpidType)
      prop = codecs->Formats[arc.Arcs.Back().FormatIndex].Name;
    else if ((index == -1 ? archive->GetArchiveProperty(propID, &prop)
                          : archive->GetProperty(index, propID, &prop)) != S_OK)
      throw "GetProperty failed";
    switch (prop.vt) {
    case VT_EMPTY:
    ...
    default:
      throw "not an Empty or BStr value";
    }
    return 0;
  } catch (const char *msg) {
...
extern "C" UInt32 c_szArcGetTimeProperty(CArchiveLink &arc, Int32 index,
                                         PROPID propID, UInt32 *value,
                                         char *errmsg) {
  try {
    CMyComPtr<IInArchive> archive = arc.GetArchive();
...
class CExtractCallbackConsole : public IExtractCallbackUI,
...
public:
  ...
  (const wchar_t *name, bool isFolder, Int32 askExtractMode,
...
static const char *kCrcFailedEncrypted =
    "CRC Failed in encrypted file. Wrong password?";
static const char *kDataError = "Data Error";
static const char *kDataErrorEncrypted =
    "Data Error in encrypted file. Wrong password?";
static const char *kUnknownError = "Unknown Error";

STDMETHODIMP CExtractCallbackConsole::SetTotal(UInt64 x) { return S_OK; }
...
STDMETHODIMP CExtractCallbackConsole::AskOverwrite(
    ...
    const wchar_t *, const FILETIME *, const UInt64 *, Int32 *answer) {
  if (BreakFlag)
    return E_ABORT;
  *answer = cb(TABI_DYNAMAP("request", "can_be_extracted?")(
      "outname", existName)("index", *index));
  return S_OK;
}

STDMETHODIMP CExtractCallbackConsole::PrepareOperation(const wchar_t *name,
                                                       bool isFolder,
                                                       Int32 askExtractMode,
                                                       const UInt64 *position) {
  if (BreakFlag)
    return E_ABORT;
  cb(TABI_DYNAMAP("request", "filename")("filename", name)(
      "is_folder?", isFolder)("mode", askExtractMode));
  return S_OK;
}

STDMETHODIMP CExtractCallbackConsole::SetRatioInfo(const UInt64 *inSize,
                                                   const UInt64 *outSize) {
  if (BreakFlag)
    return E_ABORT;
  cb(TABI_DYNAMAP("request", "progress")("compressed", *inSize - old_inSize)(
      "original", *outSize - old_outSize));
  old_inSize = *inSize;
  old_outSize = *outSize;
  return S_OK;
}
...
STDMETHODIMP CExtractCallbackConsole::SetOperationResult(Int32 operationResult,
  ...
  if (BreakFlag)
    return E_ABORT;
  cb(TABI_DYNAMAP("request", "filedone")("operationResult", operationResult)(
      "encrypted?", encrypted));
  return S_OK;
}
...
HRESULT CExtractCallbackConsole::OpenResult(const wchar_t * /* name */,
                                            HRESULT result, bool encrypted) {
  return S_OK;
}

HRESULT CExtractCallbackConsole::ThereAreNoFiles() { return S_OK; }

HRESULT CExtractCallbackConsole::ExtractResult(HRESULT result) { return S_OK; }
...
class CArchiveExtractCallback : public IArchiveExtractCallback,
  ...
  void CreateComplexDirectory(const UStringVector &dirPathParts,
                              UString &fullPath);
  HRESULT GetTime(int index, PROPID propID, FILETIME &filetime,
...
public:
  ...
  // STDMETHOD(GetInStream)(const wchar_t *name, ISequentialInStream
  // **inStream);

  STDMETHOD(CryptoGetTextPassword)(BSTR *password);

  CArchiveExtractCallback()
      : WriteCTime(true), WriteATime(true), WriteMTime(true),
        _multiArchives(false) {
    LocalProgressSpec = new CLocalProgress();
    _localProgress = LocalProgressSpec;
  }

  void InitForMulti(bool multiArchives, NExtract::NPathMode::EEnum pathMode,
  ...
  void Init(const NWildcard::CCensorNode *wildcardCensor, const CArc *arc,
            IFolderArchiveExtractCallback *extractCallback2, bool stdOutMode,
            bool testMode, bool crcMode, const UString &directoryPath,
            const UStringVector &removePathParts, UInt64 packSize);

  bool optionKeepBroken;
};

static const wchar_t *kCantAutoRename =
    L"ERROR: Can not create file with auto name";
...
void CArchiveExtractCallback::Init(
    const NWildcard::CCensorNode *wildcardCensor, const CArc *arc,
    IFolderArchiveExtractCallback *extractCallback2, bool stdOutMode,
    bool testMode, bool crcMode, const UString &directoryPath,
    const UStringVector &removePathParts, UInt64 packSize) {
  ...
  _extractCallback2.QueryInterface(IID_ICompressProgressInfo,
                                   &_compressProgress);
...
STDMETHODIMP
CArchiveExtractCallback::SetCompleted(const UInt64 *completeValue) {
  COM_TRY_BEGIN
  ...
  if (_multiArchives) {
    if (completeValue != NULL) {
      UInt64 packCur = LocalProgressSpec->InSize +
                       MyMultDiv64(*completeValue, _unpTotal, _packTotal);
...
STDMETHODIMP CArchiveExtractCallback::SetRatioInfo(const UInt64 *inSize,
                                                   const UInt64 *outSize) {
  COM_TRY_BEGIN
...
void CArchiveExtractCallback::CreateComplexDirectory(
    const UStringVector &dirPathParts, UString &fullPath) {
...
HRESULT CArchiveExtractCallback::GetTime(int index, PROPID propID,
                                         FILETIME &filetime,
                                         bool &filetimeIsDefined) {
...
STDMETHODIMP CArchiveExtractCallback::GetStream(
  ...
  *outStream = 0;
  ...
  if (askExtractMode == NArchive::NExtract::NAskMode::kExtract && !_testMode) {
      ...
      if (!pathParts.IsEmpty()) {
        ...
        if (_fi.IsDir)
          NFile::NDirectory::SetDirTime(
              fullPathNew, (WriteCTime && _fi.CTimeDefined) ? &_fi.CTime : NULL,
              (WriteATime && _fi.ATimeDefined) ? &_fi.ATime : NULL,
              (WriteMTime && _fi.MTimeDefined)
                  ? &_fi.MTime
                  : (_arc->MTimeDefined ? &_arc->MTime : NULL));
      }
    }
    ...
    if (!_isSplit) {
      ...
      RINOK(_extractCallback2->AskOverwrite(fullProcessedPath, NULL, &index64,
                                            NULL, NULL, NULL, &overwiteResult));
      if (overwiteResult == 0)
...
STDMETHODIMP CArchiveExtractCallback::PrepareOperation(Int32 askExtractMode) {
  ...
  return _extractCallback2->PrepareOperation(
      _filePath, _fi.IsDir, askExtractMode, _isSplit ? &_position : 0);
  COM_TRY_END
}
...
static HRESULT DecompressArchiveZ(const CArc &arc, UInt32 *filelist, int files,
                                  UInt64 packSize,
  ...
  extractCallbackSpec->InitForMulti(false, options.PathMode,
                                    options.OverwriteMode);
  extractCallbackSpec->Init(NULL, &arc, callback, options.StdOutMode,
                            options.TestMode, options.CalcCrc, outDir,
                            removePathParts, packSize);
  ...
  if (options.StdInMode) {
    result = archive->Extract(NULL, (UInt32)(Int32)-1, testMode,
                              extractCallbackSpec);
    NCOM::CPropVariant prop;
...
extern "C" int c_szExtract(TABI_ELEMENT *params) {
  try {
    ...
    options.PathMode = strcmp(command, "e") == 0
    ...
    HRESULT result = DecompressArchiveZ(
        arc.Arcs.Back(), filelist, files, packSize, options, extractCallback,
        extractCallbackSpec, errorMessage, packProcessed);
...
HRESULT CUpdateCallbackConsole::OpenResult(const wchar_t *name,
                                           HRESULT result) {
  return S_OK;
}

HRESULT CUpdateCallbackConsole::StartScanning() { return S_OK; }

HRESULT CUpdateCallbackConsole::ScanProgress(UInt64 /* numFolders */,
                                             UInt64 /* numFiles */,
                                             const wchar_t * /* path */) {
  return S_OK;
}

HRESULT CUpdateCallbackConsole::CanNotFindError(const wchar_t *name,
                                                DWORD systemError) {
  return S_OK;
}

HRESULT CUpdateCallbackConsole::FinishScanning() { return S_OK; }

HRESULT CUpdateCallbackConsole::StartArchive(const wchar_t *name,
                                             bool updating) {
  return S_OK;
}

HRESULT CUpdateCallbackConsole::FinishArchive() { return S_OK; }
...
HRESULT CUpdateCallbackConsole::Finilize() { return S_OK; }

HRESULT CUpdateCallbackConsole::SetNumFiles(UInt64 numFiles) { return S_OK; }

HRESULT CUpdateCallbackConsole::SetTotal(
    UInt64 size) // This "total" is "capacity" actual, it's larger than sum of
                 // files' origsize
{
  return S_OK;
}
...
HRESULT CUpdateCallbackConsole::SetRatioInfo(const UInt64 *inSize,
                                             const UInt64 *outSize) {
  RINOK(CheckBreak());
  cb(TABI_DYNAMAP("request", "progress")("original", *inSize - old_inSize)(
      "compressed", *outSize - old_outSize));
  old_inSize = *inSize;
  old_outSize = *outSize;
  return S_OK;
}
...
HRESULT CUpdateCallbackConsole::GetStream(const wchar_t *name, bool isAnti) {
  RINOK(CheckBreak());
  cb(TABI_DYNAMAP("request", "filename")("filename", name)("is_folder?", false)(
      "mode", NArchive::NExtract::NAskMode::kExtract));
  // cb(TABI_DYNAMAP ("request","filename") ("filename", last_index>=0?
  // disk_names[last_index] : name) ("is_folder?", false) ("mode",
  // NArchive::NExtract::NAskMode::kExtract));
  last_index = -1;
  return S_OK;
}

HRESULT CUpdateCallbackConsole::OpenFileError(const wchar_t *name,
                                              DWORD systemError) {
  return S_FALSE;
}

HRESULT CUpdateCallbackConsole::SetOperationResult(Int32) { return S_OK; }

HRESULT CUpdateCallbackConsole::CryptoGetTextPassword2(Int32 *passwordIsDefined,
                                                       BSTR *password) {
  RINOK(CheckBreak());
...
void AddDirFileInfo(int phyParent, int logParent, const NFind::CFileInfoW &fi,
...
UString CDirItems::GetPrefixesPath(const CIntVector &parents, int index,
                                   const UString &name) const {
  return UString();
}

UString CDirItems::GetPhyPath(int index) const { return disk_names[index]; }
...
void CDirItems::ReserveDown() { Items.ReserveDown(); }
...
void CDirItems::EnumerateDirectory(int phyParent, int logParent,
...
HRESULT EnumerateItems(const NWildcard::CCensor &censor, CDirItems &dirItems,
  ...
  for (wchar_t **filename = disk_names; *filename; filename++) {
    NFind::CFileInfoW fi;
    if (!fi.Find(*filename))
      throw "file to compress not found";
...
extern "C" int c_szCompress(TABI_ELEMENT *params) {
  try {
    ...
    TABI_MAP p(params);
    char *update_type =
        (p._str("update_type")); // Archive updating operation (a, u, f, s)
    wchar_t **compression_options =
        (wchar_t **)(p._ptr("options"));     // Compression options
    ...
    disk_names =
        (wchar_t **)(p._ptr("disk_names")); // Файлы для упаковки с диска
    stored_names = (wchar_t **)(p._ptr(
        "stored_names")); //   и имена, которые им дать в архиве
    wchar_t *password = (p._wstr("password")); // Пароль шифрования
    int volumes = (p._int("volumes")); // Размер массива volume_sizes
    UInt64 *volume_sizes = (UInt64 *)(p._ptr("volume_sizes"));
    ; // Размер томов архива
    wchar_t *sfxModule =
        (p._wstr("sfx")); // Файл sfx-модуля, "-" для удаления существующего
                          // модуля, или "--" чтобы ничего не менять
    ...
    uc.ActionSet =
        0 == strcmp(update_type, "a")   ? NUpdateArchive::kAddActionSet
    ...
    HRESULT result = UpdateArchive(codecs, censor, options, errorInfo,
...
extern "C" int c_szDefaultExtension(wchar_t *arcType, wchar_t *extension,
                                    UInt32 extensionSize, char *errmsg) {
  szInitLibrary();
  CIntVector formatIndices;
  wcscpy(extension, L"");
  if (codecs->FindFormatForArchiveType(arcType, formatIndices)) {
...
// Return possible arcType what 7z.dll is able to CREATE for given arcname w/o
// path, or ""
extern "C" int c_szFindFormatForArchiveName(wchar_t *arcname, wchar_t *arcType,
                                            UInt32 arcTypeSize, char *errmsg) {
  szInitLibrary();
  int i = codecs->FindFormatForArchiveName(arcname);
  wcscpy(arcType, i >= 0 ? codecs->Formats[i].Name
                         : L""); // i<0 == archive type wasn't recognized
  return 0;
}
