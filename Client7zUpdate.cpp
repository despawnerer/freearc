...
STDMETHODIMP COutMultiVolStream::Write(const void *data, UInt32 size,
  ...
  while (size > 0) {
    if (_streamIndex >= Streams.Size()) {
      ...
      cb(TABI_DYNAMAP("request", "volume")(
          "filename", (const wchar_t *)name)); //// modified by zbr
      continue;
    }
...
STDMETHODIMP COutMultiVolStream::Seek(Int64 offset, UInt32 seekOrigin,
                                      UInt64 *newPosition) {
  if (seekOrigin >= 3)
...
bool CUpdateOptions::Init(const CCodecs *codecs,
  ...
  if (arcTypeIndex >= 0)
    ...
    // It works incorrectly for update command if archive has some non-default
    // extension!
    if (MethodMode.FormatIndex < 0)
      MethodMode.FormatIndex =
          codecs->FindFormatForArchiveType(kDefaultArchiveType);
  }
  if (MethodMode.FormatIndex < 0)
    return false;
...
static HRESULT
Compress(CCodecs *codecs, const CActionSet &actionSet, IInArchive *archive,
         const CCompressionMethodMode &compressionMethod,
         CArchivePath &archivePath, const CObjectVector<CArcItem> &arcItems,
         bool shareForWrite, bool stdInMode,
         /* const UString & stdInFileName, */
         bool stdOutMode, const CDirItems &dirItems, bool sfxMode,
         const UString &sfxModule, const CRecordVector<UInt64> &volumesSizes,
         CTempFiles &tempFiles, CUpdateErrorInfo &errorInfo,
  ...
  if (archive != NULL) {
...
#ifdef EXTERNAL_CODECS
    {
      ...
      outArchive.QueryInterface(IID_ISetCompressCodecsInfo,
                                (void **)&setCompressCodecsInfo);
...

  {
    ...
    GetUpdatePairInfoList(dirItems, arcItems, fileTimeType,
                          updatePairs); // must be done only once!!!
  ...
  //// added by zbr: Count total amount and size of files to compress
  {
    ...
    cb(TABI_DYNAMAP("request", "total")("files", updatePairs2.Size())(
        "original", totalSize));
  }
  ...
  updateCallbackSpec->Archive =
      archive; //// added by zbr: file may be renamed, Archive required to copy
               ///all file properties except for name
  ...
  HRESULT result =
      outArchive->UpdateItems(outStream, updatePairs2.Size(), updateCallback);
  callback->Finilize();
  RINOK(result);
  if (outStreamSpec)
...
static HRESULT UpdateWithItemLists(CCodecs *codecs, CUpdateOptions &options,
                                   IInArchive *archive,
                                   const CObjectVector<CArcItem> &arcItems,
                                   CDirItems &dirItems, CTempFiles &tempFiles,
  ...
  for (int i = 0; i < options.Commands.Size(); i++) {
    ...
    RINOK(Compress(codecs, command.ActionSet, archive, options.MethodMode,
                   command.ArchivePath, arcItems, options.OpenShareForWrite,
                   options.StdInMode,
                   /* options.StdInFileName, */
                   options.StdOutMode, dirItems, options.SfxMode,
                   options.SfxModule, options.VolumesSizes, tempFiles,
                   errorInfo, callback));
...
public:
  ...
  bool RestoreDirectory() {
    return BOOLToBool(NFile::NDirectory::MySetCurrentDirectory(_path));
  }
};
#endif
...
struct CEnumDirItemUpdateCallback : public IEnumDirItemCallback {
  IUpdateCallbackUI2 *Callback;
  HRESULT ScanProgress(UInt64 numFolders, UInt64 numFiles,
                       const wchar_t *path) {
...
#ifdef _WIN32
typedef ULONG(FAR PASCAL MY_MAPISENDDOCUMENTS)(ULONG_PTR ulUIParam,
                                               LPSTR lpszDelimChar,
                                               LPSTR lpszFilePaths,
                                               LPSTR lpszFileNames,
                                               ULONG ulReserved);
...
HRESULT UpdateArchive(CCodecs *codecs, const NWildcard::CCensor &censor,
                      CUpdateOptions &options, CUpdateErrorInfo &errorInfo,
  ...
  if (archiveFileInfo.Find(archiveName)) {
    ...
    HRESULT result = archiveLink.Open2(codecs, formatIndices, false, NULL,
                                       archiveName, openCallback);
    if (result == E_ABORT)
      return result;
    ...
    if (archiveLink.VolumePaths.Size() > 1) {
      ...
      errorInfo.Message =
          L"Updating for multivolume archives is not implemented";
      return E_NOTIMPL;
    }
  ...
  if (options.StdInMode) {
    ...
    if (needScanning) {
      ...
      HRESULT res = EnumerateItems(censor, dirItems, &enumCallback, errorPaths,
                                   errorCodes);
  ...
  if (!options.StdOutMode && options.UpdateArchiveItself) {
    ...
    if ((thereIsInArchive || !options.WorkingDir.IsEmpty()) && !usesTempDir &&
  ...
  RINOK(UpdateWithItemLists(
      codecs, options, thereIsInArchive ? archiveLink.GetArchive() : 0,
      arcItems, dirItems, tempFiles, errorInfo, callback));
  ...
  if (options.EMailMode) {
    ...
    MY_LPMAPISENDDOCUMENTS fnSend =
        (MY_LPMAPISENDDOCUMENTS)mapiLib.GetProc("MAPISendDocuments");
    if (fnSend == 0) {
      errorInfo.SystemError = ::GetLastError();
...
void UpdateProduce(const CRecordVector<CUpdatePair> &updatePairs,
...
CArchiveUpdateCallback::CArchiveUpdateCallback()
    : Callback(0), ShareForWrite(false), StdInMode(false), DirItems(0),
      ArcItems(0), UpdatePairs(0), NewNames(0) {}
...
STDMETHODIMP CArchiveUpdateCallback::SetRatioInfo(const UInt64 *inSize,
...
{
  return CStatPropEnumerator::CreateEnumerator(kProperties, sizeof(kProperties)
/ sizeof(kProperties[0]), enumerator);
}
*/
...
STDMETHODIMP CArchiveUpdateCallback::GetUpdateItemInfo(UInt32 index,
  ...
  if (newData != NULL)
    *newData = BoolToInt(up.NewData);
  if (newProps != NULL)
    *newProps = BoolToInt(up.NewProps);
  if (indexInArchive != NULL) {
    *indexInArchive = (UInt32)-1;
    if (up.ExistInArchive())
      *indexInArchive = (ArcItems == 0)
                            ? up.ArcIndex
                            : (*ArcItems)[up.ArcIndex].IndexInServer;
  }
  return S_OK;
  COM_TRY_END
}

STDMETHODIMP CArchiveUpdateCallback::GetProperty(UInt32 index, PROPID propID,
                                                 PROPVARIANT *value) {
  COM_TRY_BEGIN
  const CUpdatePair2 &up = (*UpdatePairs)[index];
  ...
  if (up.ExistOnDisk()) {
    ...
    switch (propID) {
    case kpidPath:
      prop = DirItems->GetLogPath(up.DirIndex);
      break;
    case kpidIsDir:
      prop = di.IsDir();
      break;
...
STDMETHODIMP
CArchiveUpdateCallback::GetVolumeStream(UInt32 index,
...
STDMETHODIMP
CArchiveUpdateCallback::CryptoGetTextPassword2(Int32 *passwordIsDefined,
                                               BSTR *password) {
  COM_TRY_BEGIN
