...
/*--
  ...
  BWT compression mode:
    Compression     memory use : [6-9]*BlockLen  + 1Mb   (6 for fast bwt, 9 for
strong bwt) Decompression   memory use : 5*BlockLen      + 1Mb ST4 compression
mode: Compression     memory use : 5*BlockLen      + 1Mb Decompression   memory
use : 5.125*BlockLen  + 1Mb
...
const sint32 RESERVED =
    0; // неиспользуемые байты в заголовке заполняются этим значением

#ifndef FREEARC_DECOMPRESS_ONLY

sint32 GRZip_StoreBlock(uint8 *Input, sint32 Size, uint8 *Output, sint32 Mode) {
  *(sint32 *)(Output + 4) = -1;
...
sint32 __cdecl GRZip_CompressBlock(uint8 *Input, sint32 Size, uint8 *Output,
                                   sint32 Mode) {
  sint32 SSize = Size;
  ...
  *(sint32 *)Output = Size;
  ...
  if ((Size > 1024) && ((Mode & GRZ_Disable_DeltaFlt) == 0)) {
    ...
    if (RecMode) {
      ...
      if (Buffer == NULL)
        return (GRZip_StoreBlock(Input, Size, Output, 0));
      GRZip_Rec_Encode(Input, Size, Buffer, RecMode);
      Mode += GRZ_Disable_DeltaFlt;
      if ((RecMode & 1) == 1) {
        sint32 PartSize = (Size >> 1);
        sint32 Result =
            GRZip_CompressBlock(Buffer, PartSize, Output + 28, Mode);
        if (Result < 0) {
          BigFree(Buffer);
          return (GRZip_StoreBlock(Input, Size, Output, 0));
        }
        NewSize = Result;
        Result = GRZip_CompressBlock(Buffer + PartSize, Size - PartSize,
                                     Output + 28 + NewSize, Mode);
        if (Result < 0) {
          BigFree(Buffer);
          return (GRZip_StoreBlock(Input, Size, Output, 0));
        }
      ...
      if ((RecMode & 1) == 0) {
        ...
        NewSize = Result;
        Result = GRZip_CompressBlock(Buffer + PartSize, PartSize,
                                     Output + 28 + NewSize, Mode);
        if (Result < 0) {
          BigFree(Buffer);
          return (GRZip_StoreBlock(Input, Size, Output, 0));
        ...
        NewSize += Result;
        Result = GRZip_CompressBlock(Buffer + 2 * PartSize, PartSize,
                                     Output + 28 + NewSize, Mode);
        if (Result < 0) {
          BigFree(Buffer);
          return (GRZip_StoreBlock(Input, Size, Output, 0));
        ...
        NewSize += Result;
        Result = GRZip_CompressBlock(Buffer + 3 * PartSize, Size - 3 * PartSize,
                                     Output + 28 + NewSize, Mode);
        if (Result < 0) {
          BigFree(Buffer);
          return (GRZip_StoreBlock(Input, Size, Output, 0));
  ...
  if (LZP_Enabled(Mode)) {
    sint32 Result =
        GRZip_LZP_Encode(Input, Size, LZPBuffer, Get_LZP_MinMatchLen(Mode),
                         Get_LZP_HT_Size(Mode));
  ...
  for (Result = 0; Result < 8; Result++)
    LZPBuffer[Result + Size] = 0;
  Size = (Size + 7) & (~7);

  if (Mode & GRZ_Compression_ST4)
    Result = GRZip_ST4_Encode(LZPBuffer, Size, LZPBuffer);
  else
    Result = GRZip_BWT_Encode(LZPBuffer, Size, LZPBuffer,
                              Mode & GRZ_BWTSorting_Fast);

  if (Result == GRZ_NOT_ENOUGH_MEMORY) {
    if (LZP_Enabled(Mode)) {
      sint32 Result =
          GRZip_LZP_Encode(Input, SSize, LZPBuffer, Get_LZP_MinMatchLen(Mode),
                           Get_LZP_HT_Size(Mode));
      if (Result == GRZ_NOT_ENOUGH_MEMORY) {
        BigFree(LZPBuffer);
...
sint32 __cdecl GRZip_DecompressBlock(uint8 *Input, sint32 Size, uint8 *Output) {
  ...
  if (Mode == -1) {
    ...
    Result = GRZip_LZP_Decode(Input + 28, Result, Output,
                              Get_LZP_MinMatchLen(Mode), Get_LZP_HT_Size(Mode));
    return (Result);
  }
  ...
  if (Mode == -2) {
    ...
    if ((RecMode & 1) == 1) {
      Result = GRZip_DecompressBlock(Tmp, (*(sint32 *)(Tmp + 16)) + 28,
                                     Buffer + OutputPos);
      if (Result < 0) {
        BigFree(Buffer);
        return Result;
  ...
  uint8 *LZPBuffer =
      (uint8 *)BigAlloc(*(sint32 *)(Input + 8) + LZP_MaxMatchLen);
  if (LZPBuffer == NULL)
    return (GRZ_NOT_ENOUGH_MEMORY);
  ...
  if (Mode & GRZ_Compression_MTF)
    ...
    TSize =
        GRZip_WFC_Ari_Decode(Input + 28, (*(sint32 *)(Input + 8)), LZPBuffer);
  ...
  if (LZP_Enabled(Mode)) {
    sint32 Result =
        GRZip_LZP_Decode(LZPBuffer, TSize, Output, Get_LZP_MinMatchLen(Mode),
                         Get_LZP_HT_Size(Mode));
    if (Result == GRZ_NOT_ENOUGH_MEMORY) {
      BigFree(LZPBuffer);
      return (GRZ_NOT_ENOUGH_MEMORY);
...
struct GRZipMTCompressor : MTCompressor<> {
  ...
  GRZipMTCompressor(GRZIP_METHOD *method, CALLBACK_FUNC *_callback,
                    void *_auxdata)
    ...
    case 1:
      Mode = GRZ_Compression_BWT + GRZ_Compression_WFC;
      break;
    case 2:
      Mode = GRZ_Compression_BWT + GRZ_Compression_MTF;
      break;
    case 3:
      Mode = GRZ_Compression_ST4 + GRZ_Compression_WFC;
      break;
    case 4:
      Mode = GRZ_Compression_ST4 + GRZ_Compression_MTF;
      break;
    default:
      SetErrCode(FREEARC_ERRCODE_INVALID_COMPRESSOR); ////
    }
    Mode += method->EnableLZP ? Encode_LZP_HT_Size(method->HashSizeLog) +
                                    Encode_LZP_MinMatchLen(method->MinMatchLen)
                              : GRZ_Disable_LZP;
    Mode += method->AlternativeBWTSort ? GRZ_BWTSorting_Strong
                                       : GRZ_BWTSorting_Fast;
  ...
  int ReaderThread() {
    ...
    if ((errcode = AllocateBuffers(BlockSize + LZP_MaxMatchLen)) < 0)
      return errcode;

    char *RemainderPos;
    int RemainderSize =
        0; // остаток данных с предыдущего раза - адрес и количество
    for (;;) {
      CompressionJob *job = FreeJobs.Get(); // Acquire free job record
      if (job == NULL || ErrCode)
        break; // Quit on error in other thread

      job->InBuf = InputBuffers.Get(); // Acquire read buffer
      if (job->InBuf == NULL || ErrCode)
        break; // Quit on error in other thread

      // Перенесём необработанный остаток данных в начало буфера (может даже -
      // того же самого)
      if (RemainderSize > 0)
        memmove(job->InBuf, RemainderPos, RemainderSize);

      if ((job->InSize = callback("read", job->InBuf + RemainderSize,
                                  BlockSize - RemainderSize, auxdata)) < 0)
        return job->InSize;

      if ((job->InSize += RemainderSize) == 0)
      ...
      if (AdaptiveBlockSize) {
        // Пошукаем статистику прочитанных данных - может, нет смысла сжимать их
        // общим блоком
        sint32 NewSize =
            GRZip_GetAdaptiveBlockSize((uint8 *)job->InBuf, job->InSize);
        // Принято решение сжать только первые NewSize байт. Остальное оставим
        // на следующий раз
  ...
  void Process(CompressionJob &job, void *&) {
    int res = GRZip_CompressBlock((uint8 *)job.InBuf, job.InSize,
                                  (uint8 *)job.OutBuf, Mode);
    job.OutSize = (res >= 0 ? res : 0);
    job.result =
        (res == GRZ_NOT_ENOUGH_MEMORY ? FREEARC_ERRCODE_NOT_ENOUGH_MEMORY
  ...
  sint32 GRZip_GetAdaptiveBlockSize(uint8 *Input, sint32 Size) {
    ...
    if (!f)
      f = (FARPROC)::GRZip_GetAdaptiveBlockSize;

    return ((int(__cdecl *)(uint8 *, sint32))f)(Input, Size);
  }

  sint32 GRZip_CompressBlock(uint8 *Input, sint32 Size, uint8 *Output,
                             sint32 Mode) {
    // Use faster function from DLL if possible
    static FARPROC f = LoadFromDLL("GRZip_CompressBlock");
    if (!f)
      f = (FARPROC)::GRZip_CompressBlock;

    return ((int(__cdecl *)(uint8 *, sint32, uint8 *, sint32))f)(Input, Size,
                                                                 Output, Mode);
  }
};
...
struct GRZipMTDecompressor : MTCompressor<> {
  ...
  // Copy compression parameters into class fields
  GRZipMTDecompressor(GRZIP_METHOD *method, CALLBACK_FUNC *_callback,
                      void *_auxdata)
      : MTCompressor<>(method->GetNumThreads(), method->GetNumExtraBuffers()) {
    callback = _callback;
    auxdata = _auxdata;
  ...
  int ReaderThread() {
    ...
    for (;;) {
      ...
      if (NumRead == 0)
        break; // Конец данных
      if (NumRead != 28)
        return NumRead < 0 ? NumRead : FREEARC_ERRCODE_BAD_COMPRESSED_DATA;
      ...
      job->InSize = callback("read", job->InBuf + 28,
                             *(sint32 *)(job->InBuf + 16), auxdata);
      if (job->InSize != *(sint32 *)(job->InBuf + 16))
        return job->InSize < 0 ? job->InSize
  ...
  void Process(CompressionJob &job, void *&) {
    int res = GRZip_DecompressBlock((uint8 *)job.InBuf, job.InSize + 28,
                                    (uint8 *)job.OutBuf);
    job.OutSize = (res >= 0 ? res : 0);
    job.result =
        (res == GRZ_NOT_ENOUGH_MEMORY ? FREEARC_ERRCODE_NOT_ENOUGH_MEMORY
  ...
  sint32 GRZip_DecompressBlock(uint8 *Input, sint32 Size, uint8 *Output) {
    ...
    if (!f)
      f = (FARPROC)::GRZip_DecompressBlock;
...
int GRZIP_METHOD::doit(char *what, int param, void *data,
...
MemSize GRZIP_METHOD::GetSetDeCompressionMem(COMPRESSION direction, MemSize mem,
                                             bool MINMEM) {
  ...
  double koeff = direction == COMPRESS
                     ? (bwt ? 11 : 7)
                     : (bwt ? 7 : 7.125); // MemoryUsage/BlockSize ratio,
                                          // including 2 for InBuf+OutBuf
  LongMemSize lzpmem = direction == COMPRESS
                           ? (1 << HashSizeLog) * sizeof(uint32)
                           : 0; // Memory occupied by LZP hash
  LongMemSize tmem =
      LongMemSize(koeff * BlockSize) + lzpmem +
      1 * mb; // Сколько памяти требуется для одного треда упаковки/распаковки
  LongMemSize imem =
      2 *
      BlockSize; // Сколько памяти требуется для одного набора доп. буферов I/O

  if (mem == 0)
    return mymin(t * tmem + i * imem,
                 MEMSIZE_MAX); // Either Get(De)compressionMem() or
                               // Set(De)compressionMem(0) was called

  // Если дошли досюда - значит выполняем одну из операций SetXxxMem. В
  // зависимости от конкретной операции мы можем менять следующие параметры
  // метода: SetCompressionMem        :t:i и BlockSize SetMinCompressionMem
  // BlockSize SetDecompressionMem      :t:i SetMinDecompressionMem   BlockSize
  ...
  if (mem >= t * tmem + i * imem) {
    ...
    // Памяти достаточно для всех тредов сжатия, уменьшаем число дополнительных
    // буферов
    NumExtraBuffers = (mem - t * tmem) / imem;

  } else if (mem >= tmem && !MINMEM) {
    // Памяти достаточно хотя бы для одного треда сжатия - уменьшаем число
    // тредов сжатия и при необходимости число дополнительных буферов
    NumThreads = t = mem / tmem;
    int new_i = (mem - t * tmem) / imem;
    if (new_i < i)
      NumExtraBuffers = new_i;

  } else {
    // Памяти не хватило даже для одного треда сжатия - оставим только один и
    // ещё подожмём его
    ...
    if (direction == COMPRESS || MINMEM) {
      BlockSize = MemSize(
          mymin(mymax(double(mem) - 1 * mb, 1 * mb) / koeff, GRZ_MaxBlockSize));
      HashSizeLog = 15;
    }
  }
  return 0;
...
// параметры (функция, обратная к parse_GRZIP)
void GRZIP_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  ...
  sprintf(buf, "grzip:%s:m%d:%s%s%s%s%s%s", BlockSizeStr, Method,
          EnableLZP ? LZP_Str : "l", AlternativeBWTSort ? ":s" : "",
          AdaptiveBlockSize ? ":a" : "", DeltaFilter ? ":d" : "",
...
// или возвращает NULL, если это другой метод сжатия или допущена ошибка при
// задании параметров
COMPRESSION_METHOD *parse_GRZIP(char **parameters) {
  if (strcmp(parameters[0], "grzip") == 0) {
    // Если название метода (нулевой параметр) - "grzip", то разберём остальные
    // параметры
    ...
    while (!error && *++parameters) // Переберём все параметры метода
    {
      ...
      if (strlen(param) == 1)
        ...
        case 'm':
          p->Method = parseInt(param + 1, &error);
          continue;
        case 'b':
        ...
        case 'h':
          ...
          continue;
        case 't':
          p->NumThreads = parseInt(param + 1, &error);
          continue;
        case 'i':
          p->NumExtraBuffers = parseInt(param + 1, &error);
          continue;
        }
...
static int GRZIP_x =
    AddCompressionMethod(parse_GRZIP); // Зарегистрируем парсер метода GRZIP
