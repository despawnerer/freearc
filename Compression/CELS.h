...
#define FREEARC_ERRCODE_ONLY_DECOMPRESS                                        \
  (-3) /* Program builded with FREEARC_DECOMPRESS_ONLY, so don't try to use    \
          compress */
...
#define COMPRESSION_METHODS_DELIMITER                                          \
  '+' /* Разделитель алгоритмов сжатия в строковом описании компрессора */
...
#define MAX_METHOD_STRLEN                                                      \
  2048 /* Максимальная длина строки, описывающей метод сжатия */
...
struct COMPRESSION_METHOD {
  ...
  // размере словаря (то есть насколько далеко заглядывает алгоритм в поиске
  // похожих данных - для lz/bs схем), и размере блока (то есть сколько максимум
  // данных имеет смысл помещать в один солид-блок - для bs схем и lzp)
  ...
  // размер блока
  void LimitCompressionMem(MemSize mem) {
    if (GetCompressionMem() > mem)
      SetCompressionMem(mem);
  }
  void LimitDecompressionMem(MemSize mem) {
    if (GetDecompressionMem() > mem)
      SetDecompressionMem(mem);
  }
  void LimitDictionary(MemSize dict) {
    if (GetDictionary() > dict)
      SetDictionary(dict);
  }
  void LimitBlockSize(MemSize bs) {
    if (GetBlockSize() > bs)
      SetBlockSize(bs);
  }
#endif
...
#define READ(buf, size)                                                        \
  {                                                                            \
    ...
    int localSize = (size);                                                    \
    if (localSize &&                                                           \
        (errcode = callback(TABI_DYNAMAP("request", "read")("buf", localBuf)(  \
...
#define READ_LEN(len, buf, size)                                               \
  {                                                                            \
    if ((errcode = (len) = callback(TABI_DYNAMAP("request", "read")(           \
             "buf", (void *)(buf))("size", size))) < 0) {                      \
      goto finished;                                                           \
    }                                                                          \
  }
...
#define WRITE(buf, size)                                                       \
  {                                                                            \
     ...
     * return 0 */                                                             \
    if (localSize && (errcode = callback(TABI_DYNAMAP("request", "write")(     \
                          "buf", localBuf)("size", localSize))) < 0)           \
      goto finished;                                                           \
  }
