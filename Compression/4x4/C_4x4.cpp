...
#define _4x4_HEADER_SIZE                                                       \
  8 /* size of compressed block header (two int32 values) */

// Размер одного входного/выходного буфера
MemSize _4x4_BUFFER_SIZE(COMPRESSION direction, COMPRESSION_METHOD *cm,
                         MemSize BlockSize) {
  return (direction == COMPRESS
              ? BlockSize
              : (cm ? cm->GetMaxCompressedSize(BlockSize) : 0)) +
         _4x4_HEADER_SIZE;
}
...
struct _4x4Job : CompressionJob {
  ...
  int ReportedInSize;  // Number of in/out bytes processed already reported to
                       // host
  int ReportedOutSize; // -.-
};
...
struct _4x4MTCompressor : MTCompressor<_4x4Job, void *> {
  ...
  double MinOrder0Ratio; // Minimal order0_compression_ratio() that allow us to
                         // don't try to compress the block at all

  // Copy compression parameters into class fields
  _4x4MTCompressor(COMPRESSION _direction, _4x4_METHOD *method,
                   CALLBACK_FUNC *_callback, void *_auxdata)
      : MTCompressor<_4x4Job, void *>(method->GetNumThreads(),
                                      method->GetNumExtraBuffers()) {
    direction = _direction;
    callback = _callback;
    auxdata = _auxdata;
    cmethod = ParseCompressionMethod(method->Method);
    BlockSize = method->BlockSize || cmethod == NULL ? method->BlockSize
                                                     : cmethod->GetDictionary();
    MinOrder0Ratio = method->MinOrder0Percents / 100.0; // ratio = percents/100
  }

  ~_4x4MTCompressor() { delete cmethod; }
  ...
  int ReaderThread() {
    ...
    if (direction == COMPRESS) {
      ...
      int version;
      READ4_OR_EOF(version);
      PROGRESS(4, 0);
      if (version != _4x4_VERSION)
        return FREEARC_ERRCODE_BAD_COMPRESSED_DATA;
    }

    // For compression, allocate buffers immediately
    if (direction == COMPRESS) {
      if ((errcode = AllocateBuffers(
               _4x4_BUFFER_SIZE(direction, cmethod, BlockSize))) < 0)
        return errcode;
    }
    ...
    for (;;) {
      ...
      if (direction == COMPRESS) {
        ...
        READ4(job->InSize);
        // Now BlockSize is known so we are going to allocate buffers (extra
        // space is allocated since incompressible blocks may be inflated a bit)
        if (buf0 == NULL) {
          BlockSize =
              _4x4_BUFFER_SIZE(direction, cmethod,
                               job->OutSize > 0 ? job->OutSize : job->InSize);
          if ((errcode = AllocateBuffers(BlockSize)) < 0)
            return errcode;
          BlockSize -= _4x4_HEADER_SIZE;
        }
        // We cannot handle blocks larger than the first one
        if (job->InSize > BlockSize || job->OutSize > BlockSize)
          return FREEARC_ERRCODE_BAD_COMPRESSED_DATA;
        job->InBuf = InputBuffers.Get(); // Acquire read buffer
        if (job->InBuf == NULL || ErrCode)
          break; // Quit on error in other thread
  ...
  // CodecState initialization and destruction (see
  // COMPRESSION_METHOD::DeCompressMem() description)
  virtual int WorkerInit(void *&CodecState) {
    CodecState = NULL;
    return 0;
  }
  virtual void WorkerDone(void *&CodecState) {
    CodecState && cmethod->DeCompressMem(direction, NULL, 0, NULL, NULL, NULL,
                                         NULL, &CodecState);
  }

  virtual void
  Process(_4x4Job &job,
          void *&CodecState);       // Perform one (de)compression operation
  virtual void Write(_4x4Job &job); // Write output buffer and perform
...
double order0_compression_ratio(void *buf, size_t bufsize) {
  ...
  for (i = 0; i < bufsize - 3; i += 4)
    count1[p[i]]++, count2[p[i + 1]]++, count3[p[i + 2]]++, count4[p[i + 3]]++;
  for (; i < bufsize; i++)
    count1[p[i]]++;
...
// Pass through callback and count how many bytes was already reported to
// progress indicator (the rest will be reported once job outdata are written)
...
void _4x4MTCompressor::Process(_4x4Job &job, void *&CodecState) {
  job.Compressor = this;
  job.ReportedInSize = job.ReportedOutSize = 0;

  if (direction == COMPRESS) {
#ifndef FREEARC_DECOMPRESS_ONLY
    if (MinOrder0Ratio == 0 || job.InSize < 32 * kb ||
        order0_compression_ratio(job.InBuf + _4x4_HEADER_SIZE, job.InSize) <
            MinOrder0Ratio) {
      int OutputSize = job.InSize;
      job.result =
          cmethod->DeCompressMem(COMPRESS, job.InBuf + _4x4_HEADER_SIZE,
                                 job.InSize, job.OutBuf + _4x4_HEADER_SIZE,
                                 &OutputSize, _4x4_callback, &job, &CodecState);
      job.OutSize = OutputSize;
    } else {
      job.result = FREEARC_ERRCODE_OUTBLOCK_TOO_SMALL;
    }
    if (job.result ==
        FREEARC_ERRCODE_OUTBLOCK_TOO_SMALL) { // If we can't compress the block
                                              // - just store it
      char *buf = job.InBuf;
    ...
    if (int32(job.OutSize) ==
        -1) { // If this is stored block - just copy it intact
      char *buf = job.InBuf;
      job.InBuf = job.OutBuf;
      job.OutBuf = buf;
      job.OutSize = job.InSize;
      ...
      job.result =
          cmethod->DeCompressMem(DECOMPRESS, job.InBuf, job.InSize, job.OutBuf,
                                 &OutputSize, _4x4_callback, &job, &CodecState);
...
void _4x4MTCompressor::Write(_4x4Job &job) {
  ...
  if (job.result >= 0) {
    job.result = res;
    PROGRESS((direction == COMPRESS ? 0 : _4x4_HEADER_SIZE) + job.InSize -
                 job.ReportedInSize,
...
_4x4_METHOD::_4x4_METHOD() {
  ...
  BlockSize = 0;
  MinOrder0Percents =
      99; // don't try to compress the data if order0 entropy > 99%
}

// Универсальный метод, отвечает на запрос "has_progress?"
int _4x4_METHOD::doit(char *what, int param, void *data,
...
MemSize _4x4_METHOD::GetSetDeCompressionMem(COMPRESSION direction, MemSize mem,
                                            bool MINMEM) {
  Set_compress_all_at_once_Until_end_of_block _(1);
  ...
  // Сколько памяти требуется для одного треда упаковки/распаковки (при
  // распаковке может быть вызван только GetDecompressionMem)
  LongMemSize tmem =
#ifndef FREEARC_DECOMPRESS_ONLY
      direction == COMPRESS ? (MINMEM ? ::GetMinCompressionMem(Method)
                                      : ::GetCompressionMem(Method))
                            : (MINMEM ? ::GetMinDecompressionMem(Method)
  ...
  LongMemSize imem = 2 * LongMemSize(_4x4_BUFFER_SIZE(
                             direction, cmethod,
                             BlockSize ? BlockSize : ::GetDictionary(Method)));

  tmem += imem; // Для каждого треда упаковки выделяется набор I/O буферов

  if (mem == 0) {
    delete cmethod;
    return mymin(t * tmem + i * imem, MEMSIZE_MAX);
  } // Either Get(De)compressionMem() or Set(De)compressionMem(0) was called
  ...
  // метода: SetCompressionMem        :t:i и submethod/BlockSize
  ...
  if (mem >= t * tmem + i * imem) {
    ...
    // Во время распаковки это невозможно, поскольку потеряется совместимость с
    // упакованными с этими параметрами данными
    if (direction == COMPRESS || MINMEM) {
      ...
      direction == COMPRESS
          ? ::SetCompressionMem(Method, mem,
                                Method) // SetMinCompressionMem doesn't exist
          : ::SetMinDecompressionMem(Method, mem,
                                     Method); // SetDecompressionMem: see above
    }
#endif // !defined (FREEARC_DECOMPRESS_ONLY)
  }
  delete cmethod;
...
// параметры (функция, обратная к parse_4x4)
void _4x4_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  _4x4_METHOD defaults;
  char ThreadsStr[100], ExtraBuffersStr[100], BlockSizeStr[100],
      PercentsStr[100], PurifiedMethod[MAX_METHOD_STRLEN] = "";
  ...
  sprintf(
      buf, "4x4%s%s%s%s%s:%s",
      !purify && NumThreads != defaults.NumThreads ? ThreadsStr : "",
...
COMPRESSION_METHOD *parse_4x4(char **parameters) {
  if (strcmp(parameters[0], "4x4") == 0) {
    // Если название метода (нулевой параметр) - "4x4", то разберём остальные
    // параметры
    ...
    while (!error && *++parameters) // Переберём все параметры метода
    {
      ...
      // Если параметр начинается не с цифры или буквы плюс цифры - это начало
      // описания метода сжатия
      if (!(isdigit(param[0]) || isdigit(param[1]))) {
        join(parameters, COMPRESSION_METHOD_PARAMETERS_DELIMITER, p->Method,
             sizeof(p->Method));
      ...
      if (strlen(param) == 1)
        ...
        case 'i':
          ...
          continue;
        case 'r':
          p->MinOrder0Percents = parseDouble(param + 1, &error);
          continue;
        }

      // Сюда мы попадаем, если в параметре не указано его название
      // Если этот параметр удастся разобрать как целое число (т.е. в нём -
      // только цифры), то присвоим его значение полю Threads, иначе попробуем
      // разобрать его как BlockSize
...
static int _4x4_x =
    AddCompressionMethod(parse_4x4); // Зарегистрируем парсер метода 4x4
