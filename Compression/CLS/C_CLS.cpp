...
COMPRESSION_METHOD *parse_CLS(char **parameters, void *method_template) {
  if (strequ(parameters[0], ((CLS_METHOD *)method_template)->name)) {
    ...
    // проверяемого CLS метода, то запомним остальные параметры и возвратим его
    CLS_METHOD *p = new CLS_METHOD(*(CLS_METHOD *)method_template);
    // Save method parameters
    join(parameters + 1, COMPRESSION_METHOD_PARAMETERS_DELIMITER, p->params,
         sizeof(p->params));
    return p;
  } else {
...
static void CLS_BeforeUnloadDLL() {
  while (registered_methods) {
    registered_methods--;
    CLS_MAIN *ClsMain =
        (CLS_MAIN *)GetProcAddress(CLS_Handles[registered_methods], "ClsMain");
