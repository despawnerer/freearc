...
public:
  ...
  MemSize BlockSize; // Размер блока данных, обрабатываемого за раз. Каждый блок
                     // получает свой собственный словарь
  int MinCompression; // Минимальный процент сжатия. Если выходные данные
                      // больше, то вместо них будут записаны оригинальные
                      // (несжатые) данные
  int MinWeakChars; // Минимально допустимое количество weak chars. Если оно
                    // окажется меньше - произойдёт отказ от сжатия, поскольку
                    // малые значения weak chars обычно свидетельствуют о том,
                    // что это бинарный файл, который этому алгоритму сжать не
                    // удастся
