...
int LZ4_METHOD::DeCompressMem(COMPRESSION direction, void *input, int inputSize,
  ...
  if (direction == COMPRESS) {
    ...
    *(char *)output = LZ4_VERSION;
    // LZ4_compress*() return compressed size, or 0 if the compression failed
    // for any reason
    *outputSize = Compressor
                      ? LZ4HC::LZ4_compressHC((const char *)input,
                                              (char *)output + 1, inputSize)
                      : LZ4_compress_limitedOutput((const char *)input,
                                                   (char *)output + 1,
                                                   inputSize, *outputSize - 1);
    return (*outputSize)++ <= 0 ||
                   MinCompression > 0 &&
                       *outputSize >= (double(inputSize) * MinCompression) / 100
               ? FREEARC_ERRCODE_OUTBLOCK_TOO_SMALL
               : FREEARC_OK;
#else
    return FREEARC_ERRCODE_ONLY_DECOMPRESS;
#endif // !defined (FREEARC_DECOMPRESS_ONLY)
  } else {
    if (inputSize <= 0 || *(char *)input != LZ4_VERSION)
      return FREEARC_ERRCODE_BAD_COMPRESSED_DATA;
    // LZ4_uncompress_unknownOutputSize() returns output size, or negative value
    // if decompression failed
    int result = LZ4_uncompress_unknownOutputSize(
        (const char *)input + 1, (char *)output, inputSize - 1, *outputSize);
    return (result == *outputSize ? FREEARC_OK
...
int LZ4_METHOD::decompress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  int len;
  READ_LEN_OR_EOF(len, In, 1);
  if (len != 1 || *In != LZ4_VERSION)
  ...
  for (;;) {
    ...
    if (InSize < 0) {
      ...
      READ(In, InSize);
      // LZ4_uncompress_unknownOutputSize() returns output size, or negative
      // value if decompression failed
      OutSize = LZ4_uncompress_unknownOutputSize((const char *)In, (char *)Out,
                                                 InSize, BlockSize);
      if (OutSize < 0)
        ReturnErrorCode(FREEARC_ERRCODE_BAD_COMPRESSED_DATA);
      WRITE(Out, OutSize);
...
int LZ4_METHOD::compress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  for (bool FirstTime = true;; FirstTime = false) {
    ...
    // for any reason
    OutSize = Compressor
                  ? LZ4HC::LZ4_compressHC((const char *)In, (char *)Out, InSize)
                  : LZ4_compress_limitedOutput((const char *)In, (char *)Out,
                                               InSize, InSize);
    if (OutSize <= 0 ||
        MinCompression > 0 &&
...
COMPRESSION_METHOD *parse_LZ4(char **parameters) {
  if (strcmp(parameters[0], "lz4") == 0) {
    // Если название метода (нулевой параметр) - "lz4", то разберём остальные
    // параметры
    ...
    while (*++parameters && !error) {
      ...
      if (strequ(param, "hc")) {
        ...
        case 'c':
          p->Compressor = parseInt(param + 1, &error);
          continue;
        case 'b':
          p->BlockSize = parseMem(param + 1, &error);
          continue;
        case 'h':
          p->HashSize = parseMem(param + 1, &error);
          continue;
        }
...
static int LZ4_x =
    AddCompressionMethod(parse_LZ4); // Зарегистрируем парсер метода LZ4
