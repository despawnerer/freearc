...
#define TABI_GETTER(TYPE, FIELDTYPE, NAME, TYPECONST, FIELD)                   \
  ...
  TABI_GETTER(unsigned long long, long long, _ulonglong, TABI_INTEGER,
              int_number);
  ...
  void dump(char *title = NULL, int n = 0) {
    ...
    for (int i = 0; i < n ? n : 100; i++) {
      ...
      case TABI_STRING:
        if (p[i].value.str)
          fprintf(dumpfile, "\"%s\"", p[i].value.str);
        break;
      case TABI_WIDE_STRING:
        if (p[i].value.wstr)
          fwprintf(dumpfile, L"L\"%s\"", p[i].value.wstr);
        break;
      case TABI_INTEGER:
        fprintf(dumpfile, "%lld", p[i].value.int_number);
        break;
      case TABI_FLOATING:
        fprintf(dumpfile, "%g", p[i].value.float_number);
        break;
      case TABI_PTR:
        fprintf(dumpfile, "<%p>", p[i].value.ptr);
        break;
      case TABI_FUNCPTR:
        fprintf(dumpfile, "func<%p>", p[i].value.funcptr);
        break;
      case TABI_BOOL:
        fprintf(dumpfile, "%s", p[i].value.bool_value ? "TRUE" : "FALSE");
        break;
      }
    }
    fprintf(dumpfile, "\n");
  ...
  void memory_dump(int n = 0) {
    ...
    for (int i = 0; i < n ? n : 100; i++) {
      for (int j = 0; j < 4; j++)
        fprintf(dumpfile, "%02x ", *((unsigned char *)(p + i) + j));
      fprintf(dumpfile, "  ");
      for (int j = 4; j < 8; j++)
        fprintf(dumpfile, "%02x ", *((unsigned char *)(p + i) + j));
      fprintf(dumpfile, "  ");
      ...
      case TABI_WIDE_STRING:
        if (p[i].value.wstr)
          fwprintf(dumpfile, L"\"%s\"", p[i].value.wstr);
        break;
      case TABI_INTEGER:
        fprintf(dumpfile, "%lld", p[i].value.int_number);
        break;
...
public:
  TABI_DYNAMAP() : TABI_MAP(place) {
    i = 0;
    p[i].name = NULL;
  }
  template <class T> TABI_DYNAMAP(T v) : TABI_MAP(place) {
    i = 0;
    p[i].name = NULL;
    (*this)("", v);
  }
  template <class T> TABI_DYNAMAP(char *n, T v) : TABI_MAP(place) {
    i = 0;
    p[i].name = NULL;
    (*this)(n, v);
  }
...
#define TABI_SETTER(TYPE, FIELDTYPE, NAME, TYPECONST, FIELD)                   \
  ...
  TABI_SETTER(unsigned long long, long long, _ulonglong, TABI_INTEGER,
              int_number);
...
// Return value using "return" callback
template <class T> TABI_RESULT_TYPE TABI_MAP::_return(T v) {
  return _callback("return")(TABI_DYNAMAP("result", v));
}
