...
// WAV header doesn't exist or don't taken into account. This array is
// zero-ended
...
static long read_wave(long *data, void *rest, void **bufferptr, void *prevbuf,
                      long prevsize, long byte_size, long num_chan,
                      unsigned long len, CALLBACK_FUNC *callback,
                      void *auxdata) {
  ...
  long bytes_read = wanted <= prevsize ? 0
                                       : callback("read", buffer + prevsize,
                                                  wanted - prevsize, auxdata);
...
static long write_wave(long **data, long byte_size, long num_chan,
                       unsigned long len, CALLBACK_FUNC *callback,
                       void *auxdata) {
  long n;
  long i, res;
...
#ifndef FREEARC_DECOMPRESS_ONLY
int tta_compress(int level, int skip_header, int is_float, int num_chan,
                 int word_size, int offset, int raw_data,
  ...
  // Auto-detect settings for is_float/num_chan/word_size/offset unless any of
  ...
  if (level == 0) {
    ...
    // If both WAV file header and entropy autodetection fails use default
    // values
    if (!skip_header && autodetect_wav_header(prevbuf, prevsize, &is_float,
                                              &num_chan, &word_size, &offset)) {
      print_message(
          (stdout,
           "WAV header: is_float %d, num_chan %d, word_size %d, offset %d\n",
           is_float, num_chan, word_size, offset));
    } else if (autodetect_by_entropy((char *)prevbuf, prevsize, channels,
                                     bitvalues, 0.50, &is_float, &num_chan,
                                     &word_size, &offset)) {
    ...
    storing:
      ...
      while (1) {
        if (prevbuf != NULL) {
          ...
          prevbuf = (char *)malloc1d(1 * mb, 1);
        READ_LEN_OR_EOF(prevsize, prevbuf, 1 * mb);
      }
    }
  }
  // printf ("Data detected: is_float %d, num_chan %d, word_size %d, offset
  // %d\n", is_float, num_chan, word_size, offset);
  byte_size = (word_size + 7) / 8; // bytes per word

  // TTA doesn't support 32+ bit integers and non-32-bit floats
  if ((is_float && byte_size != 4) || (!is_float && byte_size >= 4))
    goto storing;
  ...
  if (offset > 0 &&
      prevptr == NULL) { // If header isn't yet in prevbuf - read it
  ...
  prevptr += offset;
  prevsize -= offset; // Exclude these data from further processing
  // printf ("offset ok\n");
  ...
  while (1) {
    ...
    bytes_read = read_wave(data, rest, &origdata, prevptr, prevsize, byte_size,
                           num_chan, frame_size, callback, auxdata);
    frame_len = bytes_read / (num_chan * byte_size);
    if (bytes_read >= prevsize) {
      BigFreeAndNil(prevbuf);
      prevsize =
          0; // Now prevbuf (data read for autodetection) is fully consumed
    } else {
      prevptr += bytes_read;
      prevsize -= bytes_read; // There are still (prevptr,prevsize) data to read
                              // in prevbuf
    }
    if ((errcode = bytes_read) <= 0)
      goto finished; // Leave loop on EOF or error reading data
    // printf ("Data read\n");
    ...
    init_bit_array_write(
        num_chan * byte_size *
        (frame_size + 100)); // it should be larger than any bytes_read
...
int tta_decompress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  void *rest = NULL;
  void *prevbuf = NULL;
  unsigned long i, level, raw_data, num_chan, word_size, byte_size, bytes_read,
      offset;
  ...
  if (level > 3 || raw_data > 1 || (is_float && byte_size != 4) ||
      (!is_float && byte_size >= 4))
    return FREEARC_ERRCODE_BAD_COMPRESSED_DATA;

  // If TTA header signals that this is stored data, then just copy input to
  // output
  if (level == 0) {
  ...
  while (1) {
    ...
    // read block data
    if (!raw_data) {
      // Read second part of block header which stores *compressed* size of
      // block
    ...
    if (frame_len) {
      errcode =
          write_wave(buffer, byte_size, num_chan, frame_len, callback, auxdata);
      if (errcode < 0)
        goto finished;
    }
    BigFreeAndNil(buffer);
...
// DRIVER
// ************************************************************************ This
// demo program shows how to use TTA sound wave compressor

// to do:
// 1. -w4 - error!!! the same for other encodings where compression really
// doesn't work
// 2. 12*0 - stored file, compression params in each block, TTA header for grzip
// algo
// 3. cobalp?
// 4. class Buffer?
...
int writeFILE(/*void* param,*/ void *buf, int size) {
  ...
  if (show_stat && size > 4) {
    ...
    if (!unpack) {
      fprintf(
          stdout,
          "Encode:  wrote %.0f bytes, %.0f%% complete, ratio: %.2f, time: %d\r",
      ...
      fprintf(
          stdout,
          "Decode:  wrote %.0f bytes, %.0f%% complete, ratio: %.2f, time: %d\r",
          (double)output_byte_count,
          (double)input_byte_count / (data_size + 1) * 100,
          (double)output_byte_count / (input_byte_count + 1),
          (int)(time(NULL) - stime));
...
int main(int argc, char **argv) {
  int tta_level = 3; // Compression level (1..3, higher means tighter and slower
                     // compression)
  ...
  int raw_data =
      0; // Write raw predictor's output without using entropy encoder
  ...
  !unpack ? tta_compress(tta_level, skip_header, is_float, num_chan, word_size,
                         offset, raw_data, readFILE, fin, writeFILE, fout)
