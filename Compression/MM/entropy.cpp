...
const unsigned long bit_shift[40] = {
    ...
    0x40000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000,
...
__inline void put_binary(unsigned long value, unsigned long bits) {
  ...
  if ((pos << 2) + 8 > bit_array_write_size) {
    // bit_array_write = (unsigned char *) realloc (bit_array_write,
    // bit_array_write_size += STEP_SIZE); if (!bit_array_write) tta_error
    // (MEMORY_ERROR, NULL);
    bit_array_write_bits =
        UINT_MAX; // Signal to use original (uncompressed) data instead
    return;
  }
...
__inline void put_unary(unsigned long value) {
  ...
  if ((pos << 2) + 8 + value / 8 > bit_array_write_size) {
    // bit_array_write = (unsigned char *) realloc (bit_array_write,
    // bit_array_write_size += mymax(STEP_SIZE,value/8+8)); if
    // (!bit_array_write) tta_error (MEMORY_ERROR, NULL);
    bit_array_write_bits =
        UINT_MAX; // Signal to use original (uncompressed) data instead
    return;
  }
  ...
  *s &= bit_mask32[fbit];
  if (value < rbit)
    *s |= (bit_mask32[value]) << fbit;
  else {
    unsigned long unary = value;
    *s++ |= (bit_mask32[rbit]) << fbit;
    unary -= rbit;
    for (; unary > 32; unary -= 32)
      *s++ = bit_mask32[32];
    if (unary)
      *s = bit_mask32[unary];
  }
...
__inline void get_unary(unsigned long *value) {
  ...
  *value = 0;
  ...
  if ((*s >> fbit) == bit_mask32[rbit]) {
    *value += rbit;
    fbit = 0;
    while (*(++s) == bit_mask32[32])
      *value += 32;
  }
  for (mask <<= fbit; *s & mask; mask <<= 1)
    (*value)++;
