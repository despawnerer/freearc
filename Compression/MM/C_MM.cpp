...
int MM_METHOD::compress(CALLBACK_FUNC *callback, void *auxdata) {
  return mm_compress(mode, skip_header, is_float, num_chan, word_size, offset,
                     reorder, callback, auxdata);
}
...
COMPRESSION_METHOD *parse_MM(char **parameters) {
  if (strcmp(parameters[0], "mm") == 0) {
    // Если название метода (нулевой параметр) - "mm", то разберём остальные
    // параметры
    ...
    while (*++parameters && !error) {
      ...
      case 'o':
        ...
        continue;
      case 'r':
        p->reorder = parseInt(param + 1, &error);
        continue;
      }
...
static int MM_x =
    AddCompressionMethod(parse_MM); // Зарегистрируем парсер метода MM
