...
__inline void filter_compress(fltst *fs, long *in) {
  long *pA, *pB, *pE;
  long out, sum;
  ...
  *(pA - 3) = ((*(pB - 3) >> 30) & 2) - 1;
  ...
  if (fs->px + fs->order == fs->dx + (BUF_SIZE - 1)) {
    memcpy(fs->dx, fs->px + 1, fs->order * sizeof(long));
    fs->px = fs->dx;
  } else
    fs->px++;

  if (fs->pl + fs->order == fs->dl + (BUF_SIZE - 1)) {
    memcpy(fs->dl, fs->pl + 1, fs->order * sizeof(long));
    fs->pl = fs->dl;
  } else
    fs->pl++;
...
__inline void filter_decompress(fltst *fs, long *in) {
  long *pA, *pB, *pE;
  long out, sum;
