...
void diff2(void *buf, int bufsize, int N, void *_base) {
  ...
  for (uint16 *p = (uint16 *)buf; p + N <= (uint16 *)((char *)buf + bufsize);
       p += N)
    for (int i = 0; i < N; i++)
      x = p[i], p[i] -= base[i], base[i] = x;
}
...
void diff4(void *buf, int bufsize, int N, void *_base) {
  ...
  for (uint32 *p = (uint32 *)buf; p + N <= (uint32 *)((char *)buf + bufsize);
       p += N)
    for (int i = 0; i < N; i++)
      x = p[i], p[i] -= base[i], base[i] = x;
}

// Reorder buffer contents so that data for each byte of each channel are placed
// continuosly
...
BYTE *reorder_words(BYTE *buf, int bufsize, int N, int width) { return buf; }
...
int mm_compress(int mode, int skip_header, int is_float, int num_chan,
                int word_size, int offset, int reorder, CALLBACK_FUNC *callback,
                void *auxdata) {
  const int BUFSIZE = 1 * mb;
  BYTE *buf = (BYTE *)malloc(
      BUFSIZE +
      1); // buffer for data processed, 1 more byte for diff24 safeness
  void *base = NULL; // previous values for all channels what will be
                     // substracted from next ones
  ...
  if ((errcode = bytes = callback("read", buf, BUFSIZE, auxdata)) <= 0)
    goto finished; // temporary! BUFFER_SIZE will be enough

  { // Select parameters of check depending on speed mode setting
    int check_bytes = mymin(mode <= 2 ? 64 * kb : mymax(64 * kb, bytes / 2),
                            bytes); // how many bytes to check
    ...
    if (is_float || num_chan || word_size) {
      ...
      print_message(
          (stdout,
           "User-defined: is_float %d, num_chan %d, word_size %d, offset %d\n",
           is_float, num_chan, word_size, offset));
    } else if (!skip_header &&
               autodetect_wav_header(buf, bytes, &is_float, &num_chan,
                                     &word_size, &offset)) {
      print_message(
          (stdout,
           "WAV header: is_float %d, num_chan %d, word_size %d, offset %d\n",
           is_float, num_chan, word_size, offset));
    } else if (autodetect_by_entropy(buf + (bytes - check_bytes) / 2,
                                     check_bytes, use_channels, use_bitvalues,
                                     0.80, &is_float, &num_chan, &word_size,
                                     &offset)) {
      print_message((stdout,
                     "Entropy analyzer: is_float %d, num_chan %d, word_size "
                     "%d, offset %d\n",
                     is_float, num_chan, word_size, offset));
    } else {
    ...
    if (offset > bytes)
      offset = bytes; // otherwise we try to write negative amount of data :)
    int byte_size = (word_size + 7) / 8; // bytes per word
    int N = num_chan * byte_size;        // bytes per sample
    int chunk =
        roundDown(BUFSIZE, N); // How many bytes should be processed each time
    int rest = bytes;
    bytes -=
        offset; // First 'offset' bytes will be copied intact w/o any processing
    bytes =
        roundDown(bytes, N); // Не обрабатывать первый блок данных целиком, если
                             // он содержит нецелое число отсчётов. Вместо этого
                             // оставшиеся байты будут включены во второй блок
    rest -=
        offset + bytes; // how many bytes will remain unprocessed at end of buf
    base = calloc(num_chan,
                  byte_size == 3
                      ? 4
                      : byte_size); // previous values for all channels what
                                    // will be substracted from next ones
    BYTE *ptr = buf + offset; // In first block we should skip already copied
                              // data, in the following blocks ptr==buf
    ...
    if (N == 0) {
      ...
      // Write header
      byte header[3];
      header[0] = 1 + reorder * 2; // Flags byte: bit0 - diff, bit1 -
                                   // reorder_bytes, bit2 - reorder_words
      header[1] = num_chan;
      header[2] = word_size;
      WRITE(header, 3);

      // Copy first few bytes intact in order to align diffX to start of
      // 16/24/32-bit data samples
      WRITE4(offset);
      WRITE(buf, offset);

      // Write a few zero bytes in order to align output to position divisible
      // by N
    ...
    for (;;) {
      switch (byte_size) {
      case 1:
        diff1(ptr, bytes, num_chan, base);
        break;
      case 2:
        diff2(ptr, bytes, num_chan, base);
        break;
      case 3:
        diff3(ptr, bytes, num_chan, base);
        break;
      case 4:
        diff4(ptr, bytes, num_chan, base);
        break;
      }
      switch (reorder) {
      case 1:
        reorder_bytes(ptr, bytes, num_chan, byte_size);
        break;
      case 2:
        reorder_words(ptr, bytes, num_chan, byte_size);
        break;
      }
      WRITE(ptr, bytes);
      ...
      buf = (BYTE *)realloc(
          buf,
          chunk +
              1); // +1 is for processing 24-bit chunks using 32-bit operations

      // Read next data chunk. This time, bytes+rest should be exactly == chunk,
      // unless we at the end of data
...
int mm_decompress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  void *buf1 = NULL;
  void *base = NULL; // room for previous values for all channels what will be
                     // substracted from next ones
  int errcode, // код, возвращённый последней операцией чтения/записи
      bytes;   // how many bytes was read to buf
  ...
  if (header[0] == 0) {
    // Copy data in chunks
    while ((errcode = bytes = callback("read", buf, BUFFER_SIZE, auxdata)) >
           0) {
      WRITE(buf, bytes);
    }
  } else {
    // Check that bits other than bit0-bit2 are not set (these should be used
    // for future extensions) Well, just now we don't support even bits1&2
    // enabled (i.e. we don't support restoring proper byte order)
    if (header[0] & ~1) {
    ...
    base = calloc(num_chan,
                  byte_size == 3
                      ? 4
                      : byte_size); // room for previous values for all channels
                                    // what will be substracted from next ones
    READ(base,
         roundUp(3 + 4 + offset, N) - (3 + 4 + offset)); // skip alignment bytes

    // Process data in chunks
    while ((errcode = bytes = callback("read", buf, chunk, auxdata)) > 0) {
      switch (byte_size) {
      case 1:
        undiff1(buf, bytes, num_chan, base);
        break;
      case 2:
        undiff2(buf, bytes, num_chan, base);
        break;
      case 3:
        undiff3(buf, bytes, num_chan, base);
        break;
      case 4:
        undiff4(buf, bytes, num_chan, base);
        break;
      } /*
switch (reorder) {
case 1 : unreorder_bytes (buf, bytes, num_chan, byte_size);  break;  //
temporary! case 2 : unreorder_words (buf, bytes, num_chan, byte_size);  break;
}             */
      WRITE(buf, bytes);
    }
  }
...
// DRIVER
// ************************************************************************ This
// demo program shows how to use MM preprocessor
...
int writeFILE(/*void* param,*/ void *buf, int size) {
  if (output_byte_count == 0) {
    ...
    !unpack ? fprintf(stdout, "Encode:  processing ..\r")
...
// Разбор командной строки, чтение входных данных, вызов encode/decode, и запись
// выходных данных
int main(int argc, char **argv) {
  ...
  while (argc > 1) {
    if (argv[1][0] == '-') {
      ...
      case 'f':
        ...
        break;
      case 'c':
        num_chan = atoi(argv[1] + 2);
        break;
      case 'w':
        word_size = atoi(argv[1] + 2);
        break;
      case 'o':
        offset = atoi(argv[1] + 2);
        break;
      case 'r':
        reorder = atoi(argv[1] + 2);
        break;
      default:
        printf("\n Unknown option '%s'\n", argv[1]);
        exit(1);
  ...
  if (argc != 2 && argc != 3) {
    ...
    printf("\n   -r# -- reorder data. -r1 - reorder bytes, -r2 - reorder words "
           "(unfinished!)");
  ...
  !unpack ? mm_compress(mode, skip_header, is_float, num_chan, word_size,
                        offset, reorder, readFILE, writeFILE)
