...
// If you need a commercial license to use the code, please write to
...
const int MAX_MATCHES =
    2048 + 256 + 16; // maximum length of hash row plus a few low-order matches
                     // from auxiliary MFs
...
struct PackMethod {
  ...
  int encoding_method; // Coder (0 - storing, 1 - bytecoder, 2 - bitcoder, 3 -
                       // huffman, 4 - arithmetic)
  ...
  int match_parser;    // Match parser (1 - greedy, 2 - lazy, 3 - flexible, 4 -
                       // optimal, 5 - even better)
  ...
  int fast_bytes; // Optimal parser: after a match/repmatch longer than or equal
                  // to fast_bytes, we skip everything until match end in order
                  // to avoid O(n^2) time behavior
};
...
PackMethod std_Tornado_method[] =
    //                 tables row  hashsize  matchfinder       buffer  parser
    //                 hash3 shift update      auxhash  fast_bytes
    {{0, STORING, false, 0, 0, NON_CACHING_MF, 1 * mb, 0, 0, 0, 999, 0, 0, 128},
     {1, BYTECODER, false, 1, 16 * kb, NON_CACHING_MF, 1 * mb, GREEDY, 0, 0,
      999, 0, 0, 128},
     {2, BITCODER, false, 1, 64 * kb, NON_CACHING_MF, 2 * mb, GREEDY, 0, 0, 999,
      0, 0, 128},
     {3, HUFCODER, true, 2, 128 * kb, NON_CACHING_MF, 4 * mb, GREEDY, 0, 0, 999,
      0, 0, 128},
     {4, HUFCODER, true, 2, 2 * mb, CACHING_MF4, 8 * mb, GREEDY, 0, 0, 999, 0,
      0, 128},
     {5, ARICODER, true, 4, 8 * mb, CACHING_MF4, 16 * mb, LAZY, 1, 0, 999, 0, 0,
      128},
     {6, ARICODER, true, 8, 32 * mb, CACHING_MF4, 64 * mb, LAZY, 1, 0, 4, 0, 0,
      128},
     {7, ARICODER, true, 32, 128 * mb, CYCLED_MF5, 256 * mb, LAZY, 2, 0, 1,
      128 * kb, 4, 128},
     {8, ARICODER, true, 128, 512 * mb, CYCLED_MF5, 1 * gb, LAZY, 2, 0, 1,
      128 * kb, 4, 128},
     {9, ARICODER, true, 256, 2 * gb, CYCLED_MF5, 1 * gb, LAZY, 2, 0, 1,
      512 * kb, 4, 128},
     {10, ARICODER, true, 256, 2 * gb, CYCLED_MF7, 1 * gb, LAZY, 2, 0, 1,
      16 * mb, 128, 128},
     {11, ARICODER, true, 8, 256 * mb, CACHING_MF5, 128 * mb, OPTIMAL, 1, 0, 1,
      64 * kb, 1, 16},
     {12, ARICODER, true, 16, 256 * mb, CACHING_MF5, 128 * mb, OPTIMAL, 1, 0, 1,
      64 * kb, 1, 32},
     {13, ARICODER, true, 24, 384 * mb, CACHING_MF5, 128 * mb, OPTIMAL, 1, 0, 1,
      128 * kb, 1, 64},
     {14, ARICODER, true, 32, 512 * mb, CACHING_MF6, 128 * mb, OPTIMAL, 2, 0, 1,
      2 * mb, 2, 64},
     {15, ARICODER, true, 64, 512 * mb, CACHING_MF6, 128 * mb, OPTIMAL, 2, 0, 1,
      2 * mb, 4, 128},
     {16, ARICODER, true, 128, 128 * mb, BT_MF5, 128 * mb, OPTIMAL, 2, 0, 1,
      4 * mb, 8, 512}};

// Default compression parameters are equivalent to option -5
const int default_Tornado_method = 5;

// If data table was not encountered in last table_dist bytes, don't check next
// table_shift bytes in order to make things faster
const int table_dist = 256 * 1024, table_shift = 128;

// Minimum lookahead for next match which compressor tries to guarantee.
// Also minimum amount of allocated space after end of buf (this allows to use
// things like p[11] without additional checks)
...
uint round_to_nearest_hashsize(LongMemSize hashsize, uint hash_row_width) {
  return mymin(round_to_nearest_power_of(
                   mymin(hashsize, 2 * gb - 1) / hash_row_width, 2) *
                   hash_row_width,
               2 * gb - 1);
}

// Dictionary size depending on memory available for dictionary+outbuf (opposite
// to tornado_compressor_outbuf_size)
uint tornado_compressor_calc_dict(uint mem) {
  return compress_all_at_once          ? mem / 9 * 4
         : mem > 2 * LARGE_BUFFER_SIZE ? mem - LARGE_BUFFER_SIZE
                                       : mem / 2;
}

// Output buffer size for compressor (the worst case is bytecoder that adds 2
// bits per byte on incompressible data)
uint tornado_compressor_outbuf_size(uint buffer, int bytes_to_compress = -1) {
  return bytes_to_compress != -1
             ? bytes_to_compress + (bytes_to_compress / 4) + 512
...
#define CHECK_FOR_DATA_TABLE(N)                                                \
  {                                                                            \
    if (p[-1] == p[N - 1] && uint(p[N - 1] - p[2 * N - 1] + 4) <= 2 * 4 &&     \
        uint(p[2 * N - 1] - p[3 * N - 1] + 4) <= 2 * 4 &&                      \
        !val32equ(p + 2 * N - 4, p + N - 4)) {                                 \
      int type, items;                                                         \
      if (check_for_data_table(N, type, items, p, bufend, table_end, buf,      \
                               offset, last_checked)) {                        \
        coder.encode_table(type, items);                                       \
        /* If data table was diffed, we should invalidate match cached by lazy \
         * match finder */                                                     \
        mf.invalidate_match();                                                 \
        goto found;                                                            \
      }                                                                        \
...
int read_next_chunk(PackMethod &m, CALLBACK_FUNC *callback, void *auxdata,
                    MatchFinder &mf, Coder &coder, byte *&p, byte *buf,
                    BYTE *&bufend, BYTE *&table_end, BYTE *&last_found,
                    BYTE *&read_point, int &bytes, int &chunk, uint64 &offset,
                    byte *(&last_checked)[MAX_TABLE_ROW][MAX_TABLE_ROW]) {
  if (bytes == 0 || compress_all_at_once)
    return 0; // All input data was successfully compressed
  // If we can't provide 256 byte lookahead then shift data toward buffer
  // beginning, freeing space at buffer end for the new data
  if (bufend - buf > m.buffer - LOOKAHEAD) {
    ...
    if (coder.support_tables && m.find_tables) {
      ...
      iterate_var(i, MAX_TABLE_ROW) iterate_var(j, MAX_TABLE_ROW)
          last_checked[i][j] = buf;
    }
    mf.invalidate_match(); // invalidate match stored in lazy MF; otherwise it
                           // may fuck up the NEXT REPCHAR checking
    coder.after_shift(p);  // tell to the coder what shift occurs
    debug(printf("==== SHIFT %08x: p=%08x ====\n", shift, p - buf));
  }
  bytes =
      callback("read", bufend, mymin(chunk, buf + m.buffer - bufend), auxdata);
  debug(printf("==== read %08x ====\n", bytes));
  if (bytes < 0)
    return bytes; // Return errcode on error
  bufend += bytes;
  read_point = bytes == 0 ? bufend : bufend - LOOKAHEAD;
  coder.flush();             // Sometimes data should be written to disk :)
  return p < bufend ? 1 : 0; // Result code: 1 if we still have bytes to
                             // compress, 0 otherwise
}

// Compress one chunk of data
template <class MatchFinder, class Coder>
int tor_compress_chunk(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
                       byte *buf, int bytes_to_compress) {
  // Read data in these chunks
  int chunk = compress_all_at_once
                  ? m.buffer
                  : mymin(m.shift > 0 ? m.shift : m.buffer, LARGE_BUFFER_SIZE);
  uint64 offset = 0; // Current offset of buf[] contents relative to file
                     // (increased with each shift() operation)
  int bytes =
      bytes_to_compress != -1
          ? bytes_to_compress
          : callback("read", buf, chunk,
                     auxdata); // Number of bytes read by last "read" call
  if (bytes < 0)
    return bytes;             // Return errcode on error
  BYTE *bufend = buf + bytes; // Current end of real data in buf[]
  BYTE *matchend =
      bufend -
      mymin(MAX_HASHED_BYTES,
            bufend - buf); // Maximum pos where match may finish (less than
                           // bufend in order to simplify hash updating)
  BYTE *read_point =
      compress_all_at_once || bytes_to_compress != -1
          ? bufend
          : bufend - mymin(LOOKAHEAD, bytes); // Next point where next chunk of
                                              // data should be read to buf
  BYTE *progress_point = buf, *next_special_p = buf;
  uint64 prev_insize = 0, prev_outsize = 0;
  // Match finder will search strings similar to current one in previous data
  MatchFinder mf(buf, m.buffer, m.hashsize, m.hash_row_width, m.auxhash_size,
                 m.auxhash_row_width);
  if (mf.error() != FREEARC_OK)
    return mf.error();
  // Coder will encode LZ output into bits and put them to outstream
  // Data should be written in HUGE_BUFFER_SIZE chunks (at least) plus chunk*2
  // bytes should be allocated to ensure that no buffer overflow may occur
  // (because we flush() data only after processing each 'chunk' input bytes)
  Coder coder(m.encoding_method, callback, auxdata,
              tornado_compressor_outbuf_size(m.buffer, bytes_to_compress),
              compress_all_at_once ? 0 : chunk * 2);
  if (coder.error() != FREEARC_OK)
    return coder.error();
  BYTE *table_end =
      coder.support_tables && m.find_tables
          ? buf
          : buf + m.buffer + LOOKAHEAD; // The end of last data table processed
  BYTE *last_found = buf; // Last position where data table was found
  byte *last_checked[MAX_TABLE_ROW]
                    [MAX_TABLE_ROW]; // Last position where data table of size
                                     // %1 with offset %2 was tried
  ...
  // Encode first four bytes directly (at least 2 bytes should be saved directly
  // in order to avoid problems with using p-2 in MatchFinder.update())
  BYTE *p;
  for (p = buf; p < buf + 4; p++) {
    if (p >= bufend)
      goto finished;
  ...
  while (TRUE) {
    if (p >= next_special_p) {
      // Read next chunk of data if all data up to read_point was already
      // processed
      if (p >= read_point) {
        if (bytes_to_compress != -1)
          goto finished; // We shouldn't read/write any data!
        byte *p1 = p;    // This trick allows to not take address of p and this
                         // buys us a bit better program optimization
        int res = read_next_chunk(m, callback, auxdata, mf, coder, p1, buf,
                                  bufend, table_end, last_found, read_point,
                                  bytes, chunk, offset, last_checked);
        progress_point = p = p1,
        matchend = bufend - mymin(MAX_HASHED_BYTES, bufend - buf);
        if (res == 0)
          goto finished; // All input data were successfully compressed
        if (res < 0)
          return res; // Error occurred while reading data
      }
      // Report to the caller samples of insize/outsize ratio
      if (p >= progress_point) {
        PROGRESS((offset + (p - buf)) - prev_insize,
                 coder.outsize() - prev_outsize);
    ...
    if (coder.support_tables && p > table_end) {
      if (mf.min_length() <
          4) // increase speed by skipping this check in faster modes
...
finished:
  stat_only(printf("\nTables %d * %d = %d bytes\n", int(table_count),
                   int(table_sumlen / mymax(table_count, 1)),
                   int(table_sumlen)));
...
int tor_compress0(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
                  void *buf0, int bytes_to_compress) {
  // SET_JMP_POINT( FREEARC_ERRCODE_GENERAL);
  //  Make buffer at least 32kb long and round its size up to 4kb chunk
  m.buffer = bytes_to_compress == -1 ? (mymax(m.buffer, 32 * kb) + 4095) & ~4095
                                     : bytes_to_compress;
  // If hash is too large - make it smaller
  if (m.hashsize / 8 > m.buffer)
    m.hashsize = 1 << lb(m.buffer * 8);
  if (m.auxhash_size / 8 > m.buffer)
    m.auxhash_size = 1 << lb(m.buffer * 8);
  // >0: shift data in these chunks, <0: how many old bytes should be kept when
  // buf shifts, -1: don't slide buffer, fill it with new data instead
  m.shift = m.shift ? m.shift
                    : (m.hash_row_width > 4     ? m.buffer / 4
  ...
  void *buf =
      buf0 ? buf0
           : BigAlloc(m.buffer +
                      LOOKAHEAD); // use calloc() to make Valgrind happy :)  or
                                  // can we just clear a few bytes after fread?
  if (!buf)
    return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;

  // MAIN COMPRESSION FUNCTION
  int result = tor_compress_chunk<MatchFinder, Coder>(
      m, callback, auxdata, (byte *)buf, bytes_to_compress);
...
int tor_compress4(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
                  void *buf, int bytes_to_compress) {
  switch (m.match_parser) {
  case GREEDY:
    return tor_compress0<MatchFinder, Coder>(m, callback, auxdata, buf,
                                             bytes_to_compress);
  case LAZY:
    return tor_compress0<LazyMatching<MatchFinder>, Coder>(
...
int tor_compress3(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
  ...
  switch (m.hash3) {
  case 0:
    return tor_compress4<MatchFinder, Coder>(m, callback, auxdata, buf,
                                             bytes_to_compress);
  case 1:
    return tor_compress4<Hash3<MatchFinder, 14, 10, FALSE>, Coder>(
        m, callback, auxdata, buf, bytes_to_compress);
  case 2:
    return tor_compress4<Hash3<MatchFinder, 16, 12, TRUE>, Coder>(
        m, callback, auxdata, buf, bytes_to_compress);
  default:
    return FREEARC_ERRCODE_INVALID_COMPRESSOR;
  }
...
template <class MatchFinder, class Coder>
int tor_compress3o(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
                   void *buf, int bytes_to_compress) {
  switch (m.hash3) {
  case 1:
    return tor_compress0_optimal<Hash3<MatchFinder, 14, 10, FALSE>, Coder>(
        m, callback, auxdata, buf, bytes_to_compress);
  case 2:
    return tor_compress0_optimal<Hash3<MatchFinder, 16, 12, TRUE>, Coder>(
        m, callback, auxdata, buf, bytes_to_compress);
  default:
    return FREEARC_ERRCODE_INVALID_COMPRESSOR;
  }
...
int tor_compress2o(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
                   void *buf, int bytes_to_compress) {
  switch (m.encoding_method) {
  case BYTECODER: // Byte-aligned encoding
    return tor_compress3o<MatchFinder, LZ77_ByteCoder>(m, callback, auxdata,
...
int tor_compress2(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
  ...
  case BYTECODER: // Byte-aligned encoding
    return tor_compress3<MatchFinder, LZ77_ByteCoder>(m, callback, auxdata, buf,
                                                      bytes_to_compress);
  case BITCODER: // Bit-precise encoding
...
int tor_compress(PackMethod m, CALLBACK_FUNC *callback, void *auxdata,
                 void *buf, int bytes_to_compress) {
// When FULL_COMPILE is defined, we compile (5*4+8)*3*2 + 9*4*2 = 222 variants
// of compressor Otherwise, we compile only variants actually used by the
// predefined -0..-16 modes
#ifdef FULL_COMPILE
  if (m.match_parser == GREEDY || m.match_parser == LAZY) {
    ...
    case NON_CACHING_MF:
      ...
      case 1:
        return tor_compress2<MatchFinder1>(m, callback, auxdata, buf,
                                           bytes_to_compress);
      case 2:
...
#else
  // -1..-5(-6)
  if (m.encoding_method == BYTECODER && m.hash_row_width == 1 && m.hash3 == 0 &&
          m.match_finder == NON_CACHING_MF && m.match_parser == GREEDY ||
      m.encoding_method == STORING) {
    return tor_compress0<MatchFinder1, LZ77_ByteCoder>(m, callback, auxdata,
                                                       buf, bytes_to_compress);
  } else if (m.encoding_method == BITCODER && m.hash_row_width == 1 &&
             m.hash3 == 0 && m.match_finder == NON_CACHING_MF &&
             m.match_parser == GREEDY) {
    return tor_compress0<MatchFinder1, LZ77_BitCoder>(m, callback, auxdata, buf,
                                                      bytes_to_compress);
    ...
    // -7..-9
  } else if (m.hash3 == 2 && m.match_finder == CYCLED_MF5 &&
             m.match_parser == LAZY) {
    return tor_compress0<
        LazyMatching<
    ...
    // -11..-13 -c4
  } else if (m.hash3 == 1 && m.match_finder == CACHING_MF5 &&
...
#define WRITE_DATA_IF(condition)                                               \
  {                                                                            \
    if (condition) {                                                           \
      ...
      write_start =                                                            \
          output; /* next time we should start writing from this pos */        \
                                                                               \
      ...
      if (write_start >= write_end) {                                          \
        /* Set up next write chunk to HUGE_BUFFER_SIZE or until buffer end -   \
         * whatever is smaller */                                              \
        write_end = write_start +                                              \
                    mymin(outbuf + bufsize - write_start, HUGE_BUFFER_SIZE);   \
      }                                                                        \
    }                                                                          \
  }

template <class Decoder>
int tor_decompress0(CALLBACK_FUNC *callback, void *auxdata, int _bufsize,
                    int minlen) {
  // SET_JMP_POINT (FREEARC_ERRCODE_GENERAL);
  int errcode = FREEARC_OK; // Error code of last "write" call
  Decoder decoder(callback, auxdata,
                  _bufsize); // LZ77 decoder parses raw input bitstream and
                             // returns literals&matches
  if (decoder.error() != FREEARC_OK)
    return decoder.error();
  uint bufsize =
      tornado_decompressor_outbuf_size(_bufsize); // Size of output buffer
  BYTE *outbuf = (byte *)BigAlloc(
      bufsize + PAD_FOR_TABLES * 2); // Circular buffer for decompressed data
  if (!outbuf)
    return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;
  outbuf += PAD_FOR_TABLES; // We need at least PAD_FOR_TABLES bytes available
                            // before and after outbuf in order to simplify
                            // datatables undiffing
  BYTE *output = outbuf;    // Current position in decompressed data buffer
  BYTE *write_start =
      outbuf; // Data up to this point was already writen to outsream
  BYTE *write_end =
      outbuf + mymin(bufsize, HUGE_BUFFER_SIZE); // Flush buffer when output
                                                 // pointer reaches this point
  if (compress_all_at_once)
    write_end = outbuf + bufsize +
                1; // All data should be written after decompression finished
  uint64 offset =
      0; // Current outfile position corresponding to beginning of outbuf
  int offset_overflow = 0; // Flags that offset was overflowed so we can't use
                           // it for match checking
  ...
  for (;;) {
    ...
    if (decoder.is_literal()) {
      ...
      *output++ = c;
      WRITE_DATA_IF(
          output >=
          write_end); // Write next data chunk to outstream if required
      ...
      // Check for simple match (i.e. match not requiring any special handling,
      // >99% of matches fall into this category)
      if (output - outbuf >= dist && write_end - output > len) {
        ...
        if (dist > bufsize || len > 2 * _bufsize ||
            (output - outbuf + offset < dist && !offset_overflow)) {
          errcode = FREEARC_ERRCODE_BAD_COMPRESSED_DATA;
          goto finished;
        }
        // Slow match copying route for cases when output-dist points before
        // buffer beginning, or p may wrap at buffer end, or output pointer may
        // run over write point
        BYTE *p =
            output - outbuf >= dist ? output - dist : output - dist + bufsize;
        do {
          *output++ = *p++;
          if (p == outbuf + bufsize)
            p = outbuf;
        ...
        // Otherwise it's a special code used to represent info about diffed
        // data tables
      } else {
        len -= IMPOSSIBLE_LEN;
        if (len == 0 || dist * len > 2 * _bufsize) {
          errcode = FREEARC_ERRCODE_BAD_COMPRESSED_DATA;
          goto finished;
        }
        stat_only(printf("\n%d: Start %x, end %x, length %d      ", len,
                         int(output - outbuf + offset),
                         int(output - outbuf + offset + len * dist),
                         len * dist));
        // Add new table to list: len is row length of table and dist is number
        // of rows
...
int tor_decompress(CALLBACK_FUNC *callback, void *auxdata, void *buf,
                   int bytes_to_compress) {
  int errcode;
  // First 6 bytes of compressed data are encoding method, minimum match length
  // and buffer size
  BYTE header[2];
  READ(header, 2);
  {
    ...
    case BYTECODER:
      return tor_decompress0<LZ77_ByteDecoder>(callback, auxdata, bufsize,
                                               minlen);

    case BITCODER:
      return tor_decompress0<LZ77_BitDecoder>(callback, auxdata, bufsize,
                                              minlen);
