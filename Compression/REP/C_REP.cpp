...
int REP_METHOD::decompress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  if (!f)
    f = (FARPROC)rep_decompress;

  return ((int(__cdecl *)(MemSize, int, int, int, int, int, int, int,
                          CALLBACK_FUNC *, void *))f)(
      BlockSize, MinCompression, ChunkSize, MinMatchLen, Barrier, SmallestLen,
      HashSizeLog, Amplifier, callback, auxdata);
}
...
MemSize REP_METHOD::GetCompressionMem(void) {
  ...
  int HashSize = CalcHashSize(HashSizeLog, BlockSize, SmallestLen, MinMatchLen,
...
// параметры (функция, обратная к parse_REP)
void REP_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  ...
  sprintf(buf, "rep:%s%s%s%s%s%s%s%s", BlockSizeStr, MinCompressionStr,
          MinMatchLenStr, purify ? "" : ChunkSizeStr, BarrierStr,
          SmallestLenStr, HashSizeLogStr, AmplifierStr);
}
  ...
  if (strcmp(parameters[0], "rep") == 0) {
    ...
    while (*++parameters && !error) {
      ...
      case 'l':
        ...
        continue;
      case 'c':
        p->ChunkSize = parseInt(param + 1, &error);
        continue;
      case 'd':
      ...
      case 'h':
        ...
        continue;
      case 'a':
        p->Amplifier = parseInt(param + 1, &error);
        continue;
      }
...
static int REP_x =
    AddCompressionMethod(parse_REP); // Зарегистрируем парсер метода REP
