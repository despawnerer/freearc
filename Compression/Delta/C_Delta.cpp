...
// параметры (функция, обратная к parse_DELTA)
void DELTA_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  DELTA_METHOD defaults;
  char BlockSizeStr[100] = ":";
  showMem(BlockSize, BlockSizeStr + 1);
  sprintf(buf, "delta%s%s", BlockSize != defaults.BlockSize ? BlockSizeStr : "",
          ExtendedTables ? ":x" : "");
}
...
COMPRESSION_METHOD *parse_DELTA(char **parameters) {
  if (strcmp(parameters[0], "delta") == 0) {
    // Если название метода (нулевой параметр) - "delta", то разберём остальные
    // параметры
