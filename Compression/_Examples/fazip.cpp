...
To-do list:
    fazip 0.4:
        ...
        save crc/vmac at fileend (read 8mb until random secret number in the
first N bytes) algorithm descriptions check icl11/x64 version (lzma doesn't
work) fazip 0.5: require 64-bit fix: ppmd grzip tta rep:3g doesn't work fazip
0.x: 4x4:external - doesn't work, probably due to problem with synchronization
when simultaneously starting multiple programs ht4/bt4 - full prefetch, check
cpu? 64-bit MemSize (lzma:h4g) steal cmdline processing from bzip2 or tornado
        external: pause arc's progress indicator when running
non-stdin-to-stdout external compressor external - не создавать tempdir при
stdin+stdout? linux:external - support waiting for b/g processes (RunCommand
"...&") WRITE4 in C_DisPack -> FWRITE4/FWRITE/FFLUSH PROGRESS/FREAD in
rep/delta/C_DisPack/...
 */
...
static enum {
  INDICATOR_NONE,
  INDICATOR_ONLY_ERROR,
  INDICATOR_ONLY_LAST,
  INDICATOR_FULL
} indicator =
    INDICATOR_FULL; // Print nothing, only error messages, errors+final stats,
                    // and errors+live stats, appropriately

static bool ENCODE; // Is it compression operation?
static Mutex IO; // Protects I/O operations: fwrite(outfile), remove(outname),
                 // fprintf(stderr) and Taskbar_*()

static const char *infile_name,
    *outfile_name; // Input/output file names or STDINOUT_NAME for stdin/stdout
static FILE *infile, *outfile; // Input/output files
static int64 filesize;         // Size of input file
static int64 insize, outsize,
    last_outsize; // Input/output bytes processed so far
static int64 progress_insize,
    progress_outsize; // Bytes reported so far via the "progress" callback
static bool indicator_started =
    false;                // Progress indicator line was printed at least once
...
const double TimeInterval =
    0.2; // Interval in seconds between statistics updates
static double GlobalTime0,
    LastGlobalTime; // Moment when (de)compression operation was started
...
static bool nocrc =
    false;            // Don't store/check checksum (primarily for benchmarking)
...
static void print_stats(bool final = false) {
  ...
  if (final
          ? (indicator < INDICATOR_ONLY_LAST)
          : (indicator < INDICATOR_FULL) || (_outsize - last_outsize < 64 * kb))
  ...
  if (final || GlobalTime - LastGlobalTime > TimeInterval) {
    ...
    int64 origsize =
        ENCODE ? _insize : _outsize; // Size of original (uncompressed) data
    int64 compsize = ENCODE ? _outsize : _insize; // Size of compressed data
    char progress[10];
    sprintf(progress, (filesize <= 0 || filesize < _insize ? "" : "%d%%: "),
            int(double(_insize) * 100 / filesize));
    fprintf(display,
            "\r%s%s -> %s: %.2lf%%  Cpu %.3lf mb/s (%.3lf sec), real %.3lf "
            "mb/s (%.3lf sec) = %.0lf%%%s",
            progress, show3(_insize, temp1), show3(_outsize, temp2),
            double(compsize) * 100 / origsize, origsize / CPUTime / mb, CPUTime,
            origsize / GlobalTime / mb, GlobalTime, CPUTime / GlobalTime * 100,
            newline);
...
void vmac_update(const void *buf, size_t size) {
  if (size)
    vmac_result[0] =
        vhash((unsigned char *)buf, size, &vmac_result[1], &vmac_state);
}
...
unsigned char rnd[16 + VMAC_KEY_LEN_BYTES] = {
    0x5B, 0x2E, 0xCB, 0xD8, 0x2B, 0xFE, 0x71, 0x76,
    0x29, 0x89, 0x07, 0x3C, 0x6C, 0x37, 0x21, 0x3D};
...
static int read_header(char **method) {
  int errcode = FREEARC_ERRCODE_BAD_HEADERS;
  void *auxdata = NULL;
  int len;
  ...
  if (buffer.get32() != BULAT_ZIGANSHIN_SIGNATURE)
    goto finished;
  if (buffer.get32() != FAZIP_SIGNATURE)
    goto finished;
  if (buffer.get8() != FAZIP_FORMAT_VERSION1)
    goto finished;

  if (buffer.get8() != TAG_COMPRESSOR)
    goto finished;
  *method = strdup((char *)buffer.p);
  buffer.p += strlen((char *)buffer.p) + 1; // read ASCIIZ string

  if (buffer.get8() != TAG_COMPRESSED_DATA)
    goto finished;
  checksum_type = buffer.get8();
  if (checksum_type != CHECKSUM_TYPE_NONEE &&
...
static char *error_msg(int errcode) {
  ...
  case FREEARC_ERRCODE_INVALID_COMPRESSOR:
    return "invalid compression method or parameters";
  case FREEARC_ERRCODE_ONLY_DECOMPRESS:
    return "program was compiled with FREEARC_DECOMPRESS_ONLY, so don't try to "
           "use compress()";
  case FREEARC_ERRCODE_OUTBLOCK_TOO_SMALL:
    return "output block size in (de)compressMem is not enough for all output "
           "data";
  case FREEARC_ERRCODE_NOT_ENOUGH_MEMORY:
    return "can't allocate memory required for (de)compression";
  case FREEARC_ERRCODE_READ:
    return "error when reading data";
  case FREEARC_ERRCODE_BAD_COMPRESSED_DATA:
    return "data can't be decompressed";
  case FREEARC_ERRCODE_NOT_IMPLEMENTED:
    return "requested feature isn't supported";
  case FREEARC_ERRCODE_NO_MORE_DATA_REQUIRED:
    return "required part of data was already decompressed";
  case FREEARC_ERRCODE_OPERATION_TERMINATED:
    return "operation terminated by user";
  case FREEARC_ERRCODE_WRITE:
    return "error when writing data";
  case FREEARC_ERRCODE_BAD_CRC:
    return "file failed CRC check";
  case FREEARC_ERRCODE_BAD_PASSWORD:
    return "password/keyfile failed checkcode test";
  case FREEARC_ERRCODE_BAD_HEADERS:
    return "archive headers are corrupted";
  case FREEARC_ERRCODE_INTERNAL:
    return "it should never happen: implementation error. Please report this "
           "bug to developers!";
  default:
    return "some error when (de)compressing";
  }
}
...
static int process(char *method) {
  int result;
  bool use_header;
  if (start_with(method, "compress:"))
    ENCODE = true, use_header = false, method = start_with(method, "compress:");
  else if (start_with(method, "decompress:"))
    ENCODE = false, use_header = false,
    method = start_with(method, "decompress:");
  else if (strcasecmp(method, "d") != EQUAL)
    ENCODE = true, use_header = true;
  else
    ENCODE = false, use_header = true;
...
int main(int argc, char *argv[]) {
  ...
  while (argv[1]) {
    if (strequ(argv[1], "-i0")) {
      ...
      display = fopen(WINDOWS_ONLY("nul") UNIX_ONLY("/dev/null"), "wb");
    } else if (strequ(argv[1], "-i1"))
      indicator = INDICATOR_ONLY_ERROR;
    else if (strequ(argv[1], "-i2"))
      indicator = INDICATOR_ONLY_LAST;
    else if (strequ(argv[1], "-i3"))
      indicator = INDICATOR_FULL;
    else if (strequ(argv[1], "-delete"))
      delete_input_file = true;
    else if (strequ(argv[1], "-nocrc"))
      checksum_type = CHECKSUM_TYPE_NONEE, nocrc = true;
  ...
  if (argc == 2 || argc == 4) {
    ...
    infile = nameequ(infile_name, STDINOUT_NAME)
                 ? stdin
                 : file_open_read_binary(infile_name);
    if (infile == NULL) {
      fprintf(display, "ERROR: can't open input file %s\n", infile_name);
      return EXIT_FAILURE;
    }
    outfile = nameequ(outfile_name, STDINOUT_NAME)
                  ? stdout
                  : file_open_write_binary(outfile_name);
    if (outfile == NULL) {
      fprintf(display, "ERROR: can't create output file %s\n", outfile_name);
      return EXIT_FAILURE;
    }
    set_binary_mode(infile);
    ...
    printf("FAZip v0.31 (2013-12-15) - demonstration of FreeArc Unified "
           "Compression Library (http://freearc.org)\n"
           "\n"
           "Usage, compression: FAZip method <infile >outfile\n"
           "                 or FAZip method infile outfile\n"
           "                 or FAZip compress:method ... - for raw compressed "
           "data\n"
           "       decompression: FAZip d <infile >outfile\n"
           "                   or FAZip d infile outfile\n"
           "                   or FAZip decompress:method ... - for raw "
           "compressed data\n"
           "Options:\n"
           ...
           "\n"
           "Notes: all FreeArc compression methods are supported, for example "
           "rep:512m+delta+tempfile+4x4:lzma:8\n"
           "         as well as cls-*.dll and facompress*.dll\n"
           "       infile/outfile may be specified as \"-\" what means "
           "stdin/stdout\n");
