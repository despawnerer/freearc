...
public:
  ...
  MemSize BlockSize; // Размер блока, обрабатываемого за раз. Совпадения ищутся
  ...
  int MinCompression; // Минимальный процент сжатия. Если выходные данные
                      ...
                      // (несжатые) данные
  int MinMatchLen; // Минимальный размер совпадающей строки, который будет
                   // сжиматься
  int HashSizeLog; // Логарифм размера хеша (в 4-байтовых словах). Большие
                   // значения увеличивают сжатие, но значительно замедляют его
  ...
  LZP_METHOD();
  // Универсальный метод: даём положительный ответ на запросы "VeryFast?" для
  // хеша<=128кб
  virtual int doit(char *what, int param, void *data, CALLBACK_FUNC *callback) {
    if (strequ(what, "VeryFast?"))
      return HashSizeLog <= 15;
    else
  ...
  virtual MemSize GetCompressionMem(void) {
    return BlockSize * 2 + (1 << HashSizeLog) * sizeof(BYTE *);
  }
