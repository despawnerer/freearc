...
public:
  ...
  char option_strings[MAX_METHOD_STRLEN]; // Текстовый буфер для хранения текста
                                          // параметров
  char *defaultopt; // Значения параметров по умолчанию
  int solid;        // Разрешено делать солид-блоки?
  int useHeader; // TRUE, если в начало сжатого потока записывается 0/1 - данные
                 // несжаты/сжаты; FALSE - при подмене внутренних методов

  // Параметры, специфичные для PPMonstr
  int order; // Порядок модели (по скольким последним сивмолам предсказывается
             // следующий)
  int MRMethod; // Что делать, когда память, выделенная для хранения модели,
                // исчерпана
  ...
  virtual int decompress(CALLBACK_FUNC *callback, void *auxdata) {
    return DeCompress(DECOMPRESS, callback, auxdata);
  }
#ifndef FREEARC_DECOMPRESS_ONLY
  virtual int compress(CALLBACK_FUNC *callback, void *auxdata) {
    return DeCompress(COMPRESS, callback, auxdata);
  }
