...
#define MAXELEM                                                                \
  8 /* size of maximum data element that can be read/written to byte stream */
...
struct OutputByteStream {
  ...
  BYTE *anchor;      // Marked position in output buffer. Shifted synchronous to
                     // buffer contents
  UINT chunk; // How many bytes should be written at least in each "write" call
  uint64
      shifted_out; // Bytes shifted out of buffer after saved by write callback
  int errcode;
  ...
  OutputByteStream(CALLBACK_FUNC *_callback, void *_auxdata, UINT _chunk,
                   UINT pad) {
    callback = _callback;
    auxdata = _auxdata;
    chunk = _chunk;
    // Add 512 bytes for LZ77_ByteCoder (its LZSS scheme needs to overwrite old
    // flag words) and 4096 for rounding written chunks
...
void OutputByteStream::putbuf(void *ptr, uint len) {
  if ((output - buf) + len > chunk)
    flush();
  CHECK(FREEARC_ERRCODE_INTERNAL, (output - buf) + len <= chunk,
        (s, "Fatal error: putbuf %d in buffer of size %d", len, chunk));
...
void OutputByteStream::flush() {
  ...
  if (output - buf > 512) {
    UINT n =
        (output - buf - 512) &
        ~4095; // how much bytes we can flush now (don't flush last 512 bytes
               // due to possible LZSS use and round down to 4096-byte chunk)
...
struct InputByteStream {
  ...
  void *auxdata;
  UINT bufsize;
  byte *buf;   // Buffer (MAXELEM additional bytes are allocated for get64, see
               // below)
  ...
  InputByteStream(CALLBACK_FUNC *_callback, void *_auxdata, UINT _bufsize) {
    callback = _callback;
    auxdata = _auxdata;
    bufsize = compress_all_at_once
                  ? _bufsize + (_bufsize / 4)
                  : LARGE_BUFFER_SIZE; // For all-at-once compression input
                                       // buffer should be large enough to hold
                                       // all compressed data
    ...
    if (error() == FREEARC_OK)
      errcode = callback("read", buf + MAXELEM, bufsize, auxdata);
  }
  ...
  uint get16() {
    fill();
    uint n = value16(input);
    input += 2;
    return n;
  }
  uint32 get24() {
    fill();
    uint32 n = value24(input);
    input += 3;
    return n;
  }
  uint32 get32() {
    fill();
    uint32 n = value32(input);
    input += 4;
    return n;
  }
  uint64 get64() {
    fill();
    uint64 n = value64(input);
    input += 8;
    return n;
  }
};
...
struct OutputBitStream : OutputByteStream {
  ...
  // Write n lower bits of x
  void putlowerbits(int n, uint32 x) { putbits(n, mask(x, n)); }
};

// Skip a few bytes at the start and end of buffer in order to align it like it
// aligned on disk
#define eat_at_start 0
#define eat_at_end 0

OutputBitStream::OutputBitStream(CALLBACK_FUNC *callback, void *auxdata,
                                 UINT chunk, UINT pad)
...
InputBitStream::InputBitStream(CALLBACK_FUNC *callback, void *auxdata,
                               UINT bufsize)
...
#define FAST_BITS                                                              \
  11 /* symbols with shorter codes (<=FAST_BITS bits) are decoded much faster  \
      */
...
// Simplified Node structure, saving counter in higher bits and index in lower
// ones (in order to allow stable sorting by counter!)
typedef uint Node0;
...
int __cdecl by_cnt_and_index(const void *a, const void *b) {
  return *(const Node0 *)a - *(const Node0 *)b;
}
...
struct HuffmanTree {
  CodecDirection
      type; // Determines whether this tree used for encoding or decoding
  ...
  int fast_index[1 << FAST_BITS]; //   Direct decoding table for short codes
                                  //   (<=FAST_BITS)
...
void HuffmanTree::init(CodecDirection _type, int _n) {
  CHECK(
      FREEARC_ERRCODE_INTERNAL, _n <= MAXHUF,
      (s,
...
// INT_MAX values used as fence at end of each list. Don't tell me that i'm
// genie :)
void HuffmanTree::build_tree(int rescale_mode) {
  ...
  int *places =
      (int *)(buf + MAXHUF); // Part of the buffer reused for better caching
  int b = 0;                 // This var will contain count of non-zero symbols

  // Nodes with small counters (<CHUNKS) sorted by counting, remaining nodes
  // (<10%) sorted by STL sort()
  const int CHUNKS = 250;
  iterate_var(i, CHUNKS + 1) places[i] = 0;
  iterate_var(i, n) { // STATIC: add "if (counter[i])"
    if (counter[i] < CHUNKS)
      places[counter[i] + 1]++;
    b++;
  }
  iterate_var(i, CHUNKS) places[i + 1] += places[i];
  // code[] is reused here as a temporary buffer for better caching  (== Node0
  // rest[b - places[CHUNKS]])
  ...
  iterate_var(i, b + 4) buf[b + i].cnt =
      INT_MAX;    // maximum possible int value used here as a fence
  ...
  // of their parents with additional bit '0' for left child and '1' for right
  // one
  buf[p2].bits = 0;
  buf[p2].code = 0;
  ...
  if (type == Encoder) {
    ...
    // Use symbol with smallest frequency to determine maximum number of bits in
    // any code
    maxbits = buf[0].bits;
    ...
    iterate_var(i, b) {
      ...
      bits[s] = sbits;
      // For decoder we fill index[] table used to decode symbols using first
      // 'maxbits' of input (it is always possible because maxbits is a maximum
      // number of bits in any code) We also fill fast_index[] table used to
      // decode short codes
      if (sbits <= FAST_BITS) {
        iterate_var(j, 1 << (FAST_BITS - sbits))
            fast_index[scode + (j << sbits)] = s;
      } else {
...
#define rescale(FACTOR)                                                        \
  iterate_var(s, n) counter[s] -=                                              \
      (counter[s] > 1 && counter[s] < FACTOR) ? 1 : counter[s] / FACTOR
  switch (rescale_mode) {
  case 0:
    rescale(2);
    break;
...
template <int EOB> struct HuffmanEncoder : OutputBitStream {
  HuffmanTree huf; // Huffman tree used to encode symbols
  int remainder;   // Count symbols remaining before huffman tree rebuild

  HuffmanEncoder(CALLBACK_FUNC *callback, void *auxdata, UINT chunk, UINT pad,
                 int n)
      : OutputBitStream(callback, auxdata, chunk, pad), huf(Encoder, n) {
...
template <int EOB> struct HuffmanDecoder : InputBitStream {
...
struct HuffmanEncoderOrder1 : OutputBitStream {
  ...
  int remainder[ORDER1_CONTEXTS]; // Count symbols remaining before huffman tree
                                  // rebuild

  HuffmanEncoderOrder1(CALLBACK_FUNC *callback, void *auxdata, UINT chunk,
                       UINT pad, int n)
      : OutputBitStream(callback, auxdata, chunk, pad) {
    context = 0;
    huf = new HuffmanTree[ORDER1_CONTEXTS];
    for (int i = 0; i < ORDER1_CONTEXTS; i++)
      remainder[i] = HUFBLOCKSIZE / 4, huf[i].init(Encoder, n);
  }

  // In order to unify order-0 and order-1 coder APIs, we provide API to specify
  // context in separate call
  int context;
...
struct HuffmanDecoderOrder1 : InputBitStream {
  ...
  HuffmanDecoderOrder1(CALLBACK_FUNC *callback, void *auxdata, UINT bufsize,
                       int n)
      : InputBitStream(callback, auxdata, bufsize) {
    iterate(ORDER1_CONTEXTS, huf[i].init(Decoder, n));
  }
...
public:
  TRangeCoder(CALLBACK_FUNC *callback, void *auxdata, UINT chunk, UINT pad)
      : OutputByteStream(callback, auxdata, chunk, pad), low(0),
        range(0xffffffff), buffer(0), help(0) {}
...
public:
  TRangeDecoder(CALLBACK_FUNC *callback, void *auxdata, UINT bufsize)
      : InputByteStream(callback, auxdata, bufsize), range(0xffffffff) {
    for (int i = 0; i < 5; i++)
...
#define INDEXES                                                                \
  2048 /* amount of indexes used for fast searching in cum[] table */
#define RANGE                                                                  \
  (1u << RANGE_BITS) /* automagically, on each recalculation there are just    \
                        RANGE symbols counted in livecnt[] */
#define RANGE_BITS                                                                                                  \
  14 /* the higher this value, тем реже происходят обновления статистики, но \
        она при этом точнее */

template <CodecDirection type> struct TCounter {
  UINT n, remainder;
  ...
  // and recalculate encoding tables if enough symbols was counted since last
  // recalculation so that sum(livecnt[])==RANGE now
  void Inc(unsigned s) {
    livecnt[s]++;
    if (--remainder == 0)
      Rescale();
...
template <CodecDirection type> TCounter<type>::TCounter(unsigned _n) {
  n = _n;
...
template <CodecDirection type> void TCounter<type>::Rescale() {
  UINT total = 0;
  remainder = RANGE;
  for (int s = 0, ind = 0; s < n; s++) {
    cnt[s] = livecnt[s], cum[s] = total, total += cnt[s],
...
template <int EOB> struct ArithCoder : TRangeCoder {
  TCounter<Encoder> c; // Provides stats about symbol frequencies

  ArithCoder(CALLBACK_FUNC *callback, void *auxdata, UINT chunk, UINT pad,
             int n)
  ...
  PRICE price(UINT x, int extra_bits) {
    return c.prices[x] + PRICE_BITS(extra_bits);
  }
};

template <int EOB> struct ArithDecoder : TRangeDecoder {
  TCounter<Decoder> c; // Provides stats about symbol frequencies

  ArithDecoder(CALLBACK_FUNC *callback, void *auxdata, UINT bufsize,
               int elements)
