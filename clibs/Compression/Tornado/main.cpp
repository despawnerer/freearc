...
static const char *codec_name[] = {"storing", "bytecoder", "bitcoder",
                                   "hufcoder", "aricoder"};
static const char *parser_name[] = {"", "greedy", "lazy", "flexible",
                                    "optimal"};

// Returns human-readable method description
static char *name(PackMethod method) {
  static char namebuf[200], h[100], b[100], bt[100], bth[100],
      auxhash_size[100], u[100], ah[100], fb[100];
  const char *hashname[] = {
      "hash4",   "???",    "???",    "???",    "cchash4", "cchash5", "cchash6",
      "cchash7", "???",    "???",    "???",    "???",     "???",     "???",
      "chash4",  "chash5", "chash6", "chash7", "???",     "???",     "???",
      "???",     "???",    "???",    "bt4",    "bt5",     "bt6",     "bt7"};
  ...
  int h3 = method.hash3;
  int hb =
      (x == NON_CACHING_MF ? 4 : x % 10); // MINLEN for the primary match finder
  ...
  sprintf(u,
          !(CYCLED_MF4 <= x && x <= CYCLED_MF7) && method.update_step != 999 &&
                  p != OPTIMAL
              ? "/u%d"
              : "",
          method.update_step); // Cycled MF ignore `step` in the update_hash()
  sprintf(ah, method.auxhash_size && hb > 4 ? " + %s:%d %s" : "", auxhash_size,
          method.auxhash_row_width,
          hb > 5 ? (p == OPTIMAL ? "chash4" : "cchash4") : "exhash4");
  sprintf(namebuf,
          c == STORING ? codec_name[c]
                       : "%s parser%s, %s%s:%d%s %s%s%s, buffer %s, %s%s",
          parser_name[p], fb, bth, h, l, u, hashname[x], ah,
          h3 == 2 ? " + 256kb hash3 + 16kb hash2"
          : h3    ? " + 64kb hash3 + 4kb hash2"
                  : "",
          b, codec_name[c], method.find_tables ? "" : " w/o tables");
  return namebuf;
}
...
struct Results {
  ...
  FILESIZE progress_insize,
      progress_outsize; // Bytes reported so far via the "progress" callback
  FILESIZE last_insize, last_outsize; // Variables protecting the progress
                                      // indicator from too often evaluation
  double start_time;                  // When (de)compression was started
  double lasttime, lasttime2;         // Last time when we've updated progress
                                      // indicator/console title
...
// Return current time (cpu time used or wall clock time). Return 0 if we
// disabled timing at compile time
...
static void start_print_stats(Results &r) {
  r.insize = r.outsize = r.progress_insize = r.progress_outsize =
      r.last_insize = r.last_outsize = 0;
...
static void print_stats(Results &r) {
  ...
  FILESIZE insize = r.progress_insize;
  bool not_enough_input = (insize - r.last_insize < PROGRESS_CHUNK_SIZE);
  FILESIZE outsize = r.progress_outsize;
  bool not_enough_output = (outsize - r.last_outsize < PROGRESS_CHUNK_SIZE);
  if (r.quiet_progress || insize == 0 || outsize == 0 ||
      not_enough_input && not_enough_output)
    return; // Prints stats no more often than once per 64 kb
  ...
  if (curtime > r.lasttime + 0.2) {
    ...
    char percents0[100] = "", remains0[100] = "", percents[1000] = "",
         remains[100] = "", insize_str[100], outsize_str[100];
    if (r.filesize) {
      // If progress by 1% takes more than 1-2 seconds - display xx.x%,
      // otherwise xx% (we don't want to switch it too frequently, therefore
      // "?1:2")
      r.show_exact_percent = double(r.filesize) / insize * time >
                             (r.show_exact_percent ? 1 : 2) * 100;
      if (r.show_exact_percent)
        sprintf(percents0, "%.1lf%%",
                double(int(double(insize) * 100 / r.filesize * 10)) / 10);
      else
        sprintf(percents0, "%d%%", int(double(insize) * 100 / r.filesize));
      sprintf(percents, "%s: ", percents0);

      int remain = int(double(r.filesize - insize) / insize * time) + 1;
      if (remain >= 3600)
        sprintf(remains0, "%02d:%02d:%02d", remain / 3600, (remain % 3600) / 60,
                remain % 60);
      else
    ...
    double origsize =
        (r.mode == _COMPRESS ? insize
                             : outsize); // Size of original (uncompressed) data
    double compsize =
        (r.mode == _COMPRESS ? outsize : insize); // Size of compressed data
    double ratio = (compsize / origsize) * 100;
    double speed =
        (origsize / mb) / mymax(time, 0.001); // Speed of processing, in MiB/s
    if (!r.quiet_result && !strequ(r.outname, "-"))
      fprintf(stderr, "\r%s%s%s -> %s: %.2lf%%, speed %.3lf mb/sec%s   ",
              r.method_name, percents, show3(insize, insize_str),
              show3(outsize, outsize_str), ratio, speed, remains);

    if (!r.quiet_title && r.filesize &&
        curtime > r.lasttime2 + 0.5) // Update window title every 0.5 seconds
    {
      const char *op = (r.mode == _COMPRESS     ? "Compressing"
                        : r.mode == _DECOMPRESS ? "Extracting"
                                                : "Benchmarking");
      sprintf(percents, "%s %s | %s %s", percents0, remains0, op,
              strequ(r.filename, "-") ? PROGRAM_NAME : r.filename);
...
static void print_final_stats(Results &r) {
  if (!r.quiet_result && r.insize && r.outsize) {
    double origsize = (r.mode == _COMPRESS
                           ? r.insize
                           : r.outsize); // Size of original (uncompressed) data
    double compsize =
        (r.mode == _COMPRESS ? r.outsize : r.insize); // Size of compressed data
    double ratio = (compsize / origsize) * 100;
    char insize_str[100], outsize_str[100];
    fprintf(stderr, "\r%s%s %s -> %s: %.2lf%%", r.method_name,
            r.mode == _COMPRESS ? "compressed" : "Unpacked",
            show3(r.insize, insize_str), show3(r.outsize, outsize_str), ratio);
#ifndef FREEARC_NO_TIMING
    double time =
        GetSomeTime() - r.start_time; // Time spent for (de)compression
    if (time > 0.001) {
...
static int64 check_int(int64 x, int error, int64 minVal, int64 maxVal,
                       char *option) {
  if (error) {
...
static int64 check_parse_int(char *param, int64 minVal, int64 maxVal,
                             char *option) {
  int error = 0;
  int x = parseInt(param, &error);
  return check_int(x, error, minVal, maxVal, option);
}

static int64 check_parse_mem(char *param, int64 minVal, int64 maxVal,
                             char *option) {
  int error = 0;
  int64 x = parseMem64(param, &error);
  return check_int(x, error, minVal, maxVal, option);
}
...
int main(int argc, char **argv) {
  ...
  static Results r;
  r.use_cpu_time = r.quiet_title = r.quiet_header = r.quiet_progress =
      r.quiet_result = FALSE;
  ...
  for (char **argv_ptr = argv; *++argv_ptr != NULL;) {
    ...
    if (*param == '-') {
      ...
      if (strcasecmp(param, "-") == 0)
        break; // "--" is a "stop processing" option
      else if (isdigit(*param))
        r.method = std_Tornado_method[check_parse_int(
            param, 1, elements(std_Tornado_method) - 1, param - 1)];
    }
  }
  ...
  for (char **argv_ptr = argv; *++argv_ptr != NULL;) {
    ...
    if (param[0] != '-' || param[1] == '\0') {
      ...
      if (strcasecmp(param, "-") == 0)
        ...
        continue;
      else if (strcasecmp(param, "z") == 0)
        global_mode = _COMPRESS;
      else if (strcasecmp(param, "d") == 0)
        global_mode = _DECOMPRESS;
      else if (strcasecmp(param, "delete") == 0)
        delete_input_files = TRUE;
      else if (strcasecmp(param, "t") == 0)
        output_filename = "";
      else if (strcasecmp(param, "q") == 0)
        r.quiet_title = r.quiet_header = r.quiet_progress = r.quiet_result =
            TRUE;
#ifndef FREEARC_NO_TIMING
      else if (strcasecmp(param, "cpu") == 0)
        r.use_cpu_time = TRUE;
#endif
      else if (strcasecmp(param, "h") == 0)
        global_mode = HELP;
      else if (strcasecmp(param, "b") == 0)
        r.method.buffer = MAX_BUFFER_SIZE; // set buffer size to the maximum
                                           // supported by LZ coders
      else if (strcasecmp(param, "x") == 0)
        r.method.match_finder = CACHING_MF4;
      else if (strcasecmp(param, "xx") == 0)
        r.method.match_finder = CYCLED_MF4;
      else if (strcasecmp(param, "x+") == 0)
        r.method.match_finder = CACHING_MF4;
      else if (strcasecmp(param, "x-") == 0)
        r.method.match_finder = NON_CACHING_MF;
      else if (strcasecmp(param, "t+") == 0)
        r.method.find_tables = TRUE;
      else if (strcasecmp(param, "t-") == 0)
        r.method.find_tables = FALSE;
      else if (strcasecmp(param, "t1") == 0)
        r.method.find_tables = TRUE;
      else if (strcasecmp(param, "t0") == 0)
        r.method.find_tables = FALSE;
      else if (strcasecmp(param, "s") == 0)
        r.method.hash3 = 1;
      else if (strcasecmp(param, "ss") == 0)
        r.method.hash3 = 2;
      else if (strcasecmp(param, "s+") == 0)
        r.method.hash3 = 1;
      else if (strcasecmp(param, "s-") == 0)
        r.method.hash3 = 0;
      else if (start_with(param, "fb"))
        r.method.fast_bytes = check_parse_int(param + 2, MIN_FAST_BYTES,
                                              MAX_FAST_BYTES, *argv_ptr);
      else if (start_with(param, "rem")) /* ignore option */
        ;
      else if (isdigit(*param))
        ; // -1..-16 option is already processed :)
      else
        switch (tolower(*param++)) {
        case 'b':
          r.method.buffer = check_parse_mem(param, MIN_BUFFER_SIZE,
                                            MAX_BUFFER_SIZE, *argv_ptr);
          break;
        case 'h':
          r.method.hashsize =
              check_parse_mem(param, MIN_HASH_SIZE, MAX_HASH_SIZE, *argv_ptr);
          break;
        case 'l':
          r.method.hash_row_width =
              check_parse_int(param, 1, MAX_HASH_ROW_WIDTH, *argv_ptr);
          break;
        case 'u':
          r.method.update_step =
              check_parse_int(param, 1, MAX_UPDATE_STEP, *argv_ptr);
          break;
        case 'c':
          r.method.encoding_method =
              check_parse_int(param, BYTECODER, ARICODER, *argv_ptr);
          break;
        case 's':
          r.method.hash3 = check_parse_int(param, 0, 2, *argv_ptr);
          break;
        case 'p':
          r.method.match_parser = parseInt(param, &error);
          break;
        case 'x':
          r.method.match_finder = parseInt(param, &error);
          break;
        case 'o':
    ...
    check_for_errors:
      ...
      if (!(r.method.match_parser == GREEDY || r.method.match_parser == LAZY ||
            r.method.match_parser == OPTIMAL)) {
        fprintf(stderr, "ERROR: bad option value: '%s'\n", *argv_ptr);
        exit(FREEARC_EXIT_ERROR);
      }
      int mf = r.method.match_finder;
      r.method.match_finder = mf =
          (mf == CACHING_MF4_DUB ? CACHING_MF4
                                 : (mf == CYCLED_MF4_DUB ? CYCLED_MF4 : mf));
      if (!(mf == NON_CACHING_MF || (CYCLED_MF4 <= mf && mf <= CYCLED_MF7) ||
            (CACHING_MF4 <= mf && mf <= CACHING_MF7) ||
            (BT_MF4 <= mf && mf <= BT_MF7))) {
  ...
  if (r.method.match_finder % 10 >= 5 &&
      (r.method.auxhash_size == 0 || r.method.auxhash_row_width == 0)) {
    char ah[100];
    showMem64(r.method.auxhash_size, ah);
    fprintf(stderr,
            "ERROR: empty auxhash (-ah%s -al%d) while MINLEN=%d (-x%d)\n", ah,
            r.method.auxhash_row_width, r.method.match_finder % 10,
  ...
  if (global_mode != HELP && fcount == 0 &&
    ...
    char h[100], ah[100], b[100], MinHashSizeStr[100], MaxHashSizeStr[100],
        MinBufSizeStr[100], MaxBufSizeStr[100];
    ...
    printf("Tornado compressor v0.6  (c) Bulat.Ziganshin@gmail.com  "
           "http://freearc.org  2014-03-08\n"
           "\n"
...
#else
        "   -q      -- be quiet; -q[thpr]* disables "
        "title/header/progress/results individually\n"
    ...
    printf(
        "   -c#     -- coder (1-bytes, 2-bits, 3-huf, 4-arith), default %d\n",
        r.method.encoding_method);
    printf("   -p#     -- parser (1-greedy, 2-lazy, 4-optimal), default %d\n",
           r.method.match_parser);
    printf("   -x#     -- match finder (0: non-caching ht4, 4-7: cycling "
           "cached ht4..ht7,\n"
           "                            14-17: shifting cached ht4..ht7, "
           "24-27: bt4..bt7), default %d\n",
           r.method.match_finder);
    printf(
        "   -s#     -- 2/3-byte hash (0-disabled, 1-fast, 2-max), default %d\n",
        r.method.hash3);
    printf("   -t#     -- table diffing (0-disabled, 1-enabled), default %d\n",
           r.method.find_tables);
    printf(
        "   -fb#    -- fast bytes in the optimal parser (%d..%d), default %d\n",
  ...
  for (char **parameters = argv; *++parameters != NULL;) {
    // If options are still parsed and this argument starts with "-" - it's an
    // option
    if (parse_options && parameters[0][0] == '-' && parameters[0][1]) {
      if (strequ(*parameters, "--"))
        parse_options = FALSE;
      continue;
    }
    ...
    if (r.mode == BENCHMARK ||
        output_filename &&
            strequ(output_filename, "")) { // Redirect output to nul
      strcpy(r.outname, "");
    } else if (output_filename) {
      if (strequ(output_filename, "-"))
        strcpy(r.outname, output_filename);
      else if (is_path_char(last_char(output_filename))) {
        sprintf(r.outname, "%s%s", output_filename, drop_dirname(r.filename));
        goto add_remove_ext;
      } else if (dir_exists(output_filename)) {
        sprintf(r.outname, "%s%c%s", output_filename, PATH_DELIMITER,
                drop_dirname(r.filename));
        goto add_remove_ext;
      } else
        strcpy(r.outname, output_filename);
      ...
      //    on decompression - remove COMPRESS_EXT (and add DECOMPRESS_EXT if
      //    file already exists)
      strcpy(r.outname, r.filename);
    add_remove_ext: // Remove COMPRESS_EXT on the end of name or add
                    // DECOMPRESS_EXT (unless we are in _COMPRESS mode)
    ...
    if (*r.outname) {
      r.fout =
          strequ(r.outname, "-") ? stdout : file_open_write_binary(r.outname);
      if (r.fout == NULL) {
    ...
    case _DECOMPRESS: {
      // if (!r.quiet_header && !strequ (r.outname, "-"))   fprintf (stderr,
    ...
    if (result == FREEARC_OK) {
      if (delete_input_files && !strequ(r.filename, "-"))
        delete_file(r.filename);
    } else {
      if (!strequ(r.outname, "-") && !strequ(r.outname, ""))
        delete_file(r.outname);
      switch (result) {
      case FREEARC_ERRCODE_INVALID_COMPRESSOR:
#ifdef FULL_COMPILE
        fprintf(
            stderr,
            "\nThis compression mode isn't supported even by the full Tornado "
            "version, you need to modify sources in order to enable it!");
#else
        fprintf(stderr, "\nThis compression mode isn't supported by the small "
                        "Tornado version, use the full version instead!");
#endif
        break;
