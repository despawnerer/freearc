...
// If this value will be smaller than real, we can hash bytes in buf that are
// not yet read Also it's number of bytes reserved after bufend in order to
// simplify p+N<=bufend checks
#define MAX_HASHED_BYTES 12

#ifdef DEBUG
void check_match(BYTE *p, BYTE *q, int len) {
  if (p == q || memcmp(p, q, len))
    printf("Bad match:  ");
}
...
static inline uint hashx(int len, BYTE *p, UINT HashShift, UINT HashMask = ~0) {
  switch (len) {
  case 4:
    return ((value32(p) * 123456791) >> HashShift) & HashMask;
  case 5:
    return ((value32(p) * 123456791 + value32(p + 1) * 789567123) >>
            HashShift) &
           HashMask;
  case 6:
    return ((value32(p) * 123456791 + value32(p + 2) * 789567123) >>
            HashShift) &
           HashMask;
  case 7:
    return ((value32(p) * 123456791 + value32(p + 3) * 789567123) >>
            HashShift) &
           HashMask;
  default:
    CHECK(FREEARC_ERRCODE_INTERNAL, TRUE,
          (s, "Fatal error: Unsupported len==%d in the hashx()", len));
    return 0;
  }
}

// Check match len/distance for greedy/lazy parsing (distances for small matches
// need to be limited)
...
// Check match at q:len for greedy/lazy parsing (distances for small matches
// need to be limited)
static inline int accept_match(int len, BYTE *p, BYTE *q, void *bufend) {
  switch (len) {
  case 4:
    return p - q < 48 * kb && p <= bufend && val32equ(p, q) ? 4 : 0;
  case 5:
    return p - q < 192 * kb && p <= bufend && val32equ(p, q) && p[4] == q[4]
               ? 5
               : 0;
  case 6:
    return p - q < 1 * mb && p <= bufend && val32equ(p, q) &&
                   val16equ(p + 4, q + 4)
               ? 6
               : 0;
  case 7:
    return p - q < 12 * mb && p <= bufend && val32equ(p, q) &&
                   val24equ(p + 4, q + 4)
               ? 7
               : 0;
  case 8:
    return p <= bufend && val32equ(p, q) && val32equ(p + 4, q + 4) ? 8 : 0;
  case 9:
    return p <= bufend && val32equ(p, q) && val32equ(p + 4, q + 4) &&
                   p[8] == q[8]
               ? 9
               : 0;
  case 10:
  ...
  default:
    CHECK(FREEARC_ERRCODE_INTERNAL, TRUE,
          (s, "Fatal error: Unsupported len==%d in the accept_match()", len));
    return 0;
  }
}

// Check match at q:len for optimal parsing
static inline bool accept_optimal_match(int len, BYTE *p, BYTE *q,
                                        void *bufend) {
  switch (len) {
  case 4:
    return p <= bufend && val32equ(p, q);
  case 5:
    return p <= bufend && val32equ(p, q) && p[4] == q[4];
  case 6:
    return p <= bufend && val32equ(p, q) && val16equ(p + 4, q + 4);
  case 7:
    return p <= bufend && val32equ(p, q) && val24equ(p + 4, q + 4);
  case 8:
    return p <= bufend && val32equ(p, q) && val32equ(p + 4, q + 4);
  case 9:
    return p <= bufend && val32equ(p, q) && val32equ(p + 4, q + 4) &&
  ...
  default:
    CHECK(FREEARC_ERRCODE_INTERNAL, TRUE,
          (s, "Fatal error: Unsupported len==%d in the accept_optimal_match()",
           len));
    return FALSE;
  }
}
...
UINT check_match_len(BYTE *p, BYTE *q, UINT maxLen) {
  UINT len = 0;
  while (val32equ(p + len, q + len)) {
    len += 4;
    if (len >= maxLen)
      return maxLen;
  }
  while (p[len] == q[len] && len < maxLen)
    len++;
  return len;
}

// ****************************************************************************
// Abstract Match Finder class which defines codebase common for all concrete
// Match Finders
// ****************************************************************************
...
struct BaseMatchFinder {
  ...
  BaseMatchFinder(BYTE *buf, uint dictsize, uint hashsize, int _hash_row_width,
                  uint auxhash_size, int auxhash_row_width);
  // Returns error code if there is any problem in MF work
  int error();
  // Called on initialization and when next data chunk read into NON-SLIDING
  // buffer
  void clear_hash(BYTE *buf);
  // Called after data was shifted 'shift' bytes backwards in SLIDING WINDOW
  // buf[]
  ...
  // Returns pointer of last match found (length is returned by find_matchlen()
  // itself)
...
BaseMatchFinder::BaseMatchFinder(BYTE *buf, uint dictsize, uint hashsize,
                                 int _hash_row_width, uint, int) {
  base = buf;
...
void BaseMatchFinder::clear_hash(BYTE *buf) {
  if (HTable)
    iterate_var(i, HashSize) HTable[i] = fromPtr(buf + 1);
}

// Called after data was shifted 'shift' bytes backwards in SLIDING WINDOW buf[]
// We should make appropriate corrections in HTable. We set HTable entries
// minimum to buf+1 because with lazy matching we can extend match found one
// byte backward
void BaseMatchFinder::shift(BYTE *buf, int shift) {
  iterate_var(i, HashSize) HTable[i] =
      HTable[i] > fromPtr(buf + shift) ? HTable[i] - shift : fromPtr(buf + 1);
}
...
struct MatchFinder1 : BaseMatchFinder {
  MatchFinder1(BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
               uint auxhash_size, int auxhash_row_width)
      : BaseMatchFinder(buf, dictsize, hashsize, hash_row_width, auxhash_size,
                        auxhash_row_width) {
    clear_hash(buf);
  }
  ...
  uint find_matchlen(byte *p, void *bufend, UINT prevlen) {
    ...
    if (val32equ(p, q)) {
      UINT len;
      for (len = MINLEN - 1; p + len + 4 < bufend && val32equ(p + len, q + len);
           len += 4)
        ;
...
MatchFinder2(BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
             uint auxhash_size, int auxhash_row_width)
    : BaseMatchFinder(buf, dictsize, hashsize, hash_row_width, auxhash_size,
                      auxhash_row_width) {
  clear_hash(buf);
...
void update_hash(BYTE *p, UINT len,
                 UINT step) { // len may be as low as 1 if LazyMatching and
                              // Hash3 are used together
  UINT h;
...
template <uint N> struct MatchFinderN : BaseMatchFinder {
MatchFinderN(BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
             uint auxhash_size, int auxhash_row_width)
    : BaseMatchFinder(buf, dictsize, hashsize, hash_row_width, auxhash_size,
                      auxhash_row_width) {
  clear_hash(buf);
...
uint find_matchlen(byte *p, void *bufend, UINT prevlen) {
  ...
  // Start with checking the first element of the hash row
  if (val32equ(p, q)) {
    for (len = MINLEN; p + len + 4 <= bufend && val32equ(p + len, q + len);
         len += 4)
      ;
    for (; p + len < bufend && p[len] == q[len]; len++)
      ;
...
template <uint N> struct ExactMatchFinder : BaseMatchFinder {
ExactMatchFinder(BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
                 uint auxhash_size, int auxhash_row_width)
    : BaseMatchFinder(buf, dictsize, hashsize, hash_row_width, auxhash_size,
                      auxhash_row_width) {
  clear_hash(buf);
...
//    a) guarantee that caching hash entries contains valid data and use this
//    fact to cut off some tests b) don't make such guarantees and make strict
//    tests of actual data in buf[]
...
CachingMatchFinder(BYTE *buf, uint dictsize, uint hashsize, int _hash_row_width,
                   uint auxhash_size, int auxhash_row_width);
...
void prefetch_hash(BYTE *p) {
  ...
  PtrVal *table = HTable + h * hash_row_width2,
         *tabend = table + hash_row_width2;
  for (; table < tabend; table += CACHE_ROW / sizeof(*table))
    prefetch(*table);
}

// Read next ptr/key from hash and save here previous pair (shifted from
// previous position)
#define next_pair()                                                            \
  PtrVal x0 = x1;                                                              \
  x1 = *table;                                                                 \
...
DISTANCE *find_all_matches(byte *p, void *bufend, DISTANCE *matches) {
  ...
  UINT h = hashx(N, p, HashShift); // Hash key of the string for which we
                                   // perform the match search
  // Pointers to the current hash element and end of hash row
  PtrVal *table = HTable + h * hash_row_width2,
         *tabend = table + hash_row_width2;
  // x1/v1 keeps last match candidate and its key (we init them with values
  // which would be saved in the first hash slot)
  PtrVal x1 = fromPtr(p);
  UINT v1 = key(p);
  BYTE *q1;
  ...
  while (table != tabend) {
    ...
    if ((t & 0xff) == 0) {
      q1 = toPtr(x1);
      if (t == 0 && accept_optimal_match(N + 3, p, q1, bufend))
        goto len7;
      else if (t & 0xff00 && accept_optimal_match(N, p, q1, bufend)) {
        SAVE_MATCH(N, p - q1);
        goto len4;
      } else if (t & 0xff0000 && accept_optimal_match(N + 1, p, q1, bufend)) {
        SAVE_MATCH(N + 1, p - q1);
        goto len5;
      } else if (accept_optimal_match(N + 2, p, q1, bufend)) {
        SAVE_MATCH(N + 2, p - q1);
        goto len6;
      }
    }
...
uint find_matchlen(byte *p, void *bufend, UINT prevlen) {
  UINT len; // Length of largest string already found
  UINT h = hashx(
      N, p, HashShift); // Hash key of string for which we perform match search
  // Pointers to the current hash element and end of hash row
  PtrVal *table = HTable + h * hash_row_width2,
         *tabend = table + hash_row_width2;
  // x1/v1 keeps last match candidate and its key (we init them with values
...
void update_hash1(BYTE *p) {
  ...
  for (int j = hash_row_width_up; j -= 2;)
    HTable[h + j] = HTable[h + j - 2], HTable[h + j + 1] = HTable[h + j - 1];
  HTable[h] = fromPtr(p);
  HTable[h + 1] = key(p);
}
// Skip match starting from p with length len and update hash with strings using
// given step
void update_hash(BYTE *p, UINT len, UINT step) {
  // Update hash with strings at the start and end of match plus part of strings
  // inside match
  if (len > 1)
    update_hash1(p + 1);
  for (int i = 2; i < len - 1; i += step)
...
template <uint N>
CachingMatchFinder<N>::CachingMatchFinder(BYTE *buf, uint dictsize,
                                          uint hashsize, int _hash_row_width,
                                          uint auxhash_size,
                                          int auxhash_row_width) {
base = buf;
hash_row_width = _hash_row_width;
...
hash_row_width_up =
    hash_row_width + (hash_row_width % 2); // increment odd values by 1
...
template <uint N> void CachingMatchFinder<N>::clear_hash(BYTE *buf) {
for (int i = 0; i < HashSize; i += 2)
  HTable[i] = fromPtr(buf + 1), HTable[i + 1] = key(buf + 1);
}

template <uint N> void CachingMatchFinder<N>::shift(BYTE *buf, int shift) {
for (int i = 0; i < HashSize; i += 2)
  HTable[i] =
      HTable[i] > fromPtr(buf + shift) ? HTable[i] - shift : fromPtr(buf + 1);
}
...
// *****************************************************************************************************
// One more caching match finder. This one doesn't shift data in hash rows, but
// keeps head index instead
// *****************************************************************************************************
...
CycledCachingMatchFinder(BYTE *buf, uint dictsize, uint hashsize,
                         int _hash_row_width, uint auxhash_size,
                         int auxhash_row_width);
...
int error() {
  return HTable == NULL || Head == NULL ? FREEARC_ERRCODE_NOT_ENOUGH_MEMORY
                                        : FREEARC_OK;
} // Returns error code if there is any problem in MF work
...
uint find_matchlen(byte *p, void *bufend, UINT prevlen) {
  ...
  UINT h = hashx(
      N, p, HashShift); // Hash key of string for which we perform match search
  UINT i = (Head[h] = Head[h] == 0
                          ? hash_row_width - 1
                          : Head[h] - 1); // index of new head of this hash row
  PtrVal x1;
  // Pointers to the current hash element and end of hash row
  PtrVal *rowstart = HTable + h * hash_row_width * 2,
         *rowend = rowstart + hash_row_width * 2;
...
len6:
  ...
  while (table != tabend) {
    ...
    if (t == 0) {
      BYTE *q1 = toPtr(x1);
      if (val32equ(p + N, q1 + N) || !ChangePair(p - q, p - q1))
        goto len7;
    }
  }
  return accept_match(N + 2, p, q, bufend);
...
template <uint N>
CycledCachingMatchFinder<N>::CycledCachingMatchFinder(BYTE *buf, uint dictsize,
                                                      uint hashsize,
                                                      int _hash_row_width,
                                                      uint auxhash_size,
                                                      int auxhash_row_width) {
...
template <uint N> void CycledCachingMatchFinder<N>::clear_hash(BYTE *buf) {
...
template <uint N> struct BinaryTreeMatchFinder : BaseMatchFinder {
...
BYTE *LastBufEnd; // Last `bufend` value passed to find_matchlen() used by
                  // update_hash()

BinaryTreeMatchFinder(BYTE *buf, uint dictsize, uint hashsize, int maxdepth,
                      uint auxhash_size, int auxhash_row_width)
    : BaseMatchFinder(buf, dictsize, hashsize, 1, auxhash_size,
                      auxhash_row_width),
      DictSize(dictsize), MaxDepth(maxdepth) {
  BinaryTree = (UINT *)BigAlloc(
      2 * sizeof(UINT) *
      DictSize); // for every dictionary position, we save pointers to the left
                 // and right "sons" as UINT indexes
  if (error() == FREEARC_OK)
    clear_hash(buf);
}
...
// Returns error code if there is any problem in MF work
int error() {
  return BinaryTree == NULL ? FREEARC_ERRCODE_NOT_ENOUGH_MEMORY
                            : BaseMatchFinder::error();
}
...
DISTANCE *find_all_matches(byte *p, void *bufend, DISTANCE *matches) {
  ...
  UINT h = hashx(
      N, p, HashShift); // Hash key of string for which we perform match search
  UINT n = UINT(
      HTable[h]); // Index of the current match checked (relative to `base`)
  HTable[h] = PtrVal(p - base);
  UINT *ptr1 = &BinaryTree[size_t(p - base) * 2], *ptr0 = ptr1 + 1;
  UINT len0 = 0, len1 = 0, lenLimit = (byte *)bufend - p;
  for (;;) {
    if (depth-- == 0 || n == kEmptyHashValue) {
      *ptr0 = *ptr1 = kEmptyHashValue;
      break;
    }
    UINT *pair = &BinaryTree[size_t(n) * 2];
    prefetch(*pair);
    BYTE *q = base + n;
    ...
    if (q[len] < p[len])
      *ptr1 = n, ptr1 = pair + 1, n = *ptr1, len1 = len;
    else
      *ptr0 = n, ptr0 = pair, n = *ptr0, len0 = len;
  }
  return matches;
}
...
uint find_matchlen(byte *p, void *bufend, UINT prevlen) {
  ...
  UINT *ptr1 = &BinaryTree[size_t(p - base) * 2], *ptr0 = ptr1 + 1;
  UINT len0 = 0, len1 = 0, lenLimit = (byte *)bufend - p;
  LastBufEnd = (byte *)bufend;
  if (bufend <= p || lenLimit <= maxLen) {
    *ptr0 = *ptr1 = kEmptyHashValue;
    return maxLen;
  }
  for (;;) {
      ...
      if (++len != lenLimit && q1[len] == p[len])
        while (++len != lenLimit)
          if (q1[len] != p[len])
            break;
      if (len > maxLen) {
        if (accept_len_dist(len, p - q1) &&
            !(len == maxLen + 1 && ChangePair(p - q, p - q1))) {
          maxLen = len;
          q = q1;
        }
        if (len == lenLimit) {
...
void update_hash1(BYTE *p, UINT maxLen = 256 * kb) {
  void *bufend =
      (LastBufEnd > p && LastBufEnd - p > maxLen) ? p + maxLen : LastBufEnd;
  find_matchlen(p, bufend, MINLEN - 1);
}
// Skip match starting from p with length len and update hash with strings using
// given step
void update_hash(BYTE *p, UINT len, UINT step) {
  step = mymax(step, len / 2503); // optimization to avoid O(n^2) time behavior
...
// Called when next data chunk read into NON-SLIDING buffer
template <uint N> void BinaryTreeMatchFinder<N>::clear_hash(BYTE *buf) {
if (HTable)
  iterate_var(i, HashSize) HTable[i] = PtrVal(kEmptyHashValue);
// No need to initialize the BinaryTree[]
}

// Called after data was shifted 'shift' bytes backwards in SLIDING WINDOW
// buf[]. We make appropriate corrections in HTable and BinaryTree, reducing and
// moving values and replacing pointers to data shifted out with
// kEmptyHashValue.
template <uint N> void BinaryTreeMatchFinder<N>::shift(BYTE *buf, int shift) {
iterate_var(i, HashSize) HTable[i] =
    PtrVal(UINT(HTable[i]) > shift ? UINT(HTable[i]) - shift : kEmptyHashValue);
for (size_t i = shift * 2; i < size_t(DictSize) * 2; i++)
  BinaryTree[i - shift * 2] =
      BinaryTree[i] > shift ? BinaryTree[i] - shift : kEmptyHashValue;
}
...
template <class MatchFinder> struct LazyMatching {
MatchFinder mf;
uint nextlen;
BYTE *prevq, *nextq;

LazyMatching(BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
             uint auxhash_size, int auxhash_row_width)
    : mf(buf, dictsize, hashsize, hash_row_width, auxhash_size,
         auxhash_row_width) {
  nextlen = 0;
}
void clear_hash(BYTE *buf) {
...
uint find_matchlen(byte *p, void *bufend, UINT _prevlen) {
  ...
  if (nextlen >= MINLEN && nextq[-1] == *p &&
      (nextlen + 1 >= prevlen && nextdist < prevdist ||
       ...
       nextlen + 1 > prevlen + 1 ||
       nextlen + 1 + 1 >= prevlen && prevlen >= MINLEN &&
           ChangePair(nextdist, prevdist))) {
    debug(printf("Extending %d:%d -> %d:%d\n", nextlen, nextq - p, nextlen + 1,
                 nextq - 1 - p));
    prevlen = nextlen + 1;
    prevq = nextq - 1;
    return prevlen;
  ...
  // Truncate current match if match at next position will be better (LZMA's
  // algorithm)
  if (nextlen >= prevlen && nextdist < prevdist / 4 ||
      nextlen == prevlen + 1 && !ChangePair(prevdist, nextdist) ||
      nextlen > prevlen + 1 ||
      nextlen + 1 >= prevlen && prevlen >= MINLEN &&
          ChangePair(nextdist, prevdist)) {
    return MINLEN - 1;
  } else {
    return prevlen;
...
void invalidate_match() {
  mf.invalidate_match();
  nextlen = MINLEN - 1; // We can't assign 0 because pointer to p already
                        // inserted into hash - find_match may return q==p
}
};

// ****************************************************************************
// This is MF transformer which adds separate small hashes for searching
// 2/3-byte strings
// ****************************************************************************
...
Hash3(BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
      uint auxhash_size, int auxhash_row_width);
~Hash3();
int error();
...
// Prefetch MF data associated with provided `p` (we suppose that Hash3 own data
// are small and therefore are already cached)
...
Hash3<MatchFinder, HASH3_LOG, HASH2_LOG, FULL_UPDATE>::Hash3(
    BYTE *buf, uint dictsize, uint hashsize, int hash_row_width,
    uint auxhash_size, int auxhash_row_width)
    : mf(buf, dictsize, hashsize, hash_row_width, auxhash_size,
         auxhash_row_width) {
...
return HTable == NULL || HTable2 == NULL ? FREEARC_ERRCODE_NOT_ENOUGH_MEMORY
                                         : mf.error();
}

template <class MatchFinder, int HASH3_LOG, int HASH2_LOG, bool FULL_UPDATE>
void Hash3<MatchFinder, HASH3_LOG, HASH2_LOG, FULL_UPDATE>::clear_hash(
    BYTE *buf) {
mf.clear_hash(buf);
clear_hash3(buf);
}
...
template <class MatchFinder, int HASH3_LOG, int HASH2_LOG, bool FULL_UPDATE>
void Hash3<MatchFinder, HASH3_LOG, HASH2_LOG, FULL_UPDATE>::clear_hash3(
    BYTE *buf) {
if (HTable)
  iterate_var(i, HashSize) HTable[i] = buf + 1;
if (HTable2)
  iterate_var(i, HashSize2) HTable2[i] = buf + 1;
}

template <class MatchFinder, int HASH3_LOG, int HASH2_LOG, bool FULL_UPDATE>
void Hash3<MatchFinder, HASH3_LOG, HASH2_LOG, FULL_UPDATE>::shift(BYTE *buf,
                                                                  int shift) {
mf.shift(buf, shift);
iterate_var(i, HashSize) HTable[i] =
    HTable[i] > buf + shift ? HTable[i] - shift : buf + 1;
iterate_var(i, HashSize2) HTable2[i] =
    HTable2[i] > buf + shift ? HTable2[i] - shift : buf + 1;
}
...
template <class MatchFinder1, class MatchFinder2> struct CombineMF {
MatchFinder1 mf1;
MatchFinder2 mf2;
BYTE *q;

// Constructor just saves references to two matchfinders being combined
CombineMF(BYTE *buf, uint dictsize, uint hash_size, int hash_row_width,
          uint auxhash_size, int auxhash_row_width)
    : mf1(buf, dictsize, hash_size, hash_row_width, 0, 0),
      mf2(buf, dictsize, auxhash_size, auxhash_row_width, 0, 0){};

// Prefetch MF data associated with provided `p` (we suppose that mf2 data are
// small and therefore are already cached)
void prefetch_hash(BYTE *p) { mf1.prefetch_hash(p); }

// Fill the `matches` buffer with len/dist from mf2 (shorter matches), then mf1
// (longer matches)
DISTANCE *find_all_matches(byte *p, void *bufend, DISTANCE *matches) {
...
uint find_matchlen(byte *p, void *bufend, UINT prevlen) {
  ...
  // If second match is longer and it's not too far while being just 1 byte
  // longer
  if (len1 >= mf1.MINLEN && len1 > len2 &&
      !(len2 >= mf2.MINLEN && len1 == len2 + 1 && ChangePair(p - q2, p - q1))) {
    q = q1;
    return len1;
  } else {
    q = q2;
...
static uint min_length() {
  return mymin(MatchFinder1::min_length(), MatchFinder2::min_length());
}
