...
int DISPACK_METHOD::decompress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  BYTE *In = NULL,
       *Out = NULL; // Указатели на входные и выходные данные, соответственно
  ...
  for (;;) {
    ...
    if (!is_tag(tag) || tag == TAG_DATA) {
      // скопируем неупакованные данные, из них 4 байта мы уже возможно
      // прочитали ;)
      int done = 0, len;
      if (tag == TAG_DATA) {
        READ4(len);
      ...
      int InSize, OutSize; // количество байт во входном и выходном буфере,
                           // соответственно
      // Произвести декодирование и получить размер выходных данных
      READ4(OutSize);
      READ4(InSize);
      if (OutSize > BlockSize || InSize > InBufferSize)
        ReturnErrorCode(FREEARC_ERRCODE_BAD_COMPRESSED_DATA);
      READ(In, InSize);
...
EXETYPE detect(BYTE *buf, int len) {
  ...
  return double(e8) / len >= 0.002 && double(exe + obj) / e8 >= 0.20 &&
                 double(exe) / e8 >= 0.01
             ? EXETYPE_EXE
             : EXETYPE_DATA;
}
...
int DISPACK_METHOD::compress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  for (;;) {
    ...
    if ((BaseAddress += InSize + len) >= 3u << 30)
      BaseAddress -= 2u << 30;
  }
finished:
...
// параметры (функция, обратная к parse_DISPACK)
void DISPACK_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  DISPACK_METHOD defaults;
  char BlockSizeStr[100] = ":";
  showMem(BlockSize, BlockSizeStr + 1);
  sprintf(buf, "dispack070%s%s",
...
COMPRESSION_METHOD *parse_DISPACK(char **parameters) {
  if (strcmp(parameters[0], "dispack") == 0 ||
    ...
    // Если название метода (нулевой параметр) - "dispack", то разберём
    // остальные параметры
...
static int DISPACK_x =
    AddCompressionMethod(parse_DISPACK); // Зарегистрируем парсер метода DISPACK
