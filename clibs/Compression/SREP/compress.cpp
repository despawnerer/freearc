// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Single block compressor for slow methods (-m3..-m5) using matching with
// fixed-size blocks ********************************************************
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

bool record_match(bool ROUND_MATCHES, unsigned L, unsigned MIN_MATCH,
                  unsigned BASE_LEN, Offset block_start, HashTable &h,
                  char *buf, int block_size, STAT *&stat,
                  unsigned last_match_end, unsigned &match_end,
                  unsigned &literal_bytes, unsigned i, Chunk k) {
  unsigned add_len,
      match_len = h.match_len(k, &buf[last_match_end], &buf[i],
                              buf + block_size, block_start, buf, &add_len);
  ...
  if (match_len >= MIN_MATCH) // If match is large enough, then ...
  {
    ...
    if (match_offset < pc.max_offset)
      pc.record_match++, pc.total_match_len += match_len;
    ENCODE_LZ_MATCH(
        stat, ROUND_MATCHES, BASE_LEN, match_start - last_match_end,
        match_offset,
        match_len); // ... save triple (literal_len, match_offset, match_len)
...
#define check_match(i, hash, match_found)                                      \
  {                                                                            \
    /*if ((i) >= last_match_end) */ {                                          \
      Chunk k =                                                                \
          h.find_match(&buf[i], i, block_size, hash,                           \
                       block_start + i); /* Number of previous chunk with the  \
                                            same contents as current one  */   \
      if (k != NOT_FOUND) {              /* If match was found - record it              */                                                               \
        unsigned match_end;                                                    \
        if (record_match(ROUND_MATCHES, L, MIN_MATCH, BASE_LEN, block_start,   \
                         h, buf, block_size, stat, last_match_end, match_end,  \
                         literal_bytes, i, k)) {                               \
...
void compress(bool ROUND_MATCHES, unsigned L, unsigned MIN_MATCH,
              unsigned BASE_LEN, Offset block_start, HashTable &h, char *buf,
              int block_size, unsigned &literal_bytes, STAT *in_statbuf,
              STAT *statbuf, STAT *&stat) {
  stat = statbuf;             // where to save next found match info
  NUMBER last_match_end = 0;  // points to the end of last match written, we
                              // shouldn't start new match before it
  literal_bytes = block_size; // number of literal bytes = block size minus sum
                              // of match lengths
  ...
  DECODE_LZ_MATCH(instat, false, ROUND_MATCHES, BASE_LEN, block_start, lit_len,
                  LZ_MATCH, lz_match);
  ...
  if (2 * L <= block_size) {
    ...
    // PRIME1 powers required to calculate hash2 from hash1 by adding a few
    // missing bytes right *before* the hash1 window start
    BigHash PRIME1_powers[CYCLES];
    PRIME1_powers[CYCLES - 1] = power(BigHash(PRIME1), L - CYCLES);
    ...
    for (NUMBER i = 0; i < 1; i++) {
      ...
      for (NUMBER j = 0; true; j++) {
        h.mark_match_possibility<ACCELERATOR>(
            hash1); // Mark bitarr for the last OFFSET bytes of chunk
        if (j == OFFSET)
          break;
        hash1.update(buf[j], buf[L - OFFSET + j]);

        hash2.update(
            0, buf[L - OFFSET + j]); // Calc hash2(buf[0]..buf[L-1]) from the
                                     // hash1(buf[0]..buf[L-OFFSET-1]):
      } //   just add OFFSET next bytes while "removing" zero bytes

      check_match(i, hash2, match_found1);
    match_found1:;
      h.add_hash(&buf[i], i, block_size, (block_start + i) / L, hash2,
                 block_start + i);
    }
      ...
      for (NUMBER next_chunk = i + L; likely(i < next_chunk);) {
        ...
        if (i >= match_start) {
          ...
          if (match_start >= last_match_end) {
            ...
            ENCODE_LZ_MATCH(stat, ROUND_MATCHES, BASE_LEN, literal_len,
          ...
          DECODE_LZ_MATCH(instat, false, ROUND_MATCHES, BASE_LEN,
                          block_start + match_start + match_len, lit_len,
                          LZ_MATCH, lz_match);
          match_start = lz_match.dest - block_start;
          match_len = lz_match.len;
          match_offset = lz_match.dest - lz_match.src;
        }

        // UPDATE hash1 UNTIL last_match_end, ROUNDED DOWN TO X-BYTES BOUNDARY
        const NUMBER X = mymax(CYCLES, 4);
        NUMBER next_i = mymin(next_chunk - CYCLES,
                              (last_match_end > 0 ? last_match_end - 1 : 0));
        ...
        const int LOOKAHEAD =
            (ACCELERATOR
                 ? 256
                 : 128); // 2*50 ns (TLB+Data) === LOOKAHEAD*hash1.update(1ns)
                         // ==>  LOOKAHEAD ~= 100 bytes per cycle
        NUMBER last_i = mymin(next_chunk, i + LOOKAHEAD);
        BigHash hashes1_buf[LOOKAHEAD];
        BigHash hashes2_buf[LOOKAHEAD * 2];

        // PREFETCH bitarr AND UPDATE hash1
        if (ACCELERATOR) {
          BigHash *hashes1 = hashes1_buf;
          for (NUMBER i0 = i; likely(i0 < last_i);
               i0 += CYCLES) // MAIN CYCLE, processing every input byte, except
                             // for (most part of) match contents
          {
        ...
        for (BigHash *hashes1 = hashes1_buf; likely(i < last_i);) {
          if (ACCELERATOR) {
            ...
            if (unlikely(h.check_match_possibility<ACCELERATOR>(hsh1))) {
              i++;
              hash2.value = hsh1; // Calc hash2(buf[i]..buf[i+L-1]) from the
                                  // hash1(buf[i+OFFSET]..buf[i+L-1])
        ...
        for (BigHash *hashes2 = hashes2_buf; hashes2 < hashes2_last;) {
          BigHash hsh = *hashes2++;
          NUMBER i = NUMBER(*hashes2++);
          check_match(i, hsh, match_found2);
        }
      match_found2:;
      }

      // SECOND SUB-MAIN CYCLE, REPROCESSING LAST `CYCLES` BYTES OF THE l-BYTE
      // CHUNK
      if (ACCELERATOR) {
        hash2.value = hash1.value = saved_hash1;
        h.mark_match_possibility<ACCELERATOR>(
            hash1); // Mark bitarr for the last CYCLES bytes of chunk
        ...
        for (NUMBER j = 1; j < CYCLES; j++) {
          ...
          hash2.update(
              0, buf[i - CYCLES + j +
                     L]); // Calc hash2(buf[i+CYCLES-1]..buf[i+L+CYCLES-2]) from
                          // the hash1(buf[i+CYCLES-1]..buf[i+L-1]):
        } //   just add CYCLES-1 next bytes while "removing" zero bytes
      }
      h.add_hash(&buf[i], i, block_size, (block_start + i) / L,
                 ACCELERATOR ? hash2 : hash1, block_start + i);
    }
  }
}
