// Full sources required to compile srep.cpp are available at
// http://freearc.org/Download-Alpha.aspx All rights reserved. Mail me if you
// have any questions or want to buy a commercial license for the source code.
...
#if 0 // #ifdef FREEARC_WIN : unfortunately, it doesnt' show consistent speed
      // improvement Also, CryptoAPI functions should be dynaloaded from the
      // Advapi32.dll because they are available only starting from WinXP
...
void compute_sha1(void *, void *buf, int size, void *result) {
  if (CryptoAPI.sha1(buf, size, result))
    return;
  hash_state state;
  sha1_init(&state);
...
void compute_md5(void *, void *buf, int size, void *result) {
  if (CryptoAPI.md5(buf, size, result))
    return;
  hash_state state;
  md5_init(&state);
...
// hash содержит значение хеш-функции от последних L обработанных байт, для
// удобства обновления используется скользящая хеш-функция.
// constructor(buf,L,seed) создаёт хеш, параметризованный seed, и инициализирует
// его первыми L байтами буфера update(sub,add) выносит из хеша байт sub и
// добавляет байт add.

template <class ValueT> struct FakeRollingHash {
...
template <class T> T power(T base, unsigned n) {
  T result = 1;
  while (n) {
    if (n % 2)
      result *= base, n--;
...
template <class ValueT> struct PolynomialHash {
  ...
  void update(BYTE b) { value = value * PRIME + b; }
...
template <class ValueT> struct PolynomialRollingHash {
  operator ValueT() { return value; }
  ValueT value, PRIME, PRIME2, PRIME3, PRIME4, PRIME5, PRIME6, PRIME7, PRIME8,
      PRIME_L, PRIME_L1, PRIME_L2, PRIME_L3;
  int L;
  ...
  PolynomialRollingHash(int _L, ValueT seed) {
    ...
    PRIME_L3 =
        seed *
        (PRIME_L2 = seed * (PRIME_L1 = seed * (PRIME_L = power(PRIME, L))));
  }
  ...
  template <int N> void update(void *_ptr) {
    BYTE *ptr = (BYTE *)_ptr;
    switch (N % 4) {
    case 0:
      break;
    ...
    case 1:
      value = value * PRIME + ptr[L] - PRIME_L * ptr[0];
      break;

    case 2:
      value = value * PRIME2 + PRIME * ptr[L] + ptr[L + 1] - PRIME_L1 * ptr[0] -
              PRIME_L * ptr[1];
      break;

    case 3:
      value = value * PRIME3 + PRIME2 * ptr[L] + PRIME * ptr[L + 1] +
              ptr[L + 2] - PRIME_L2 * ptr[0] - PRIME_L1 * ptr[1] -
              PRIME_L * ptr[2];
      break;
    }

    for (int i = 0; i < N / 4; i++, ptr += 4)
      value = value * PRIME4 + PRIME3 * ptr[N % 4 + L] +
              PRIME2 * ptr[N % 4 + L + 1] + PRIME * ptr[N % 4 + L + 2] +
              ptr[N % 4 + L + 3] - PRIME_L3 * ptr[N % 4 + 0] -
              PRIME_L2 * ptr[N % 4 + 1] - PRIME_L1 * ptr[N % 4 + 2] -
              PRIME_L * ptr[N % 4 + 3];
  }
...
template <class ValueT> void PolynomialRollingHash<ValueT>::moveto(void *_buf) {
  value = 0;
  BYTE *buf = (BYTE *)_buf;
  const int N = 16, S = 4;
  for (int i = 0; i < (L & ~(N - 1)); i += N) {
#define STEP(n)                                                                \
  (value = value * PRIME4 + PRIME3 * buf[i + (n)*S] +                          \
           PRIME2 * buf[i + 1 + (n)*S] + PRIME * buf[i + 2 + (n)*S] +          \
           buf[i + 3 + (n)*S])
    STEP(0);
    if (N > S)
      STEP(1);
...
// Requires GCC4.3 and SSE4.2-enabled CPU; and of course compatible only with
// Crc32CastagnoliPolynom
#include <cpuid.h>
#include <x86intrin.h>
...
void compute_siphash(void *key, void *buf, int size, void *result) {
  *(uint64_t *)result =
      siphash((unsigned char *)key, (const unsigned char *)buf, size);
}
...
#define VMAC_TAG_LEN                                                           \
  128 /* Requesting VMAC-128 algorithm (instead of VMAC-64) */
  ...
  void init(void *seed = NULL) {
    if (!initialized || seed) {
      ...
      if (seed)
        ...
        cryptographic_prng(key, VMAC_KEY_LEN_BYTES);
...
struct VDigest {
  ...
  void compute(const void *ptr, size_t size, void *result) {
    ...
    vhash2.compute(ptr, size,
                   (BYTE *)result + sizeof(Digest) - VMAC_TAG_LEN_BYTES);
  }
};
...
// Hash function processing the (buf,size) and storing computed hash value to
// the result
...
struct hash_descriptor {
  ...
  unsigned hash_seed_size; // additional bytes stored in the archive header
                           // (seed value for randomized hashes)
  ...
  hash_func_t hash_func;   // hash function
    ...
    {"md5", 0, 0, MD5_SIZE, 0, compute_md5},
    {"", 1, 0, MD5_SIZE, 0, 0},
    {"sha1", 2, 0, SHA1_SIZE, 0, compute_sha1},
    {"sha512", 3, 0, SHA512_SIZE, 0, compute_sha512},
    {"vmac", 4, VMAC_KEY_LEN_BYTES, VMAC_TAG_LEN_BYTES, new_vhash,
     compute_vhash},
    {"siphash", 5, SIPHASH_KEY_LEN_BYTES, SIPHASH_TAG_LEN_BYTES, new_siphash,
     compute_siphash},
};

const char *DEFAULT_HASH = "vmac",
           *HASH_LIST = "vmac(default)/siphash/md5/sha1/sha512";
...
typedef size_t
    HashValue; // Hash of L-byte block, used as first step to find the match
typedef uint32 StoredHashValue; // Hash value stored in hasharr[]
typedef uint64
    BigHash; // We need 64 bits for storing index+value in the chunkarr+hasharr
typedef uint32 Chunk; // Uncompressed file are splitted into L-byte chunks, it's
                      // the number of chunk in the file
const Chunk MAX_CHUNK = Chunk(-1), NOT_FOUND = 0;
const int MAX_HASH_CHAIN = 12;

#define min_hash_size(n)                                                       \
  (((n) / 4 + 1) * 5) /* Minimum size of hash for storing n elements */
...
struct SliceHash {
  ...
  SliceHash(Offset filesize, unsigned _L, unsigned MIN_MATCH,
    ...
    slice_size = L / slices_in_block; // to do: 8/16-byte entry (считывать в
                                      // check по одному байту из h)
    check_slices =
        int((MIN_MATCH - L) / slice_size) -
        io_accelerator; // if less that this amount of slices around match has
                        // the same hashes, then we are sure that match can't be
                        // extended to MIN_MATCH size
    if (io_accelerator < 0 || check_slices <= 0)
      memreq = 0; // no need in SliceHash since each potential match is almost
                  // guaranteed to have MIN_MATCH matched bytes
    else
      memreq = filesize / L * sizeof(entry); // one `entry` per L input bytes
  }
  ...
  bool check(Chunk chunk, void *p, int i, int block_size) {
    if (h == NULL)
      return true;
    if (i < L ||
        block_size - i < 2 * L) // not enough bytes around chunk in the buffer
                                // to check them against saved hashes
      return true;
    entry checksum = h[chunk + 1];
    NUMBER j, k;
    for (j = 0;; j++) {
      if (j == check_slices)
        return true;
      if (((checksum >> (j * BITS)) & ONES) !=
          hash((char *)p + L + j * slice_size, slice_size))
        break;
    }
    checksum = h[chunk - 1];
    for (k = 0;; k++) {
      if (k + j == check_slices)
        return true;
      if (((checksum >> ((slices_in_block - (k + 1)) * BITS)) & ONES) !=
          hash((char *)p - (k + 1) * slice_size, slice_size))
        break;
    }
    return false;
  }
...
struct HashTable {
  ...
  HashTable(bool _ROUND_MATCHES, bool _COMPARE_DIGESTS,
      ...
      : mmap_infile(_mmap_infile),
        slicehash(_filesize, _L, MIN_MATCH, io_accelerator) {
    _errcode = ERROR_MEMORY;
    ROUND_MATCHES = _ROUND_MATCHES;
    COMPARE_DIGESTS = _COMPARE_DIGESTS;
    PRECOMPUTE_DIGESTS = _PRECOMPUTE_DIGESTS;
    CONTENT_DEFINED_CHUNKING = _CONTENT_DEFINED_CHUNKING;
    L = _L;
    filesize = mymax(_filesize, L);
    curchunk = 0;
    bitarr = NULL;
    chunkarr = NULL;
    hasharr = NULL;
    ...
    if (INMEM_COMPRESSION) {
      _errcode = NO_ERRORS;
      hs = total_chunks = bitarrsize = 0;
      slicehash.alloc(LargePageMode);
      return;
    }
    MainDigest.init();
    PrepDigest =
        MainDigest; // we need two equal digests since they are used in 2
                    // threads and have internal state modified due hashing
    total_chunks = filesize / L;
    if (CONTENT_DEFINED_CHUNKING)
      total_chunks +=
          total_chunks /
          (total_chunks > 1024
               ? 10
               : 1); // In the CONTENT_DEFINED_CHUNKING mode, chunks may have
                     // any size, so we alloc 10% extra space
    chunknum_mask = roundup_to_power_of(total_chunks + 2, 2) - 1;
    hash_mask = ~chunknum_mask; // Masks for cnunk number and hash bits in the
                                // chunkarr[] item
    hs = roundup_to_power_of(min_hash_size(total_chunks), 2);
    if (hs > size_t(-1) || total_chunks > MAX_CHUNK - 2)
      return;
    hashsize = hs, hashsize1 = hashsize - 1,
    hash_shift = sizeof(HashValue) * CHAR_BIT - lb(hashsize);
    bitarrsize =
        (BITARR_ACCELERATOR == 0 || CONTENT_DEFINED_CHUNKING)
            ? 0
            : roundup_to_power_of(
                  mymax(total_chunks / CHAR_BIT * BITARR_ACCELERATOR, 2),
                  2); // bit array checking works fine until 1/8 of bitarr[]
                      // gets filled
    bitshift =
        sizeof(HashValue) * CHAR_BIT -
        lb(bitarrsize); // bitarrsize should be >=2, otherwise hash>>bitshift ==
                        // hash>>64 == hash>>0 and indexing panics

    // Allocate arrays starting with the most frequently accessed (to increase
    // their chances to become allocated using large pages)
    bitarr = (BYTE *)BigAlloc(bitarrsize, LargePageMode);
    if (!bitarr && bitarrsize != 0)
      return;
    chunkarr = (Chunk *)BigAlloc(hashsize * sizeof(Chunk), LargePageMode);
    if (!chunkarr)
      return;
    if (!CONTENT_DEFINED_CHUNKING) {
      hasharr = (StoredHashValue *)BigAlloc(
          total_chunks * sizeof(StoredHashValue), LargePageMode);
      if (!hasharr)
        return;
    }
    slicehash.alloc(LargePageMode);
    if (CONTENT_DEFINED_CHUNKING) {
      startarr =
          (Offset *)BigAlloc(total_chunks * sizeof(Offset), LargePageMode);
      if (!startarr)
        return;
    }
    if (COMPARE_DIGESTS) {
      digestarr =
          (Digest *)BigAlloc(total_chunks * sizeof(Digest), LargePageMode);
      if (!digestarr)
        return;
    }

    my_memset(bitarr, 0, bitarrsize);
    my_memset(chunkarr, 0, hashsize * sizeof(*chunkarr));
    if (NOT_FOUND != 0) {
  ...
  // method settings
  Offset memreq() {
    return hs * sizeof(*chunkarr) +
           total_chunks * ((CONTENT_DEFINED_CHUNKING ? sizeof(*startarr)
                                                     : sizeof(*hasharr)) +
                           (COMPARE_DIGESTS ? sizeof(*digestarr) : 0)) +
           bitarrsize + slicehash.memreq;
  }

  // Performed once for each block read
  void prepare_buffer(Offset offset, char *buf, int size) {
    if (PRECOMPUTE_DIGESTS) { // Save chunk digests for secondary, reliable
                              // match checking
      Chunk curchunk = offset / L;
...
#define index_hash(hash2) (hash2)          /* value used to index chunkarr[] */
  ...
  Chunk add_hash(void *p, int i, int block_size, Chunk curchunk, BigHash hash2,
                 Offset new_offset) {
    return add_hash0<false>(p, i, block_size, curchunk, index_hash(hash2),
                            stored_hash(hash2), new_offset);
  }
  ...
  Chunk find_match(void *p, int i, int block_size, BigHash hash2,
                   Offset new_offset) {
    return find_match0(p, i, block_size, index_hash(hash2), stored_hash(hash2),
                       new_offset);
  }
...
#define speed_opt true /* true: don't use slicehash to try >1 match in -m5 */
  ...
  Chunk add_hash0(void *p, int i, int block_size, Chunk curchunk, BigHash index,
    ...
    CDC &&pc.find_match++;
    CDC || (hasharr[curchunk] = stored_value); // save hash of chunk for
                                               // primary, quick match checking
    if (curchunk == NOT_FOUND)
      return NOT_FOUND; // it's impossible to hash this chunk number since it's
                        // used as signal value

    size_t h = first_hash_slot(index);
    int limit = MAX_HASH_CHAIN;
    Chunk found = NOT_FOUND, saved_hash = chunkarr_value(index, 0);
    for (Chunk value;
         (value = chunkarr[hash_index(h)]) != NOT_FOUND && --limit;) {
      ...
      if (get_hash(value) == saved_hash) {
        ...
        if (match_offset < pc.max_offset)
          CDC &&pc.check_hasharr++;
        // Replace in hash chunk with the same digest (-m1..-m3) or hash value
        // (-m4/-m5), supposing that it has the same contents
        if (CDC || hasharr[chunk] == stored_value) {
          if (match_offset < pc.max_offset)
            CDC &&pc.hash_found++; // reuse chunkarr[] item only when...
          if (!COMPARE_DIGESTS
                  ? (speed_opt ||
                     slicehash.check(
                         chunk, p, i,
                         block_size)) // .. subblocks around has the same 1-bit
                                      // digests (-m5) or always (-m4)
                  : (0 ==
                     memcmp(
                         digestarr[chunk], digestarr[curchunk],
                         sizeof(*digestarr)))) // .. entire block has the same
                                               // 160-bit digest (-m1..-m3)
          {
            found = chunk;
            break;
          }
      ...
      h++, ((limit & 3) == 0) &&
               (CDC && pc.find_match_memaccess++,
                h = next_hash_slot(index, h)); // compute next hash slot
    }
  ...
  Chunk find_match0(void *p, int i, int block_size, BigHash index,
    ...
    size_t h = first_hash_slot(index);
    int limit = MAX_HASH_CHAIN;
    Chunk saved_hash = chunkarr_value(index, 0);
    for (Chunk value;
         (value = chunkarr[hash_index(h)]) != NOT_FOUND && --limit;) {
      // First check a few hash bits stored in unused chunkarr[] item bits
      if (get_hash(value) == saved_hash) {
        ...
        if (hasharr[chunk] == stored_value) {
          if (match_offset < pc.max_offset)
            pc.hash_found++;
          if (!COMPARE_DIGESTS) {
            // ... we either suppose that chunks are the same (for -m4), check
            // 1-bit digests of subchunks around (for -m5) ...
  ...
  unsigned match_len(Chunk start_chunk, char *min_p, char *start_p,
    ...
    if (new_offset - old_offset < pc.max_offset)
      pc.check_len++;
    char *p = start_p;
    *add_len = 0;
    ...
    if (COMPARE_DIGESTS) {
      ...
      while (p += L,
             (old_offset += L) < offset) // Skip checking first chunk since it
                                         // was already done in find_match()
      {
    ...
    } else if (old_offset < offset) { // -m4/-m5: check match by rereading old
                                      // data from infile

      // First, compare bytes before match start (which is rounded to L-byte
      // chunk boundary)
      int n = mymin(old_offset,
                    mymin(L, start_p - min_p)); // how much bytes we can check
      ...
      for (; old_offset < offset; old_offset += BUFSIZE) {
        ...
        int len = mmap_infile.read(&old, oldbuf, old_offset,
                                   BUFSIZE); // Read old data from file
        if (len != BUFSIZE)
          goto stop; // If there was any problem reading entire buf
        for (char *q = old; q < old + len; p++, q++)
          if (p == last_p || *p != *q)
            goto stop; // Exit function once we've found end of match
      }
    } else if (!ROUND_MATCHES) {
      // -m4/-m5: compare bytes (that are present in current block) before match
      // start
      int i,
          n = mymin(old_offset - offset,
                    mymin(L, start_p - min_p)); // how much bytes we can check
      for (i = 1; i <= n && start_p[-i] == (buf + (old_offset - offset))[-i];
           i++)
        ;
      *add_len = i - 1;
    }
    ...
    for (char *q = buf + (old_offset - offset); p < last_p && *p == *q;
         p++, q++)
      ;

  stop:
    return p - start_p + *add_len;
  }

  // Chunk start position
  Offset start(Chunk chunk) {
    return CONTENT_DEFINED_CHUNKING ? startarr[chunk] : Offset(chunk) * L;
  }

  // Chunk size in -m1/-m2 mode
  Offset chunksize_CDC(Chunk chunk) {
    return startarr[chunk + 1] - startarr[chunk];
  }
...
Offset HashTable::find_match_CDC(Offset offset, void *p, int size,
                                 BYTE *vhashes) {
  // we have space allocated only for total_chunks chunks
  if (++curchunk >= total_chunks)
    return 0;
  ...
  if (sizeof(*digestarr) + sizeof(BigHash) > 2 * VMAC_TAG_LEN_BYTES) {
    fprintf(stderr, "\nfind_match_CDC hashsize error: %d+%d > 2*%d\n",
            int(sizeof(*digestarr)), int(sizeof(BigHash)), VMAC_TAG_LEN_BYTES);
    abort();
  }
  memcpy(digestarr + curchunk, vhashes, sizeof(*digestarr));
  BigHash index = *(BigHash *)(vhashes + sizeof(*digestarr));

  // найти в хеш-таблице старый блок, эквивалентный новому, и заменить его новым
  // блоком
  ...
  if (chunk != NOT_FOUND && chunksize_CDC(chunk) == size) {
    if (offset < pc.max_offset)
      pc.check_len++, pc.record_match++, pc.total_match_len += size;
...
// CONTENT_DEFINED_CHUNKING needs more space because there is no lower limit on
// the match size So we just alloc block_size bytes
...
bool decompress(bool ROUND_MATCHES, unsigned L, FILE *fout, Offset block_start,
...
public: // ******************************************************* HIGH-LEVEL
  ...
  INDEX save(char *ptr, int len) { // Save contents of memory area
    INDEX index = INVALID_INDEX, first_index = INVALID_INDEX,
...
private: // ******************************************************* MID-LEVEL
  ...
  INDEX next_index(
      INDEX index) { // Returns contents of "next chunk" field for given chunk
    return *(INDEX *)chunk_ptr(index);
  }
  void set_next_index(INDEX index,
                      INDEX next_index) { // Sets "next chunk" field of given
...
private: // ******************************************************* LOW-LEVEL
  ...
  void allocate_block() { // Allocate one more block of memory and add its
...
struct VIRTUAL_MEMORY_MANAGER {
  ...
  VIRTUAL_MEMORY_MANAGER(char *_vmfile_name, Offset _VMBLOCK_SIZE)
      : vmfile_name(_vmfile_name), vmfile(NULL), vmbuf(NULL),
        VMBLOCK_SIZE(_VMBLOCK_SIZE), new_block(0), total_read(0),
        total_write(0) {}
...
// Decompress data using stat[] and in[] and return original data in outbuf[].
// Returns TRUE on successful decompression
bool decompress_FUTURE_LZ(bool ROUND_MATCHES, unsigned L, FILE *fout,
  ...
  for (LZ_MATCH_ITERATOR lz_match = lz_matches.begin();
    ...
    if (lz_match->is_marking_point()) {
      ...
      if (lz_match->len >= maximum_save && lz_match->src < block_start) {
        file_seek(fout, lz_match->src);
        file_read(fout, out, lz_match->len);
        PLUS_READ();
      } else {
...
struct BG_COMPRESSION_THREAD : BackgroundThread {
  ...
  unsigned k;
  char *dict;
  index *hashtable[BUFFERS]; // Place for saving info about maximum hashes and
                             // their indexes
  char *bufptr[BUFFERS];
  char *buf[BUFFERS];
  ...
  BG_COMPRESSION_THREAD(bool _ROUND_MATCHES, bool _COMPARE_DIGESTS,
                        unsigned _BASE_LEN, bool _no_writes,
                        hash_func_t _hash_func, void *_hash_obj,
                        Offset _filesize, Offset _dictsize, unsigned _bufsize,
                        unsigned _header_size, HashTable &_h,
                        DictionaryCompressor &_inmem, MMAP_FILE &_infile,
                        FILE *_fin, FILE *_fout, FILE *_fstat,
                        LPType LargePageMode)
      : errcode(NO_ERRORS), k(0), ROUND_MATCHES(_ROUND_MATCHES),
        COMPARE_DIGESTS(_COMPARE_DIGESTS), BASE_LEN(_BASE_LEN),
        no_writes(_no_writes), hash_func(_hash_func), hash_obj(_hash_obj),
        filesize(_filesize), bufsize(_bufsize), header_size(_header_size),
        h(_h), inmem(_inmem), infile(_infile), fin(_fin), fout(_fout),
        fstat(_fstat) {
    dictsize = roundUp(mymax(_dictsize, BUFFERS * bufsize),
                       bufsize); // Dictionary size should be divisible by
                                 // bufsize and BUFFERS*bufsize at least
    dict = (char *)BigAlloc(dictsize, LargePageMode);
    for (int i = 0; i < BUFFERS; i++) {
      hashtable[i] = (index *)malloc(
          sizeof(index) * bufsize / inmem.L *
          2); // For every L bytes in the buffer, we need 2 hash table elements
      statbuf[i] = (STAT *)malloc(
          sizeof(STAT) * (MAX_STATS_PER_BLOCK(bufsize, BASE_LEN) + 10));
  ...
  Offset memreq() {
    return dictsize +
           BUFFERS * (sizeof(index) * bufsize / inmem.L * 2 +
                      sizeof(STAT) * MAX_STATS_PER_BLOCK(bufsize, BASE_LEN) +
                      header_size);
  }
...
private: // Background thread code
  void run() {
    Offset pos = 0;
    index buf_offset = 0;
    for (int i = 1, first_block = 1;;
         buf_offset = (buf_offset + bufsize) % dictsize, i = (i + 1) % BUFFERS,
             first_block = 0) // i = 1 0 1 0 1...  first_block = 1 0 0 0...
    {
      // 1. Read input data
      buf[i] = dict + buf_offset;
      len[i] = infile.read(&bufptr[i], buf[i], pos, bufsize, fin); // mmap
      if (!COMPARE_DIGESTS && infile.mmapped()) {
        len[i] = file_read(fin, buf[i], bufsize);
        bufptr[i] = buf[i]; // file_read
      }
      if (filesize - pos < len[i]) {
        fprintf(stderr,
                "\n  ERROR! Input file is larger than filesize specified");
        errcode = ERROR_IO;
        ReadDone.Signal();
        break;
      } // Ensure that we don't read more than `filesize` bytes

      // 2. Perform b/g processing of input data
      if (hash_func) // Save checksum of every input block for error-checking
      ...
      if (!first_block && !no_writes) {
        if (!save_data((i - 1 + BUFFERS) % BUFFERS))
          goto cleanup;
      }
  ...
  *next_filename = NULL;
  ...
  const bool ROUND_MATCHES =
      ...
      (dictsize == 0); // Match lengths are multiplies of L, otherwise -
                       // arbitrary value (>= min_match for -m4/-m5)
  const bool EXHAUSTIVE_SEARCH =
      (method == SREP_METHOD5); // Check all matches starting at L/2 length in
                                // order to find L-byte match
  if (!L && !min_match)
    min_match = (CONTENT_DEFINED_CHUNKING ? 4096 : 512); // Default -l values
  if (!L) {
    if (CONTENT_DEFINED_CHUNKING)
  ...
  if (CONTENT_DEFINED_CHUNKING)
    dictsize = 0; // CDC isn't yet compatible with in-memory compression
  if (vm_mem > size_t(-1))
  ...
  if (filenames[1] == NULL) {
    printf(
        ...
        "   -dhBYTES/-dc/-dl: size of hash / size of hash chunk / minimum "
  ...
  if (cmdmode == COMPRESSION) {
    ...
    size_t total_blocks = 0;
    {
      ...
      DictionaryCompressor inmem(dictsize, dict_hashsize, dict_min_match,
                                 dict_chunk, BASE_LEN, bufsize,
                                 BG_COMPRESSION_THREAD::BUFFERS, LargePageMode);
      BG_COMPRESSION_THREAD bg_thread(
          ROUND_MATCHES, COMPARE_DIGESTS, BASE_LEN, FUTURE_LZ,
      ...
      bg_thread.start();
      if (verbosity > 1) {
        char l_c_t_str[100], l_c_a_str[100], hashname_str[100],
            bufsize_str[100], dictsize_str[100], dict_hashsize_str[100],
            dict_compressor_str[100];
        sprintf(l_c_t_str, " -l%d -c%d -t%d", min_match, L, NumThreads);
        sprintf(l_c_a_str, " -l%d -c%d -a%d/%d", min_match, L, accel,
                ACCELERATOR);
        ...
        sprintf(dict_compressor_str, " -d%s -dh%s -dl%d -dc%d", dictsize_str,
                dict_hashsize_str, dict_min_match, dict_chunk);
        fprintf(
            stderr,
...
#define INDEX_LZ_FOOTER_SIZE (sizeof(STAT) * 6)
      ...
      for (;;) {
        ...
        inmem.compress(bg_thread.dict, buf, len, hashptr, literal_bytes,
                       INMEM_COMPRESSION ? statbuf : static_statbuf, stat);
        if (!INMEM_COMPRESSION) {
          ENCODE_LZ_MATCH(stat, ROUND_MATCHES, BASE_LEN, len + 1,
                          Offset(BASE_LEN), BASE_LEN);
        } // pseudo-match used to limit matches read from the static_statbuf[]
        ...
        if (INMEM_COMPRESSION) {
          ...
          case 0:
            compress<0>(ROUND_MATCHES, L, min_match, BASE_LEN, origsize, h, buf,
                        len, literal_bytes, static_statbuf, statbuf, stat);
            break;
          case 1:
          ...
          case 16:
            compress<16>(ROUND_MATCHES, L, min_match, BASE_LEN, origsize, h,
                         buf, len, literal_bytes, static_statbuf, statbuf,
                         stat);
            break;
          case 32:
        ...
        if (verbosity) {
          ...
          if (origsize != last_origsize &&
            ...
            fprintf(stderr,
                    ...
                    "real %.0lf mb/s (%.3lf sec) = %.0lf%%%s",
                    int(double(origsize) * 100 / filesize),
                    show3(origsize, temp1), show3(compsize, temp2),
                    double(compsize) * 100 / origsize, print_pc ? counters : "",
                    origsize / CPUTime / mb, CPUTime,
                    origsize / GlobalTime / mb, GlobalTime,
                    CPUTime / GlobalTime * 100, newline);
    ...
    if (FUTURE_LZ || INDEX_LZ) {
      *last_block = NULL; // Mark end of linked list
      ...
      STAT *statbuf = new STAT[MAX_STATS_PER_BLOCK(bufsize, BASE_LEN)],
           *stat; // for every BASE_LEN input bytes, we can write up to 4 STAT
                  // values to statbuf
      ...
      LZ_MATCH *lz_matches_by_dest =
          new LZ_MATCH[lz_matches_count + 1]; // +1 is for the loop barrier
      my_memcpy(lz_matches_by_dest, lz_matches,
                lz_matches_count * sizeof(LZ_MATCH));
      ...
      for (int i = 0, j = 0; i < lz_matches_count || j < lz_matches_count;) {
        if (lz_matches[i].src < lz_matches_by_dest[j].dest) {
          if (lz_matches[i].src / bufsize != lz_matches[i].dest / bufsize &&
              lz_matches[i].len < maximum_save)
            ram += MEMORY_MANAGER::needmem(lz_matches[i].len),
                max_ram = mymax(ram, max_ram);
          i++;
        } else {
          if (lz_matches_by_dest[j].src / bufsize !=
                  lz_matches_by_dest[j].dest / bufsize &&
              lz_matches_by_dest[j].len < maximum_save)
            ram -= MEMORY_MANAGER::needmem(lz_matches_by_dest[j].len);
          j++;
        }
      }
      if (verbosity) {
        char temp1[100], maximum_save_str[100];
        showMem(maximum_save, maximum_save_str);
        sprintf(temp1, (maximum_save != unsigned(-1) ? " with -m%s" : ""),
                maximum_save_str);
        printf("\rDecompression memory%s is %d mb", temp1,
               int(max_ram / mb) + 1);
      }
      ...
      // literal data
      file_seek(ftemp, 0);
      Offset processed = 0, total_stat_size = 0;
      i = 0;
      ...
      for (COMPRESSED_BLOCK *block = first_block; block; block = block->next) {
        ...
        for (; lz_matches[i].src < block->end; i++) {
          ...
          ENCODE_LZ_MATCH(stat, false, BASE_LEN, src - block_pos,
        ...
        if (FUTURE_LZ) {
          ...
          if (verbosity) {
            ...
            if (processed == origsize ||
              ...
              LastGlobalTime = GlobalTime;
              fprintf(stderr, "\b\b\b\b\b\b%5.1lf%%",
                      double(processed) * 100 / origsize);
    ...
    if (len != sizeof(STAT) * ARCHIVE_HEADER_SIZE ||
      ...
      error(ERROR_COMPRESSION, "Not an SREP compressed file: %s", finame);
    ...
    // For keyed hashes like VMAC, we should read the key (seed) and create a
    // hash using this key
    len = file_read(fin, header, hash_seed_size);
    if (len != hash_seed_size)
      error(ERROR_COMPRESSION,
            "Decompression problem: unexpected end of file %s or I/O error",
            finame);
    if (selected_hash->new_hash) {
    ...
    if (cmdmode == INFORMATION)
      fprintf(stderr, "%s: -l%d -hash=%s%s",
              FUTURE_LZ  ? "Future-LZ"
              : INDEX_LZ ? "Index-LZ"
                         : "I/O LZ",
              BASE_LEN, selected_hash->hash_name, INDEX_LZ ? "" : "\n");
    ...
    if (INDEX_LZ) {
      ...
      if (header[5] != ~BULAT_ZIGANSHIN_SIGNATURE ||
          header[4] != ~SREP_SIGNATURE)
        error(ERROR_COMPRESSION,
              "Not found SREP compressed file footer in file %s", finame);
      if (footer_version != SREP_FOOTER_VERSION1)
        error(ERROR_COMPRESSION,
              "Incompatible compressed file footer format: v%d (%s supports "
              "only v%d) in file %s",
              footer_version, program_version, SREP_FOOTER_VERSION1, finame);
      if (compsize > filesize)
        error(ERROR_COMPRESSION,
              "Broken SREP compressed file footer: %0.lf bytes footer + %0.lf "
              "bytes index in file %s",
              double(footer_size), double(stat_size), finame);
      ...
      unsigned total_blocks =
          (footer_size - INDEX_LZ_FOOTER_SIZE) / sizeof(STAT);
      ...
      if (cmdmode == INFORMATION) {
        ...
        origsize = filesize - footer_size - stat_size -
                   full_archive_header_size -
        ...
        fprintf(stderr, ".  %s -> %s: %.2lf%%\n", show3(filesize, temp1),
                show3(origsize, temp2), double(filesize) * 100 / origsize);
        goto cleanup;
      }
    }
    ...
    for (bool finished = false; !finished;) {
      ...
      {
        unsigned compsize1 = header[0] + header[2], origsize1 = header[1],
                 statsize1 = header[2];
        ...
        if (cmdmode == INFORMATION) {
          // We scan through the compressed file only in order to compute the
        ...
        if (len != statsize1)
          error(ERROR_COMPRESSION,
                "Decompression problem: unexpected end of file %s or I/O error",
                *index_file ? index_file : finame);
        len = file_read(fin, buf + statsize1, compsize1 - statsize1);
        if (len != compsize1 - statsize1)
          error(ERROR_COMPRESSION,
                "Decompression problem: unexpected end of file %s or I/O error",
                finame);
        ...
        bool ok = IO_LZ ? decompress(ROUND_MATCHES, BASE_LEN, ftemp, origsize,
                                     statptr, buf + statsize1, buf + compsize1,
                                     out, out + origsize1)
                        : decompress_FUTURE_LZ(ROUND_MATCHES, BASE_LEN, ftemp,
                                               origsize, statptr, statendptr,
    ...
    print_decompression_stats:
      if (verbosity) {
        double GlobalTime = GetGlobalTime() - GlobalTime0;
        if (origsize != last_origsize &&
            (finished || GlobalTime - LastGlobalTime > TimeInterval)) {
          LastGlobalTime = GlobalTime;
          last_origsize = origsize;
          double CPUTime = GetCPUTime();
          char stats[1000], total_reads_str[100], vm_stats_str[100], temp1[100],
              temp2[100];
          sprintf(total_reads_str,
                  maximum_save != unsigned(-1) ? ", I/Os %.0lf" : "",
                  double(total_reads));
          sprintf(vm_stats_str, vm.total_write ? ", VM %d/%d, R/W %d/%d" : "",
                  int(vm.current_mem() / mb), int(vm.max_mem() / mb),
                  int(vm.total_read / mb), int(vm.total_write / mb));
          sprintf(stats,
                  (IO_LZ || !print_pc
                       ? ""
                       : ".  Matches %.0lf %.0lf %.0lf%s, RAM %d/%d%s"),
                  double(cur_matches), double(max_matches),
                  double(total_matches), total_reads_str,
                  int(mm.current_mem() / mb), int(mm.max_mem() / mb),
                  vm_stats_str);
          fprintf(stderr,
                  "\r%d%%: %s -> %s: %.2lf%%.  Cpu %.0lf mb/s (%.3lf sec), "
                  "real %.0lf mb/s (%.3lf sec) = %.0lf%%%s%s",
                  int(double(compsize) * 100 / filesize),
                  show3(compsize, temp1), show3(origsize, temp2),
                  double(compsize) * 100 / origsize, origsize / CPUTime / mb,
                  CPUTime, origsize / GlobalTime / mb, GlobalTime,
                  CPUTime / GlobalTime * 100, stats, newline);
