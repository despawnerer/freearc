...
int dict_compress(MemSize BlockSize, int MinCompression, int MinWeakChars,
  ...
  while ((x = callback("read", (In = (BYTE *)BigAlloc(BlockSize)), BlockSize,
    ...
    if (x || MinCompression > 0 &&
      ...
      BigFreeAndNil(In);
      // Записать сжатый блок и выйти, если при записи произошла ошибка/больше
      // данных не нужно
...
int dict_decompress(MemSize BlockSize, int MinCompression, int MinWeakChars,
  ...
  for (;;) {
    ...
    if (InSize < 0) {
      ...
      // x = DictDecode (InSize, callback, auxdata);   // для работы в
      // фиксированном объёме памяти
      if (x)
        break;
      BigFreeAndNil(In);
...
finished:
  BigFreeAndNil(In);
  BigFreeAndNil(Out);
  return x <= 0 ? x : FREEARC_ERRCODE_GENERAL; // 0, если всё в порядке, и код
                                               // ошибки иначе
}
...
int DICT_METHOD::decompress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  if (!f)
    f = (FARPROC)dict_decompress;

  return ((int (*)(MemSize, int, int, int, int, int, int, CALLBACK_FUNC *,
                   void *))f)(BlockSize, MinCompression, MinWeakChars,
...
// параметры (функция, обратная к parse_DICT)
void DICT_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  DICT_METHOD defaults;
  char BlockSizeStr[100], MinCompressionStr[100], MinWeakCharsStr[100];
  char MinLargeCntStr[100], MinMediumCntStr[100], MinSmallCntStr[100],
      MinRatioStr[100];
  showMem(BlockSize, BlockSizeStr);
  sprintf(MinCompressionStr,
          MinCompression != defaults.MinCompression ? ":%d%%" : "",
          MinCompression);
  sprintf(MinWeakCharsStr, MinWeakChars != defaults.MinWeakChars ? ":c%d" : "",
          MinWeakChars);
  sprintf(MinLargeCntStr, MinLargeCnt != defaults.MinLargeCnt ? ":l%d" : "",
          MinLargeCnt);
  sprintf(MinMediumCntStr, MinMediumCnt != defaults.MinMediumCnt ? ":m%d" : "",
          MinMediumCnt);
  sprintf(MinSmallCntStr, MinSmallCnt != defaults.MinSmallCnt ? ":s%d" : "",
          MinSmallCnt);
  sprintf(MinRatioStr, MinRatio != defaults.MinRatio ? ":r%d" : "", MinRatio);
  sprintf(buf, "dict:%s%s%s%s%s%s%s", BlockSizeStr, MinCompressionStr,
          MinWeakCharsStr, MinLargeCntStr, MinMediumCntStr, MinSmallCntStr,
          MinRatioStr);
}
...
COMPRESSION_METHOD *parse_DICT(char **parameters) {
  if (strcmp(parameters[0], "dict") == 0) {
    // Если название метода (нулевой параметр) - "dict", то разберём остальные
    // параметры
    ...
    while (*++parameters && !error) {
      ...
      if (strlen(param) == 1)
        ...
        case 'b':
          ...
          continue;
        case 'c':
          p->MinWeakChars = parseInt(param + 1, &error);
          continue;
        case 'l':
          p->MinLargeCnt = parseInt(param + 1, &error);
          continue;
        case 'm':
          p->MinMediumCnt = parseInt(param + 1, &error);
          continue;
        case 's':
          p->MinSmallCnt = parseInt(param + 1, &error);
          continue;
        case 'r':
          p->MinRatio = parseInt(param + 1, &error);
          continue;
        }
