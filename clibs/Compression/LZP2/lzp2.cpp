/*
   ...
   - LZP2 source repository: http://freearc.org:8002/freearc/file - go to
   Compression/LZP2

   Compilation defines:
   LZPE             if enabled, use chars 128..255 to encode match lengths; may
   be used only for English texts HASHBYTES=2..4   number of bytes used to find
   match MINLEN           minimum match length HASHLOG          logarithm of
   hash size (in elements, each element is a char*) MAX_OFFSET       maximum
   match offset (in order to improve caching)
*/
...
// #define LZPE                    /* Use chars 128..255 to encode match
// lengths; may be used only for English texts */
...
#ifndef MAX_OFFSET
#define MAX_OFFSET                                                             \
  65536 /* maximum match offset (in order to improve caching) */
#endif

// Calculate HASHLOG-bits wide hash of HASHBYTES bytes preceding p
#define calchash(p)                                                            \
  (((*(unsigned *)(p - HASHBYTES) & ((1 << (HASHBYTES * 8)) - 1)) *            \
    123456791) >>                                                              \
   (32 - HASHLOG))
...
int compress(char *inbuf, int insize, char *outbuf) {
  ...
  char *outptr = outbuf, *ptr = inbuf + HASHBYTES, *last_ptr = inbuf,
       *bufend = inbuf + insize;
  ...
  while (ptr < bufend) {
    ...
    if (len > MINLEN || ptr + len >= bufend) {
      ...
      int litlen = ptr - last_ptr;
      int orig_len = len, orig_litlen = litlen;
  ...
  if ((n) >= MAX) {                                                            \
    ...
    *outptr++ = (n);                                                           \
      ...
      *outptr++ = LITTLE(litlen, 15) * 16 + LITTLE(len, 15);
      BIG(litlen, 15);
      BIG(len, 15);
      memcpy(outptr, last_ptr, orig_litlen);
      outptr += orig_litlen;
#else
      memcpy(outptr, last_ptr, orig_litlen);
      outptr += orig_litlen;
