...
bool compressorIsEncrypted_Guess(char *compressor) {
  ...
  return (strstr(c, "+aes-") != NULL || strstr(c, "+serpent-") != NULL ||
...
// Реализация стандартного интерфейса методов сжатия/шифрования
// COMPRESSION_METHOD
...
public:
  ...
  int rounds; // Количество шагов при шифровании, чем больше - тем медленней, но
              // надёжней. 0 - использовать знаечние по умолчанию,
              // рекомендованное создателями алгоритма
  ...
  // Конструктор, присваивающий параметрам метода шифрования значения по
  // умолчанию
  ENCRYPTION_METHOD();

  // Универсальный метод, отвечает на запросы "encryption?", "KeySize" и
  // "IVSize"
