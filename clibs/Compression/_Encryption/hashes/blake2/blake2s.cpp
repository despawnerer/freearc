...
static inline int
blake2s_param_set_salt(blake2s_param *P,
...
static inline int
blake2s_param_set_personal(blake2s_param *P,
...
int blake2s_init(blake2s_state *S, const uint8_t outlen) {
  ...
  const blake2s_param P = {outlen, 0, 1, 1, 0, {0}, 0, 0, {0}, {0}};
...
int blake2s_init_key(blake2s_state *S, const uint8_t outlen, const void *key,
  ...
  const blake2s_param P = {outlen, keylen, 1, 1, 0, {0}, 0, 0, {0}, {0}};
...
static inline int blake2s_compress(blake2s_state *S,
  ...
  row4 = _mm_xor_si128(
      _mm_setr_epi32(0x510E527F, 0x9B05688C, 0x1F83D9AB, 0x5BE0CD19),
      LOAD(&S->t[0]));
  ROUND(0);
  ROUND(1);
  ROUND(2);
...
int blake2s_update(blake2s_state *S, const uint8_t *in, uint64_t inlen) {
  while (inlen > 0) {
    ...
    if (inlen > fill) {
      ...
      memcpy(S->buf, S->buf + BLAKE2S_BLOCKBYTES,
             BLAKE2S_BLOCKBYTES); // Shift buffer left
...
int blake2s_final(blake2s_state *S, uint8_t *out, uint8_t outlen) {
  ...
  memset(S->buf + S->buflen, 0,
         2 * BLAKE2S_BLOCKBYTES - S->buflen); /* Padding */
  blake2s_compress(S, S->buf);
