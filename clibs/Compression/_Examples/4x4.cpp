...
Semaphore IO; // Prevents simultaneous execution of I/O operations from
              // different threads
...
int callback_func(const char *what, void *data, int size, void *param) {
  ...
  if (strequ(what, "read") && ENCODE) {
    IO.Lock(); // Lock guarantees that jobs will be queued to WriterJobs in the
               // same order as blocks are read from input stream
    ...
    job->Saved
        .Wait(); // pause execution until (de)compressed data will be written
    return size;
  }
  return 0;
}
...
static DWORD WINAPI DeCompressionThread(void *paramPtr) {
  ...
  try {
    for (;;) {
      if (ENCODE) {
        if (Compress(method, callback_func, job) < 0)
          break;
      } else {
        IO.Lock();
...
void WriterThread(int NumThreads) {
  try {
    for (;;) {
      ...
      IO.Lock();
      if (ENCODE) {
        // Save compressed block header: original/compressed blocksize and
        // compression method
        write4(job->insize);
...
void DeCompressionMachine() {
  ...
  for (int i = 0; i < NumThreads; i++) {
    CThread t;
    t.Create(DeCompressionThread, &job[i]);
  }
