...
int COMPRESSION_METHOD::server() {
  ...
  // Global services
  if (strequ(service, "register"))
    return FREEARC_OK; // to do: Register();

  // Invocation-specific services
  if (strequ(service, "decompress")) {
    parse_method();
    return decompress(p._callback("callback"), p);
  }
#ifndef FREEARC_DECOMPRESS_ONLY
  if (strequ(service, "compress")) {
    parse_method();
    return compress(p._callback("callback"), p);
  }
  if (strequ(service, "canonize")) {
    parse_method();
    char a[1000];
    ShowCompressionMethod(a, FALSE);
    return p._return(a);
  }
  if (strequ(service, "SetCompressionMem")) {
    parse_method();
    SetCompressionMem(p._int("mem"));
    char a[1000];
    ShowCompressionMethod(a, FALSE);
    return p._return(a);
  }
  if (strequ(service, "SetDictionary")) {
    parse_method();
    SetDictionary(p._int("mem"));
    char a[1000];
    ShowCompressionMethod(a, FALSE);
    return p._return(a);
  }
  if (strequ(service, "SetBlockSize")) {
    parse_method();
    SetBlockSize(p._int("mem"));
    char a[1000];
    ShowCompressionMethod(a, FALSE);
    return p._return(a);
  }
  if (strequ(service, "SetDecompressionMem")) {
    parse_method();
    SetDecompressionMem(p._int("mem"));
    char a[1000];
    ShowCompressionMethod(a, FALSE);
    return p._return(a);
  }
  if (strequ(service, "GetCompressionMem")) {
    parse_method();
    return p._return(GetCompressionMem());
  }
  if (strequ(service, "GetDictionary")) {
    parse_method();
    return p._return(GetDictionary());
  }
  if (strequ(service, "GetBlockSize")) {
    parse_method();
    return p._return(GetBlockSize());
  }
#endif
  if (strequ(service, "GetDecompressionMem")) {
    parse_method();
    return p._return(GetDecompressionMem());
  }
...
struct STORING_METHOD : COMPRESSION_METHOD {
  ...
  // (функция, обратная к parse_method)
...
// ****************************************************************************************************************************
// ПОДДЕРЖКА ТАБЛИЦЫ ЗАРЕГИСТРИРОВАННЫХ МЕТОДОВ СЖАТИЯ И ПОИСК В ЭТОЙ ТАБЛИЦЕ
// РЕАЛИЗАЦИИ ЧИСТО КОНКРЕТНОГО МЕТОДА *************
// ****************************************************************************************************************************
...
int CELS_Register(TABI_FUNCTION *method) {
  CHECK(FREEARC_ERRCODE_INTERNAL, methodsCount < elements(methodsTable),
...
int CELS_Call(TABI_ELEMENT *params) {
  ...
  if (start_with(service, "Limit") && isupper(service[5]))
    return p._return(p._str("method")); // to do: get & set
                                        /*
                                          {
                                            char new_service[MAX_COMPRESSOR_STRLEN];
                                            sprintf(new_service, "Get%s", service+5);
                                            MemSize mem = TABI_callret(CELS_Call, TABI_DYNAMAP(p) ("service",
                                          new_service));                                       if (mem <=
                                          p._longlong("mem"))         // if method already                                       uses less memory than
                                          specified limit                                       return
                                          p._return(p._str("method"));                                       //                                       then
                                          return method unmodified                                       sprintf(new_service, "Set%s", service+5);                                       return
                                          TABI_call(CELS_Call, TABI_DYNAMAP(p) ("service", new_service));
                                          }
                                        */
  if (strequ(service, "encryption?"))
    return 1; // to do: aes-specific
  if (start_with(p._str("method", ""), "aes"))
    return p._return(p._str("method")); // to do: aes-specific
