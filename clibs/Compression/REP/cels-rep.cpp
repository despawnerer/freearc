...
struct REP_METHOD : COMPRESSION_METHOD {
  ...
  MemSize BlockSize; // Размер буфера. Совпадения ищутся только в пределах этой
                     // дистанции. Расход памяти - BlockSize+BlockSize/4
  int MinCompression; // Минимальный процент сжатия. Если выходные данные
                      // больше, то вместо них будут записаны оригинальные
                      // (несжатые) данные
  int ChunkSize; // Размер блоков, КС которых заносится в хеш
  int MinMatchLen; // Минимальная длина строки, при которой она будет заменяться
                   // ссылкой на предыдущее вхождение
  int Barrier; // Граница, после которой допускается использовать совпадения
               // меньшего размера (поскольку lzma/ppmd всё равно пропустит их)
  int SmallestLen; // Этот меньший размер
  int HashSizeLog; // Логарифм размера хеша (в 4-байтовых словах). Большие
                   // значения увеличивают сжатие, но замедляют его. При нулевом
                   // значении оптимальный размер вычисляется автоматически
  ...
  virtual int decompress(CALLBACK_FUNC *callback, void *auxdata) {
    return rep_decompress(BlockSize, MinCompression, ChunkSize, MinMatchLen,
                          Barrier, SmallestLen, HashSizeLog, Amplifier,
                          callback, auxdata);
  }
  ...
  virtual int compress(CALLBACK_FUNC *callback, void *auxdata) {
    return rep_compress(BlockSize, MinCompression, ChunkSize, MinMatchLen,
                        Barrier, SmallestLen, HashSizeLog, Amplifier, callback,
                        auxdata);
  }
  ...
  virtual void parse_method() {
    ...
    split(local_method, COMPRESSION_METHOD_PARAMETERS_DELIMITER, parameters,
          MAX_PARAMETERS);
    if (!strequ(parameters[0], "rep"))
      throw "REP_METHOD:parse_method";

    // Если название метода (нулевой параметр) - "rep", то разберём остальные
    // параметры
    ...
    while (*++parameters && !error) {
      ...
      case 'b':
        BlockSize = parseMem(param + 1, &error);
        continue;
      case 'l':
        MinMatchLen = parseInt(param + 1, &error);
        continue;
      case 'c':
        ChunkSize = parseInt(param + 1, &error);
        continue;
      case 'd':
        Barrier = parseMem(param + 1, &error);
        continue;
      case 's':
        SmallestLen = parseInt(param + 1, &error);
        continue;
      case 'h':
        HashSizeLog = parseInt(param + 1, &error);
        continue;
      case 'a':
        Amplifier = parseInt(param + 1, &error);
        continue;
      }
      ...
      if (!error)
        ...
        error = 0, BlockSize = parseMem(param, &error);
    }
    if (error)
      throw "rep:parse_method"; // Ошибка при парсинге параметров метода
  }

  // Записать в buf[MAX_METHOD_STRLEN] строку, описывающую метод сжатия и его
  // параметры (функция, обратная к parse_method)
  virtual void ShowCompressionMethod(char *buf, bool purify) {
    REP_METHOD defaults(NULL);
    char BlockSizeStr[100], MinCompressionStr[100], BarrierTempStr[100],
        BarrierStr[100], SmallestLenStr[100], HashSizeLogStr[100],
        AmplifierStr[100], ChunkSizeStr[100], MinMatchLenStr[100];
    ...
    sprintf(MinCompressionStr,
            ...
            MinCompression);
    sprintf(MinMatchLenStr, MinMatchLen != defaults.MinMatchLen ? ":%d" : "",
            MinMatchLen);
    sprintf(ChunkSizeStr, ChunkSize != defaults.ChunkSize ? ":c%d" : "",
            ChunkSize);
    sprintf(BarrierStr, Barrier != defaults.Barrier ? ":d%s" : "",
            BarrierTempStr);
    sprintf(SmallestLenStr, SmallestLen != defaults.SmallestLen ? ":s%d" : "",
            SmallestLen);
    sprintf(AmplifierStr, Amplifier != defaults.Amplifier ? ":a%d" : "",
            Amplifier);
    sprintf(HashSizeLogStr, HashSizeLog != defaults.HashSizeLog ? ":h%d" : "",
            HashSizeLog);
    sprintf(buf, "rep:%s%s%s%s%s%s%s%s", BlockSizeStr, MinCompressionStr,
            MinMatchLenStr, ChunkSizeStr, BarrierStr, SmallestLenStr,
            HashSizeLogStr, AmplifierStr);
  }

  // Настроить метод сжатия на использование заданного объёма памяти
  virtual void SetCompressionMem(MemSize mem) {
    int L; // Размер блоков, КС которых заносится в хеш
    int HashSize = CalcHashSize(HashSizeLog, mem / 5 * 4, SmallestLen,
                                MinMatchLen, ChunkSize, Amplifier, &L);
  ...
  virtual MemSize GetCompressionMem() {
    int L; // Размер блоков, КС которых заносится в хеш
    int HashSize = CalcHashSize(HashSizeLog, BlockSize, SmallestLen,
                                MinMatchLen, ChunkSize, Amplifier, &L);
...
int rep_server(TABI_ELEMENT *params) { return REP_METHOD(params).server(); }
