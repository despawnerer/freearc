...
** REP со словарём больше размера
ОЗУ**************************************************************
...
/*
    Для нахождения совпадений длиной от MinLen байт нужно заносить в хеш
   значения контрольной функции от блоков длиной L = MinLen/2 байт с частотой k
   = sqrt(L) байт. Искать в этой хеш-таблице совпадения для блоков, начинающихся
   в первых test=k байтах из каждого блока длиной L байт.
*/
...
MemSize CalcHashSize(MemSize HashBits, MemSize BlockSize, int SmallestLen,
  ...
  *L = ChunkSize ? ChunkSize : rounddown_to_power_of(Len - 1, 2) / 2;
  // Размер хеша должен соответствовать 4*количество значений. которые мы хотим
  // в него занести, но не превышать четверти от размера буфера / объёма входных
  // данных (Size/16*sizeof(int)==Size/4)
  return HashBits > 0
             ? (1 << HashBits)
             : roundup_to_power_of(BlockSize / 3 * 2, 2) / mymax(*L / 4, 16);
}
...
struct Job {
  int *
      hashtable; // Place for saving info about maximum hashes and their indeces
  byte *buf;     // Buffer being hashed
  int bytes;     // How much bytes to hash in buf[] (plus it needs a L-byte
                 // lookahead bytes after that)
  int L; // Size of rolling hash AND number of positions among those we are
  ...
  void process() {
    ...
    if (num_blocks > 0) {
      int hash = 0;
      for (int i = 0; i < L; i++)
        update_hash(0, buf[i]); // Initial hash value == hash of first L bytes
                                // of the buffer

      // Split buffer into L-byte blocks and store maximal hash for each block
      // and its position to hashptr[]
...
int rep_compress(unsigned BlockSize, int MinCompression, int ChunkSize,
                 int MinMatchLen, int Barrier, int SmallestLen, int HashBits,
                 int Amplifier, CALLBACK_FUNC *callback, void *auxdata) {
  // НАСТРОЙКА ПАРАМЕТРОВ АЛГОРИТМА
  if (SmallestLen > MinMatchLen)
    SmallestLen = MinMatchLen;
  int L; // Размер блоков, КС которых заносится в хеш
  int HashSize = CalcHashSize(HashBits, BlockSize, SmallestLen, MinMatchLen,
                              ChunkSize, Amplifier, &L);
  int HashMask = HashSize - 1;
  int *hasharr = NULL;
  int errcode = FREEARC_OK;
  int DataEnd = 0, last_i = 0,
      last_match = 0; // last_match points to the end of last match written, we
                      // shouldn't start new match before it

  byte *buf = (byte *)BigAlloc(
      BlockSize); // Буфер, куда будут помещаться входные данные
  if (buf == NULL)
    return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY; // Error: not enough memory

  // Alloc hash array
  hasharr = (int *)BigAlloc(HashSize * sizeof(int));
  if (HashSize && hasharr == NULL) {
    BigFree(buf);
    return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;
  } // Error: not enough memory
  memset(hasharr, 0, HashSize * sizeof(int));
  debug(verbose > 0 && MinMatchLen == SmallestLen &&
        printf(" Buf %d mb, MinLen %d, Hash %d mb, Amplifier %d\n",
               ((BlockSize - 1) / mb) + 1, MinMatchLen,
               (HashSize * sizeof(int)) >> 20, Amplifier));
  debug(verbose > 0 && MinMatchLen != SmallestLen &&
        printf(" Buf %d mb, MinLen %d, Barrier %d, Smallest Len %d, Hash %d "
               "mb, Amplifier %d\n",
               ((BlockSize - 1) / mb) + 1, MinMatchLen, Barrier, SmallestLen,
               (HashSize * sizeof(int)) >> 20, Amplifier));

  FOPEN();

  // Буфера для отдельного хранения длин, смещений совпадений, длин несжатых
  // блоков и самих этих блоков. Такое группирование позволяет увеличить
  // конечную степень сжатия
  int bsize = (mymin(BlockSize, MAX_BLOCK) / SmallestLen + 1) *
              sizeof(int32); // Макс. объём данных, который может быть записан в
                             // любой из этих буферов
  Buffer lens(bsize), offsets(bsize), datalens(bsize), dataOffsets(bsize);

  // Приготовления к хешированию в фоновых потоках
  const int HTJOBS =
                GetCompressionThreads(), // количество одновременно выполняемых
                                         // заданий по хешированию
      HTBUFS = mymax(
          16,
          HTJOBS *
              2), // количество буферов для сохранения результатов хеширования
                  // (минимум 16 для более равномерного распределения нагрузки)
      HTBLOCK = mymax(256 * kb, L * 4) / L *
                L, // объём данных, обрабатываемых одним заданием хеширования
      HTCOUNT =
          HTBLOCK / L *
          2; // кол-во чисел, запоминаемых одним заданием хеширования (на каждые
             // L байт запоминается наибольшее значение хеша и его индекс)
  int free_jobs =
      HTBUFS; // количество свободных буферов для результатов хеширования
  int ht_index =
      0; // индекс очередного свободного буфера для результатов хеширования
  int next_job_i = 0;
  int next_flush = 0;
  int *ht = (int *)BigAlloc(HTBUFS * HTCOUNT * sizeof(*ht));
  MultipleProcessingThreads<Job> HashingThreads;
  HashingThreads.MaxJobs = HTBUFS;
  HashingThreads.NumThreads = HTJOBS;
  if (ht == NULL || HashingThreads.start() != 0) {
  ...
  {
    bool DoItOnce = true; // Для однократной записи BlockSize в выходной буфер
                          // ПОСЛЕ чтения первых входных данных
    bool MoreInputData = true; // Продолжать чтение входных данных
    bool MoreHashedData =
        true; // Продолжать получение хешированных блоков и их сжатие

    while (MoreHashedData) {
      int literals = 0;
      lens.empty(), offsets.empty(), datalens.empty(),
          dataOffsets.empty(); // Очистим буфера
#ifdef DEBUG
      int match_cnt = 0, matches = 0;
#endif

      // Накапливаем сжатые данные, пока не обработаем очередные 8 мб входных
      // данных
      next_flush += mymin(BlockSize - next_flush, MAX_BLOCK);
      while (last_i + L < next_flush) { // Добавляем L байт потому, что первый
                                        // хешируемый блок на L байт меньше
        ...
        while (free_jobs > 0 && DataEnd < BlockSize && MoreInputData) {
          ...
          int Size;
          READ_LEN(Size, buf + DataEnd, mymin(BlockSize - DataEnd, HTBLOCK));
          if (Size < 0) {
            errcode = Size;
            goto finished;
          } // Error: can't read input data
          if (DoItOnce) {
            FWRITE4(BlockSize);
            DoItOnce = false;
          } // Запишем размер словаря в выходной поток
          if (Size == 0)
            MoreInputData = false;
          DataEnd += Size; // Граница прочитанных данных

          // Отправляем прочитанные данные на хеширование (первый хешируемый
          // блок на L байт меньше)
          int bytes = DataEnd - (next_job_i + L);
          if (bytes <= 0) {
            if (Size > 0)
              continue;
            else
              bytes = 0 /* no MoreHashedData signal */;
          }
          Job job = {ht + ht_index * HTCOUNT, buf + next_job_i, bytes,
                     L}; // last byte accessed: buf[i+bytes+L-1] == buf[i+
                         // DataEnd-(i+L) +L-1] == buf[DataEnd-1]
          HashingThreads.Put(job);
          debug(verbose > 0 &&
                printf(" Read (%x,%x),  Job[%d] (%x,%x),  free_jobs %d\n",
                       DataEnd - Size, Size, ht_index, next_job_i, bytes,
                       free_jobs));
          next_job_i += bytes;
        ...
        if (job.bytes == 0) {
          MoreHashedData = false;
          goto encode_data;
        }

        // ОСНОВНОЙ ЦИКЛ, НАХОДЯЩИЙ ПОВТОРЯЮЩИЕСЯ СТРОКИ ВО ВХОДНЫХ ДАННЫХ
        for (; last_i < next_fill; last_i += L) {
          // Проверяем совпадение в лучшем на следующие L байт блоке тоже длины
          // L
          int hash = *hashptr++; // Считываем хеш и индекс этого блока, уже
                                 // найденные одним из наших фоновых потоков
          int i = *hashptr++ + last_i;
          if (i >= last_match) { // Проверяем совпадение только если предыдущее
                                 // найденное совпадение уже кончилось
            int match = hasharr[hash & HashMask];
            if (match) {
              if (match >= i && match < DataEnd)
                goto no_match; // match попадает на ещё не обработанные данные,
                               // то есть он заведомо устарел
              // Наименьшее/наибольшее значение, которое может принимать при
              // поиске индекс базирующийся на i, чтобы индекс базирующийся на
              // match, не вышел за пределы буфера и не заглянул в будущие
              // данные
              int LowBound = match < i             ? i - match
                             : match - DataEnd > i ? 0
                                                   : i - (match - DataEnd);
              int HighBound = BlockSize - match + i;
              // Найдём реальные начало и конец совпадения, сравнивая вперёд и
              // назад от buf[i] <=> buf[match] i ограничено снизу и сверху
              // значениями last_match и DataEnd, соответственно
              int start = find_match_start(buf + match, buf + i,
                                           buf + mymax(last_match, LowBound)) -
                          buf;
              int end = find_match_end(buf + match, buf + i,
                                       buf + mymin(DataEnd, HighBound)) -
                        buf;
              // start и end - границы совпадения вокруг i. Проверим, что
              // найденное совпадение имеет длину >=MinMatchLen (или
              // SmallestLen, если дистанция >Barrier)
              if (end - start >=
                  (i - match < Barrier ? MinMatchLen : SmallestLen)) {
                int offset = i - match;
                if (offset < 0)
                  offset += BlockSize;
                // Совпадение найдено! Запишем информацию о нём в выходные
                // буфера
                ...
                // Запомнить позицию конца найденного совпадения и вывести
                // отладочную статистику
                debug((match_cnt++, matches += end - start));
                debug(verbose > 1 &&
                      printf("Match %d %d %d  (lit %d)\n", -offset, start,
                             end - start, start - last_match));
    ...
    encode_data:
      ...
      if (next_flush == BlockSize || !MoreHashedData)
        last_i =
            DataEnd; // Закодировать все оставшиеся / до конца буфера данные
      if (last_match > last_i) { // Если последний матч кончается в ещё не
                                 // проиндексированной области
        datalens.put32(
            0); //   Ничего кодировать не надо, но datalens должен всё равно
                //   быть ровно на одну запись длиннее lens/offsets
      } else {
        // Записать в выходные буфера остаток данных от последнего найденного
        // совпадения до последней проиндексированной позиции
      ...
      if (next_flush ==
          BlockSize) { // Если происходит переход через границу буфера
        DataEnd = last_match = last_i = next_job_i = next_flush =
            0; //   Да! Начать заполнять буфер с начала!
      }
      // Записать размер сжатых данных и количество найденных совпадений в буфер
      int outsize = sizeof(int32) * 2 + lens.len() + offsets.len() +
                    datalens.len() + literals;
      QUASIWRITE(outsize);
      ...
      debug(verbose > 0 &&
            printf(" Compressed %d -> %d (%d + %d), %d bytes in %d matches, "
                   "free_jobs %d\n",
                   literals + matches, outsize, outsize - literals, literals,
                   matches, match_cnt, free_jobs));
    }
  }

  // Записать финальный блок, содержащий несжавшийся остаток данных, и 0 -
...
finished:
  ...
  BigFree(hasharr);
  if (errcode >=
      0) { // Only if we are sure that b/g threads are finished all their jobs
...
int rep_decompress(unsigned BlockSize, int MinCompression, int ChunkSize,
                   int MinMatchLen, int Barrier, int SmallestLen, int HashBits,
                   int Amplifier, CALLBACK_FUNC *callback, void *auxdata) {
  int errcode;
  byte *buf0 = NULL;
  MemSize bufsize, ComprSize;
  // Память для словаря может выделяться несколькими кусками чтобы не страдать
  // от фрагментации адресного пространства
  ...
  // Цикл, выделяющий память для словаря отдельными кусками как можно большего
  // размера
  {
    MemSize cumulative_size[MAX_BLOCKS + 1] = {
        0}; // Суммарный размер буферов, предшествующих i-му
    MemSize remaining_size =
        BlockSize; // Сколько ещё памяти нужно выделить для словаря
    while (remaining_size > 0) {
      if (total_blocks >= MAX_BLOCKS)
        ReturnErrorCode(FREEARC_ERRCODE_NOT_ENOUGH_MEMORY);
      MemSize data_size = remaining_size;
      for (;;) {
        if (NULL != (datap[total_blocks] = (byte *)BigAlloc(data_size)))
          break;
        if (data_size <= 1 * mb)
          ReturnErrorCode(FREEARC_ERRCODE_NOT_ENOUGH_MEMORY);
        data_size -= 1 * mb;
      ...
      total_blocks++;
      cumulative_size[total_blocks] =
          cumulative_size[total_blocks - 1] + data_size;
    }
    ...
    byte *end = endp[0]; // Конец текущего буфера     (start <= last_data <=
                         // data <= end)
    for (;;) {
      // Прочитаем один блок сжатых данных
      READ4(ComprSize);
      if (ComprSize == 0)
        break; // EOF flag (see above)
      ...
      // Заголовок блока содержит размер таблиц lens/offsets/datalens; затем
      // идут сами эти таблицы и наконец несжавшиеся данные
      int num = *(int32 *)buf;
      buf += sizeof(int32); // Количество совпадений (= количеству записей в
                            // таблицах lens/offsets/datalens)
      int32 *lens = (int32 *)buf;
      buf += num * sizeof(int32);
      int32 *offsets = (int32 *)buf;
      buf += num * sizeof(int32);
      int32 *datalens = (int32 *)buf;
      buf +=
          (num + 1) * sizeof(int32); // Точнее, datalens содержит num+1 записей

      // Каждая итерация этого цикла копирует одну строку несжатых данных и один
      // match, которые interleaved в нашей реализации процесса упаковки
      for (int i = 0;; i++) {
        int len = datalens[i];
        // Пока копируемая строка пересекает границу буфера
        while (end - data < len) { // printf("  str: %d %d %d %d\n", BlockSize,
                                   // data-datap[0], end-data, len);
        ...
        if (i == num)
          break; // В самом конце у нас ещё один блок несжавшихся данных
                 // (возможно, нулевой длины) без парного ему lz-матча

        int offset = offsets[i];
        len = lens[i];
        debug(verbose > 1 &&
              printf("Match %d %d %d\n", -offset,
                     data - start + cumulative_size[current_block], len));
        // Пока одна из копируемых строк пересекает границу буфера
        while ((offset > data - start && len) || end - data < len) {
          MemSize dataPos =
              data - start +
              cumulative_size[current_block]; // Absolute position of LZ dest
          MemSize fromPos = dataPos - offset;
          int k; // Absolute position of LZ src
          if (offset <= dataPos) {
            // Копируемая строка имеет меньшее смещение, чем текущая позиция
            for (k = current_block; fromPos < cumulative_size[k]; k--)
              ; // ищем блок, которому принадлежит копируемая строка
          } else {
          ...
          byte *from = fromPos - cumulative_size[k] +
                       datap[k];   // Memory address of LZ src
          byte *fromEnd = endp[k]; // End of membuf containing LZ src

          int bytes = mymin(
              len, mymin(end - data,
                         fromEnd - from)); // How much bytes we can copy without
                                           // overrunning src or dest buffers
          // printf("? %d-%d=%d %d(%d %d)\n", dataPos, offset, fromPos, bytes,
          // end-data, fromEnd-from);

          // Копируем влезающий кусочек
          memcpy_lz_match(data, from, bytes);
          data += bytes;
          len -= bytes;

          // Если dest буфер кончился - записываем заполненный буфер, и
        ...
        // Копирование без пересечения границы буфера
        memcpy_lz_match(data, data - offset, len);
        data += len;
      }

      // Вывод распакованных данных, печать отладочной статистики и подготовка к
      // следующей итерации цикла
      WRITE(last_data, data - last_data);
      debug(verbose > 0 && printf(" Decompressed: %u => %u bytes\n",
                                  ComprSize + sizeof(int32), data - last_data));
      last_data = data;
      // NB! check that buf==buf0+Size, data==data0+UncomprSize, and add buffer
      // overflowing checks inside cycle
    }
    errcode = FREEARC_OK;
  }
finished:
...
4. Проверить, что блок плохо упаковался, и заменить его одним литералом.
     Точнее, возвратить прежнее значение last_match и очистить все выходные
буфера
5. last_small_match - если маленький match найден на небольшом расстоянии
(<Barrier), то игнорировать маленькие матчи на больших расстояних (>Barrier)
пока этот не кончится. Это позволит нам перестать отбирать хлеб у больших ребят
:)
6. -l8192 -s512
...
просчитывать chksum/i+chksum и hasharr+hash&HashMask в заднем потоке
  использовать все свободные биты для chksum (например, rep:1g:32:c16 - два бита
в начале и два в конце) делать префетч по этим просчитанным адресам на 1-4
L-байтных блока вперёд: __builtin_prefetch (hashptr[K], 0/1, 0-3); упростить и
развернуть эти циклы, сделать для них template по <test,k,L> при поиске границ
матча сравнивать по 4+ байта помечать хеши 256-байтных строк в последних 512 мб
в однобитной таблице, сегментируя её на 8-32 бита в идеале - найти способ
реализовать rep:32 таким же быстрым, как rep:512 опционально - интегрировать его
в tor:3 (в основном это имеет смысл, если удастся сделать быстрый rep:32) ? if
(i>=last_match) -> if (i+L>=last_match)  -- поскольку в следующие L байт шанса
проверить матч у нас не будет...

  anchored hashing:
хешировать/проверять блоки длины меньше L, чтобы увеличить вероятность попадания
на матч запоминать N наибольших значений хеша внутри блока хранить M значений в
каждой строке хеш-таблицы, сдвигая их при обновлении строки (а-ля HT4)
  оптимизация:
? I/O в основном треде, сжатие в b/g треде (два набора выходных буферов
lens/offsets/...)
__builtin_prefetch для адресов в hasharr[] - вычислять, делать prefetch и
ненадолго сохранять их в 8-элементном массиве
