...
int TTA_METHOD::compress(CALLBACK_FUNC *callback, void *auxdata) {
  ...
  if (!f)
    f = (FARPROC)tta_compress;

  return ((int(__cdecl *)(int, int, int, int, int, int, int, CALLBACK_FUNC *,
                          void *))f)(level, skip_header, is_float, num_chan,
                                     word_size, offset, raw_data, callback,
                                     auxdata);
}
...
COMPRESSION_METHOD *parse_TTA(char **parameters) {
  if (strcmp(parameters[0], "tta") == 0) {
    // Если название метода (нулевой параметр) - "tta", то разберём остальные
    // параметры
    ...
    while (*++parameters && !error) {
      ...
      case 'm':
        p->level = parseInt(param + 1, &error);
        continue;
      case 's':
        p->skip_header = 1;
        continue;
      case 'f':
        ...
        continue;
      case 'c':
        p->num_chan = parseInt(param + 1, &error);
        continue;
      case 'w':
        p->word_size = parseInt(param + 1, &error);
        continue;
      case 'o':
        p->offset = parseInt(param + 1, &error);
        continue;
      case 'r':
        p->raw_data = parseInt(param + 1, &error);
        continue;
      }
...
static int TTA_x =
    AddCompressionMethod(parse_TTA); // Зарегистрируем парсер метода TTA
