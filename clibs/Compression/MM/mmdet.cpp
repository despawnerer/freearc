...
/* The following code snippet shows how to use the library.
   ...
   (say, probably, there is no need for 3*8 if 2*16 already shows good results).
        ...
        // Calc stats for various number of channels, bitwidth and
   enabled/disabled diff'ing for (int diff=0; diff<=1; diff++) for (int N=1;
   N<=MAX_CHANNELS; N++) for (int bits=8; bits<=32; bits+=8) {
        ...
        // Use MM compression if it provides at least 5% better compression than
   order-0 model if (best_model.result < order0.result*0.95)
            // Print parameters of best model detected
            printf ("BEST model is %d channels * %d bits, %s (%3.1f%% compared
   to order-0 model)", best_model.channels, best_model.bitwidth,
...
//        Details:
//        http://www.encode.ru/forums/index.php?action=vthread&forum=1&topic=391
// 1. floating-point values detection
// 2. optimizations: specific loops for each N (done for 8/16 bits), fast
// long->ulong conversion
// 3. provide additional code to detect signed vs unsigned data, MM vs data
// tables,
...
// MAIN ALGORITHM
// ************************************************************************ The
// algorithm is straightforward - calculate compressed size using order-0 model
// for 8/16/24/32 bit values (for 16-32 bits we group values in slots). Then
// repeat the calculation after MM substraction processing with varying word
// length (8-32) and number of channels. Smallest result points to the best
// model
...
struct Model {  // Model's parameters:
  int channels; //     Number of channels
  int bitwidth; //     Size of each element, in bits
  int offset;   //     Offset of first sample relative to buffer start, in bytes
  int diff; //     Whether successive elements in each channel substracted from
            //     each other
            // Results shown by the model:
  double bits; //     Bits used to encode small values and number of range for
               //     large values
  unsigned long xbits; //     Extra bits (used for encoding of values not in
                       //     -128..127 range)
  ...
  long (
      *stats)[STATSIZE]; // Current stats (counters for each channel and value)

  // Create new model for experiments :)
  Model(void *_buf = 0, unsigned _bufsize = 0) {
  ...
  // Calculate results of the model as level of compression using order-0
  // arithmetic coder
  void calc_results();
...
void Model::start_count(int _channels, int _bitwidth, int _offset, int _diff) {
  ...
  diff = _diff;
  bits = 0;
  // Initial bit count = additinal space probably required for huffman tables
  // (very estimated)
...
inline void Model::lcount(int channel, unsigned long x) { stats[channel][x]++; }
...
// Each channel has its own set of slots, which accounts for independent
// encoding of each channel.
inline void Model::ucount(int channel, unsigned long x) {
  if (x < (1 << 8))
    stats[channel][x]++;
  else if (x < (1 << 16))
    stats[channel][256 + (x >> 8)]++, xbits += 8;
  else if (x < (1 << 24))
    stats[channel][512 + (x >> 16)]++, xbits += 16;
  else
    stats[channel][768 + (x >> 24)]++, xbits += 24;
}
...
// Calculate results of the model as level of compression its output using
// order-0 arithmetic coder
...
// Next routines run through buffer counting statistics for elements of various
// size. They contains code specialized on N - number of channels - in order to
// improve performance
...
void Model::_8bit_run(void *buf, unsigned bufsize, int N) {
  ...
  switch (N) {
  case 1:
    for (unsigned char *p = (unsigned char *)buf; p + N <= bufend; p += N)
      ucount(0, p[0]);
    break;

  case 2:
    for (unsigned char *p = (unsigned char *)buf; p + N <= bufend; p += N)
      ucount(0, p[0]), ucount(1, p[1]);
    break;

  case 3:
    for (unsigned char *p = (unsigned char *)buf; p + N <= bufend; p += N)
      ucount(0, p[0]), ucount(1, p[1]), ucount(2, p[2]);
    break;
...
void Model::_16bit_run(void *buf, unsigned bufsize, int N) {
  ...
  switch (N) {
  case 1:
    for (short *p = (short *)buf; p + N <= bufend; p += N)
      count(0, p[0]);
    break;

  case 2:
    for (short *p = (short *)buf; p + N <= bufend; p += N)
      count(0, p[0]), count(1, p[1]);
    break;

  case 3:
    for (short *p = (short *)buf; p + N <= bufend; p += N)
      count(0, p[0]), count(1, p[1]), count(2, p[2]);
    break;
...
void Model::run(void *buf, unsigned bufsize, int channels, int bitwidth) {
  switch (bitwidth) {
  case 8:
    _8bit_run(buf, bufsize, channels);
    break;
  case 16:
    _16bit_run(buf, bufsize, channels);
    break;
  case 24:
    _24bit_run(buf, bufsize, channels);
    break;
  case 32:
    _32bit_run(buf, bufsize, channels);
    break;
  default:
    abort();
  }
...
void Model::_8bit_diff_run(void *buf, unsigned bufsize, int N) {
  ...
  switch (N) {
  case 1:
    for (unsigned char *p = (unsigned char *)buf; p + N * 2 <= bufend; p += N)
...
void Model::_16bit_diff_run(void *buf, unsigned bufsize, int N) {
  ...
  switch (N) {
  case 1:
    for (short *p = (short *)buf; p + N * 2 <= bufend; p += N)
      count(0, p[1] - p[0]);
    break;

  case 2:
    for (short *p = (short *)buf; p + N * 2 <= bufend; p += N)
      count(0, p[2] - p[0]), count(1, p[3] - p[1]);
    break;

  case 3:
    for (short *p = (short *)buf; p + N * 2 <= bufend; p += N)
      count(0, p[3] - p[0]), count(1, p[4] - p[1]), count(2, p[5] - p[2]);
    break;
...
void Model::diff_run(void *buf, unsigned bufsize, int channels, int bitwidth) {
  switch (bitwidth) {
  case 8:
    _8bit_diff_run(buf, bufsize, channels);
    break;
  case 16:
    _16bit_diff_run(buf, bufsize, channels);
    break;
  case 24:
    _24bit_diff_run(buf, bufsize, channels);
    break;
  case 32:
    _32bit_diff_run(buf, bufsize, channels);
    break;
  default:
    abort();
  }
...
static inline UINT value(BYTE *p) { return value24(p - 3); }
...
Model &Model::rolz_run(int hashlog, int hash_row_width) {
  ...
  do {
    ...
    for (int j = 0; j < hash_row_width; j++, p1 = p) {
      ...
      HTable[k + j] = p1;
      if (i == value(p) &&
          value32(p + MinLen - 4) == value32(In + MinLen - 4)) {
        len = MinLen;
        while (In + len <= InEnd && value32(p + len) == value32(In + len))
          len += 4;
        while (In[len] == p[len])
          len++;
        if (len > best_len) {
...
Model &Model::lz77_run(int hashlog, int hash_row_width) {
  start_count(2, 16, 0, 0);
  if (bufsize < 32)
    abort();
  int literals = 0, matches = 0;
  void *bufend = (char *)buf + bufsize - 8;
  UINT i, HashSize = 1 << hashlog, HashShift = 32 - hashlog,
          HashMask = ~(hash_row_width - 1);
  ...
  for (BYTE *p = (BYTE *)buf + 1; p < bufend;) {
    ...
    case 4: {
      q = HTable[h];
      BYTE *q1 = HTable[h + 1], *q2 = HTable[h + 2], *q3 = HTable[h + 3];
    ...
    if (len < (1 << 8))
      ...
      stats[0][512 + (len >> 8)]++, xbits += 8;
    else
      stats[0][768 + (len >> 24)]++, xbits += 24;
    //   Counting distance in another stats table
    int n;
    for (n = 0; dist >= 128; n += 4, dist /= 16)
      ;
...
int autodetect_wav_header(void *buf, long size, int *is_float, int *num_chan,
                          int *word_size, int *offset) {
  struct wave_hdr_t {
  ...
  if (size < sizeof(*wave_hdr) || wave_hdr->ChunkID != RIFF_SIGN ||
      wave_hdr->Format != WAVE_SIGN || wave_hdr->Subchunk1ID != fmt_SIGN ||
      wave_hdr->Subchunk1Size > wave_hdr->ChunkSize ||
      wave_hdr->NumChannels == 0 || wave_hdr->BitsPerSample > MAX_BPS)
  ...
  if (wave_hdr->Subchunk1Size > 16) {
    if (wave_hdr->Subchunk1Size - 16 >= (BYTE *)buf + size - p)
      return 0; // detection fails
...
// ** channels[]/bitvalues[] are the variants of number of channels/wordsizes we
// should try
// ****************************************************************************************
int autodetect_by_entropy(void *buf, int bufsize, int channels[],
                          int bitvalues[], double min_entropy, int *is_float,
                          int *num_chan, int *word_size, int *offset) {
  if (bufsize < 500)
    return 0; // Not enough data for detection
  Model model(buf,
              bufsize); // Object for testing various models of MM compression

  // Collect stats for order-0 model for comparison
  model.run_order0();
  double model0_result = model.result;
  // printf("order0: entropy %.2f, min %.2f\n", double(model.result)/bufsize,
  // min_entropy);
  ...
  for (int i = 0;; i++) {
    ...
    for (int j = 0;; j++) {
      ...
      for (int offset = 0; offset * 8 < bits; offset++) {
        ...
        // printf("%d*%d+%d: entropy %.2f", N, bits, offset,
        // double(model.result)/bufsize);
        //  Select best model so far
        if (model.result < best_model.result &&
            model.bitwidth >= best_model.bitwidth)
          best_model = model /*, printf(" - better!")*/;
        // Also, prefer new model if it uses larger words and still "good
        // enough"
        if (model.bitwidth > best_model.bitwidth &&
            model.result < best_model.result * 1.05 &&
            model.result < model0_result * 0.95)
          best_model = model /*, printf(" - good enough ;)")*/;
        // Opposite case - prefer new model if it has less bits but improves
        // compression at least 5%
        if (model.result < best_model.result * 0.95)
          best_model = model /*, printf(" - much better!")*/;
        // printf("\n");
      }
    }
  }

  // Use MM compression if it provides at least 5% better compression compared
  // to order-0 model
...
// Number of channels that multimedia file may have. Used for auto-detection
// when header of any known file type doesn't exist or don't taken into account.
// This array is zero-ended
static int channels[] = {1, 2, 3, 4, 0};
// Number of bits per word that multimedia file may have. Also used for
// auto-detection
...
int detect_mm_bytes(int mode, int filesize) {
  return mymin(mode <= 2 ? 64 * kb
                         : mymax(64 * kb, mymin(filesize, 1 * mb) / 2),
               filesize);
}
...
int detect_mm(int mode, void *buf, int bufsize) {
  ...
  return autodetect_by_entropy(buf, bufsize, use_channels, use_bitvalues, 0.80,
...
int detect_mm_header(int mode, void *buf, int bufsize) {
  int is_float, num_chan, word_size, offset;
  return autodetect_wav_header(buf, bufsize, &is_float, &num_chan, &word_size,
                               &offset);
}

// ****************************************************************************************
// ** Analyze data and return best guess of their type (binary, text,
// compressed...)
// ****************************************************************************************
int count_desc_order(const void *a, const void *b) {
  return *(int *)b - *(int *)a;
}
...
void detect_datatype(BYTE *buf, int bufsize, char *type) {
  ...
  int count[256];
  iterate(256, count[i] = 0);
  // ..ищем матчи с помощью простой LZ-схемы и определяем долю REPDIST кодов
  // среди них
  int matches = 0, repdists = 0, sumlen = 0, dist[4] = {0, 0, 0, 0};
#define TABLE_SIZE 16384
#define hash(p) (value16(p) % TABLE_SIZE)
  BYTE *table[TABLE_SIZE];
  iterate(TABLE_SIZE, table[i] = NULL);
  if (bufsize > 10)
    for (BYTE *p = buf; p < buf + bufsize - 10; p++) {
      BYTE *q = table[hash(p)];
      if (q && value16(p) == value16(q)) {
        if (p - q <
            256) { // собираем статистику только по матчам с дистанцией <256
          matches++;
          int x = p - q;
        ...
        while (*p == *q && p < buf + bufsize - 10)
          count[*p]++, table[hash(p)] = p, p++, q++;
        count[*p]++;
        if (o - q < 256)
          sumlen += p - o;
      } else {
...
#undef hash
  ...
  //             $text если активных символов от 17 до 80, число повторов
  //             дистанций невелико и lz-матчи составляют хотя бы 10% данных
  strcpy(type, buf == NULL
                   ? "$precomp $compressed $text"
                   : // list of data types this procedure is able to recognize
               ...
               17 <= normal_chars &&
                       normal_chars <= 80
                       // && matches < bufsize/6
                       && repdists < 0.20 * matches && sumlen > 0.10 * bufsize
                   ? "$text"
                   : "default");
  // if(bufsize) printf (" %8s %d\n", type, normal_chars);
  // if(bufsize)  printf (" %8s %5.2lf%% %5d %5.2lf%% %5.0lf %5.2lf %5.2lf%%\n",
  // type, order0/bufsize*100, normal_chars, double(repdists+1)/(matches+1)*100,
  // double(matches), double(sumlen)/(matches+1),
  // double(sumlen)/(bufsize+1)*100); printf("STAT: normal_chars=%d,
  // repdists=%d, matches=%d, sumlen=%d, bufsize=%d\n\n", normal_chars,
  // repdists, matches, sumlen, bufsize, bufsize, bufsize, buf);
}
...
// DRIVER
// ************************************************************************ This
// demo program shows how to use MM detector and moreover demonstrates more
// complex technique of gathering statistics while processing file in small
// chunks
...
double timer(void) {
  ...
  double res =
      double(PerformanceCountEnd.QuadPart - PerformanceCountStart.QuadPart) /
      Frequency.QuadPart;
...
void print_stats(char *model_name, Model model) {
  printf("%-36s:%9ld +%9ld =%9ld bytes (%3.1lf%%): %.3lf seconds\n", model_name,
         (long)(model.bits / 8), model.xbits / 8, model.result,
         (double)model.result * 100 / filesize, timer());
}
...
int main(int argc, char **argv) {
  ...
  if (!lz_mode && !lz77_mode && !det_mode) {
    ...
    // Calc stats for various number of channels, bitwidth and enabled/disabled
    // diff'ing
      ...
      for (int N = 1; N <= MAX_CHANNELS; N++) {
        for (int bits = 8; bits <= 32; bits += 8) {
          for (int offset = 0; offset * 8 < bits; offset++) {
            ...
            sprintf(model_name, "%2dbit%s%s%s", bits, channelsStr, offsetStr,
                    diff ? ", diffed" : "");

            // Try model with given parameters, collect appropriate statistics
            // and print it
            diff ? model.diff_run(N, bits, offset) : model.run(N, bits, offset);
    ...
    for (int diff = 0; diff <= 1; diff++) {
      for (int N = 1; N <= MAX_CHANNELS; N++) {
        for (int bits = 8; bits <= 32; bits += 8) {
          ...
          sprintf(model_name, "%2dbit%s%s", bits, channels,
                  diff ? ", diffed" : "");
    ...
    int is_float, num_chan, word_size, offset;
    if (autodetect_wav_header(buf, bytes, &is_float, &num_chan, &word_size,
                              &offset))
      printf("WAV header: %dbit%s * %d channel(s), offset %d\n", word_size,
             is_float ? " float" : "", num_chan, offset);
  ...
  } else if (lz77_mode) // Try LZ77 model with various hashsize and hashchain
                        // length
  {
    for (int l = 1; l <= 4; l *= 2) {
      if (*title)
  ...
  } else if (det_mode) // Binary/text/compressed detection mode
  {
    ...
    char results[MAX_TYPES][100] = {""};
    int counters[MAX_TYPES] = {0};
