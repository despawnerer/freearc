...
static int FindMatchFinder(const char *s) {
  for (int m = 0;
       m < (int)(sizeof(kMatchFinderIDs) / sizeof(kMatchFinderIDs[0])); m++)
...
int lzma_compress2(int dictionarySize, int hashSize, int algorithm,
                   int numFastBytes, int matchFinder, int matchFinderCycles,
                   int posStateBits, int litContextBits, int litPosBits,
                   void *input, int inputSize, void *output, int *outputSize,
                   CALLBACK_FUNC *callback, void *auxdata, void **CodecState) {
  // NULL input & callback is a special empty call just to release LzmaEnc
  // handle stored in the *CodecState
  if (!input && !callback) {
    LzmaEnc_Destroy(*CodecState, &g_Alloc, &g_Alloc);
    *CodecState = NULL;
    return FREEARC_OK;
  }

  CallbackInStream inStream;
  inStream.Read = CallbackRead;
  inStream.callback = callback;
  inStream.auxdata = auxdata;
  inStream.errcode = FREEARC_OK;
  inStream.total = 0;
  inStream.first_read = TRUE;
  CallbackOutStream outStream;
  outStream.Write = CallbackWrite;
  outStream.callback = callback;
  outStream.auxdata = auxdata;
  outStream.errcode = FREEARC_OK;
  outStream.total = 0;
  CompressProgress progress;
  progress.Progress = CallbackProgress;
  progress.callback = callback;
  progress.auxdata = auxdata;
  progress.errcode = FREEARC_OK;
  progress.lastInSize = 0;
  progress.lastOutSize = 0;
  ...
  if (CodecState && *CodecState) {
    ...
    switch (matchFinder) {
    case kHC4:
      props.btMode = MF_HashChain;
      props.numHashBytes = 4;
      break;
    case kBT2:
      props.btMode = MF_BinaryTree;
  ...
  if (res == SZ_OK) {
    if (input && output && outputSize) {
      ...
      res = LzmaEnc_MemEncode(
          enc, (Byte *)output, &outsize, (const Byte *)input, inputSize,
          writeEndMark, (ICompressProgress *)&progress, &g_Alloc, &g_Alloc);
      *outputSize = outsize;
      CallbackProgress(&progress, inputSize,
                       *outputSize); // report bytes compressed after the last
                                     // call to progress.Progress
    } else {
      res = LzmaEnc_Encode(enc, (ISeqOutStream *)&outStream,
                           (ISeqInStream *)&inStream,
                           (ICompressProgress *)&progress, &g_Alloc, &g_Alloc);
      CallbackProgress(&progress, inStream.total,
                       outStream.total); // report bytes compressed after the
                                         // last call to progress.Progress
    }
  }
  ...
  if (inStream.errcode != FREEARC_OK)
    return inStream.errcode;
  if (outStream.errcode != FREEARC_OK)
    return outStream.errcode;
  if (progress.errcode != FREEARC_OK)
    return progress.errcode;
  return SRes_to_FreeArc(res);
}

#endif // !defined (FREEARC_DECOMPRESS_ONLY)

SRes LzmaDec_AllocateUsingProperties(CLzmaDec *p, const CLzmaProps propNew,
                                     ISzAlloc *alloc,
                                     bool Memory_to_memory_decompression) {
...
int lzma_decompress2(int dictionarySize, int hashSize, int algorithm,
                     int numFastBytes, int matchFinder, int matchFinderCycles,
                     int posStateBits, int litContextBits, int litPosBits,
                     void *input, int inputSize, void *output, int *outputSize,
  ...
  // NULL input & callback is a special empty call just to release CLzmaDec
  // state stored in the *CodecState
  if (!input && !callback) {
  ...
  if (_state == NULL) {
    ...
    SRes res = LzmaDec_AllocateUsingProperties(_state, LzmaProps, &g_Alloc,
                                               Memory_to_memory_decompression);
    if (res != SZ_OK) {
      delete _state;
      return SRes_to_FreeArc(res);
    }
  ...
  if (Memory_to_memory_decompression) {
    ...
    res = LzmaDec_DecodeToDic(_state, *outputSize, (const Byte *)input, &insize,
  ...
  {
    Byte *_inBuf;
    BIGALLOC(Byte, _inBuf, RANGE_DECODER_BUFFER_SIZE)
    for (UInt32 _inPos = 0, _inSize = 0;;) {
      if (_inPos == _inSize) {
        READ_LEN(
            _inSize, _inBuf,
            RANGE_DECODER_BUFFER_SIZE); // Read compressed data in 256kb chunks
        _inPos = 0;                     // current position inside input data
      }

      SizeT oldDicPos = _state->dicPos;
      SizeT curSize = mymin(
          _state->dicBufSize - oldDicPos,
          HUGE_BUFFER_SIZE); // Write decompressed data in 8mb chunks at most

      SizeT inSizeProcessed = _inSize - _inPos;
      ELzmaStatus status;
      res = LzmaDec_DecodeToDic(_state, oldDicPos + curSize, _inBuf + _inPos,
                                &inSizeProcessed, LZMA_FINISH_ANY, &status);
      ...
      if (inSizeProcessed == 0 && outSizeProcessed == 0)
        ReturnErrorCode(SRes_to_FreeArc(
            status == LZMA_STATUS_FINISHED_WITH_MARK ? SZ_OK : SZ_ERROR_DATA));
...
// Если строка str начинается со start, то возвратить адрес остатка, иначе -
// NULL
...
LZMA_METHOD::LZMA_METHOD() {
  ...
  numFastBytes = 32;
  matchFinder = kHT4;
  matchFinderCycles = 0; // библиотека LZMA определит количество циклов
                         // автоматически, исходя из matchFinder и numFastBytes
  posStateBits = 2;
  litContextBits = 3;
  litPosBits = 0;
}
...
int LZMA_METHOD::doit(char *what, int param, void *data,
                      CALLBACK_FUNC *callback) {
  if (strequ(what, "has_progress?"))
    return 1; // Да, этот алгоритм поддерживает отчёт о прогрессе упаковки
  else
...
int LZMA_METHOD::DeCompressMem(COMPRESSION direction, void *input,
                               int inputSize, void *output, int *outputSize,
                               CALLBACK_FUNC *callback, void *auxdata,
                               void **CodecState) {
#ifndef FREEARC_DECOMPRESS_ONLY
  ...
  if (!c)
    c = (FARPROC)lzma_compress2;

  if (direction == COMPRESS) {
    SetDictionary(dictionarySize); // Ограничим размер словаря чтобы сжатие
                                   // влезало в 4гб памяти :)
  ...
  if (!d)
    d = (FARPROC)lzma_decompress2;

  return ((int (*)(int, int, int, int, int, int, int, int, int, void *, int,
                   void *, int *, CALLBACK_FUNC *, void *,
                   void **))(direction == COMPRESS ? c : d))(
      dictionarySize, hashSize, algorithm, numFastBytes, matchFinder,
      matchFinderCycles, posStateBits, litContextBits, litPosBits, input,
      inputSize, output, outputSize, callback, auxdata, CodecState);
}
...
// Extra 6 mb are required for MT compressor (4.25mb for MT bufs plus 1.7mb
// added to the dictionary unless in compress_all_at_once mode)
static MemSize mtbuf(int algo, int threads) {
  return (algo && (threads != 1) ? (compress_all_at_once ? 4 * mb : 6 * mb)
                                 : 0);
}
...
void LZMA_METHOD::CalcCompressionMemories(MemSize *pmem, MemSize *phashSize) {
  ...
  // Дополнительная область словаря, после заполнения которой происходит его
  // сдвиг
  LongMemSize sizeReserv =
      matchFinder == kHT4
          ? (props.dictSize <= 768 * mb ? props.dictSize / 4
                                        : props.dictSize / 8)
          : (props.dictSize <= ((UInt32)2 << 30) ? props.dictSize / 2
                                                 : props.dictSize / 4);
  ...
  LongMemSize hash_memory = props.hashSize +
                            sons * sizeof(CLzRef) * props.dictSize +
                            RC_BUF_SIZE(props.dictSize) + 768 * kb +
                            mtbuf(props.algo, props.numThreads);
  ...
  if (pmem)
    *pmem = MemSize(mymin(
        MEMSIZE_MAX, hash_memory + (compress_all_at_once ? 0 : buf_memory)));

  // Размер хеш-таблицы
  if (phashSize)
    *phashSize = props.hashSize;
}

// Вычисляет словарь, использующий не более mem памяти для сжатия заданным
// LZMA_METHOD. Метод не очень точен - он не учитывает тонкости
//   расчёта sizeReserv, округление props.hashSize/k вниз до степени 2 и другие
//   детали (см. CalcCompressionMemories)
MemSize calcDictSize(LZMA_METHOD *p, MemSize oldDictionarySize, MemSize mem) {
  double mem4 =
      mymax(double(mem) - p->hashSize - (compress_all_at_once ? 0 : 1 * mb) -
                mtbuf(p->algorithm, GetCompressionThreads()) - 1 * mb,
            0);
  double buf_k =
      compress_all_at_once ? 0 : (p->matchFinder == kHT4 ? 1.25 : 1.5);
  switch (p->matchFinder) {
  case kBT2:
    buf_k += 8;
    break;
  case kBT3:
    buf_k += (p->hashSize ? 8 : 10);
    break;
  case kBT4:
    buf_k += (p->hashSize ? 8 : 10);
    break;
  case kHC4:
    buf_k += (p->hashSize ? 4 : 6);
    break;
  case kHT4:
    buf_k += (p->hashSize ? 0 : 1);
    break;
  }
...
void LZMA_METHOD::SetCompressionMem(MemSize mem) {
  ...
  if (compress_all_at_once && matchFinder == kHT4) {
    hashSize = MemSize(floor(
                   mymax(double(mem / mb) -
                             mtbuf(algorithm, GetCompressionThreads()) / mb - 1,
                         1))) *
               mb; // Use all memory for hash because original and compressed
                   // data are held in memory buffers provided by caller
  } else {
    hashSize = 0; // otherwise large hash may leave too little memory for the
                  // dictionary (to do: make a proportional cutoff and take into
                  // account rounding of hash in HT4 method)
...
void LZMA_METHOD::SetDictionary(MemSize mem) {
  ...
  dictionarySize =
      mymin(dictionarySize,
            rounddown_mem(calcDictSize(this, dictionarySize, UINT_MAX)));
#endif
...
MemSize LZMA_METHOD::GetDecompressionMem(void) {
  ...
  return compress_all_at_once ? 1 * mb
                              : dictionarySize + RANGE_DECODER_BUFFER_SIZE;
}

// Записать в buf[MAX_METHOD_STRLEN] строку, описывающую метод сжатия и его
// параметры (функция, обратная к parse_LZMA)
void LZMA_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  char DictionaryStr[100], HashStr[100], fcStr[100], pbStr[100], lcStr[100],
      lpStr[100], algStr[100];
  showMem(dictionarySize, DictionaryStr);

  MemSize hs = hashSize;
#ifndef FREEARC_DECOMPRESS_ONLY
  // Вычислим реальный размер хеш-таблицы (он может быть меньше hashSize из-за
  // того, что каждый bucket должен содержать 2^n элементов)
  if (hashSize)
    CalcCompressionMemories(NULL, &hs);
#endif
  showMem(hs, HashStr);

  LZMA_METHOD defaults;
  sprintf(fcStr, matchFinderCycles != defaults.matchFinderCycles ? ":mc%d" : "",
          matchFinderCycles);
  sprintf(pbStr, posStateBits != defaults.posStateBits ? ":pb%d" : "",
          posStateBits);
  sprintf(lcStr, litContextBits != defaults.litContextBits ? ":lc%d" : "",
          litContextBits);
  sprintf(lpStr, litPosBits != defaults.litPosBits ? ":lp%d" : "", litPosBits);
  matchFinder == kHT4
      ? (algorithm == 2 ? sprintf(algStr, "a%d", algorithm)
                        : sprintf(algStr, algorithm == 0 ? "fast" : "normal"))
      : sprintf(algStr, "%s:%s",
                algorithm == 0   ? "fast"
                : algorithm == 1 ? "normal"
                                 : "max",
                kMatchFinderIDs[matchFinder]);
  for (char *p = algStr; *p; p++)
    *p = tolower(*p); // strlwr(algStr);
  sprintf(buf, "lzma:%s%s%s:%s:%d%s%s%s%s", DictionaryStr, hashSize ? ":h" : "",
          hashSize ? HashStr : "", algStr, numFastBytes, fcStr, pbStr, lcStr,
          lpStr);
}
...
COMPRESSION_METHOD *parse_LZMA(char **parameters) {
  if (strcmp(parameters[0], "lzma") == 0) {
    // Если название метода (нулевой параметр) - "lzma", то разберём остальные
    // параметры
    ...
    while (*++parameters) {
      ...
      bool optional = param[0] == '*';
      optional &&param++;
      if (start_from(param, "d"))
        p->dictionarySize = parseMem(param + 1, &error);
      else if (start_from(param, "h")) {
        MemSize h = parseMem(param + 1, &error);
        if (error)
          goto unnamed;
        else
          p->hashSize = h;
      } else if (start_from(param, "a"))
        p->algorithm = parseInt(param + 1, &error);
      else if (start_from(param, "fb"))
        p->numFastBytes = parseInt(param + 2, &error);
      else if (start_from(param, "mc"))
        p->matchFinderCycles = parseInt(param + 2, &error);
      else if (start_from(param, "lc"))
        p->litContextBits = parseInt(param + 2, &error);
      else if (start_from(param, "lp"))
        p->litPosBits = parseInt(param + 2, &error);
      else if (start_from(param, "pb"))
        p->posStateBits = parseInt(param + 2, &error);
      else if (start_from(param, "mf"))
        p->matchFinder =
            FindMatchFinder(param[2] == '=' ? param + 3 : param + 2);
      else if (strequ(param, "fastest"))
        p->algorithm = 0, p->matchFinder == INT_MAX && (p->matchFinder = kHT4),
        p->numFastBytes = 5, p->matchFinderCycles = 1;
      else if (strequ(param, "fast"))
        p->algorithm = 0, p->matchFinder == INT_MAX && (p->matchFinder = kHT4),
        p->numFastBytes = 32, p->matchFinderCycles = 0;
      else if (strequ(param, "normal"))
        p->algorithm = 1, p->matchFinder == INT_MAX && (p->matchFinder = kHT4),
        p->numFastBytes = 32, p->matchFinderCycles = 0;
      else if (strequ(param, "max"))
        p->algorithm = 1, p->matchFinder == INT_MAX && (p->matchFinder = kBT4),
        p->numFastBytes = 128, p->matchFinderCycles = 0;
      else if (strequ(param, "ultra"))
        p->algorithm = 1, p->matchFinder == INT_MAX && (p->matchFinder = kBT4),
        p->numFastBytes = 128, p->matchFinderCycles = 128;
      else {
      unnamed: // Сюда мы попадаем, если в параметре опущено его наименование
        // Это может быть строка - имя MatchFinder'а, целое число - значение
        // numFastBytes, или обозначение памяти - значение dictionarySize
        error = 0;
        ...
        if (n >= 0)
          ...
          if (!error)
            ...
            error = 0;
            MemSize n = parseMem(param, &error);
            if (!error)
              p->dictionarySize = n;
          }
        }
...
static int LZMA_x =
    AddCompressionMethod(parse_LZMA); // Зарегистрируем парсер метода LZMA
