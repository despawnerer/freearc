...
int main(int argc, char *argv[]) {
  ...
  if (argc == 3 || argc == 5) {
    ...
    infile = nameequ(infile_name, "-") ? stdin : fopen(infile_name, "rb");
    if (infile == NULL) {
      fprintf(stderr, "Can't open input file %s!\n", infile_name);
      return EXIT_FAILURE;
    }
    outfile = nameequ(outfile_name, "-") ? stdout : fopen(outfile_name, "wb");
    if (outfile == NULL) {
    ...
    printf("FreeArc-LZMA v2.1 (2012-03-15) - external LZMA compressor for "
           "FreeArc\n"
           "\n"
           ...
           "\n"
           "Notes: method may include any LZMA parameters supported by "
           "FreeArc, for example lzma:ht4:fastest:1g\n"
