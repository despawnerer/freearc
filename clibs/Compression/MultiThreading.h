...
public:
  ...
  int SetSize(int max_elements) {
    ...
    head = tail = 0;
    size = max_elements +
           1; // +1 simplifies distinguishing between full and empty queues
...
RunBackgroundThread(void *param) {
  ((BackgroundThread *)param)->run();
  return 0;
}
...
template <class Job> struct ProcessingThread : BackgroundThread {
  ...
  virtual void run() {
    SetCompressionThreadPriority(); // Понизить приоритет треда (рас)паковки
                                    // чтоб не завешивать машину
    for (;;) {
...
template <class Job> struct MultipleProcessingThreads {
  struct Task {
    Job job;
    Event *event;
  };
...
template <class Job>
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE
MultipleProcessingThreads_RunWorkerThread(void *param) {
  ((MultipleProcessingThreads<Job> *)param)->WorkerThread();
  return 0;
}

// Alloc resources and start b/g threads
template <class Job> int MultipleProcessingThreads<Job>::start() {
  if (MaxJobs <= 0 || NumThreads <= 0)
    return -1; // Errcode

  WorkerQueue.SetSize(MaxJobs +
                      1); // +1 for a NULL job signalling thread to finish
  ...
  for (int i = 0; i < NumThreads; i++)
    WorkerThreadsPtr[i].Create(MultipleProcessingThreads_RunWorkerThread<Job>,
                               this);
...
template <class Job = CompressionJob, typename Worker = void *>
struct MTCompressor {
  ...
  SyncQueue<Job *>
      WriterJobs; // Queue of jobs for Writer thread, ordered by input chunks
  ...
  virtual int
  AllocateBuffers(int BufferSize); // Allocate I/O buffers of given size
  virtual int Init();              // Creates threads and jobs
  virtual int Run();               // Main function performing all work
  virtual void
  Process(Job &job,
          Worker &worker) = 0;      // Perform one (de)compression operation
  virtual void Write(Job &job) = 0; // Write output buffer and perform
  ...
  virtual int WorkerInit(Worker &worker) {
    return 0;
  } // Init WorkerThread local data, returning non-zero error code on fail
  virtual void WorkerDone(Worker &worker) {} // Release WorkerThread local data

  MTCompressor(int _NumThreads = -1,
               int _NumExtraBuffers = -1) { // default values
    NumThreads = (_NumThreads > 0 ? _NumThreads : GetCompressionThreads());
    NumBuffers = NumThreads + (_NumExtraBuffers >= 0 ? _NumExtraBuffers : 2);
    ImmediatelyFreeInbuf = true;
    ErrCode = 0;
    Jobs = NULL;
    WorkerThreadsPtr = NULL;
    bufs = buf0 = NULL;
  }
...
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE RunWorkerThread(void *param) {
  ((MTCompressor<CompressionJob> *)param)->WorkerThread();
  return 0;
}
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE RunWriterThread(void *param) {
  ((MTCompressor<CompressionJob> *)param)->WriterThread();
  return 0;
}

// Allocate I/O buffers of given size
template <class Job, typename Worker>
int MTCompressor<Job, Worker>::AllocateBuffers(int BufferSize) {
  ...
  for (int i = 0; i < NumBuffers; i++) {
    ...
    if (buf == NULL)
      return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;
    InputBuffers.Put(buf);
    *bufs++ = buf;

    buf = (char *)BigAlloc(BufferSize);
    if (buf == NULL)
      return FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;
    OutputBuffers.Put(buf);
    *bufs++ = buf;
  }
...
template <class Job, typename Worker> int MTCompressor<Job, Worker>::Run() {
  ...
  // *****************************************************************************************************
  SetErrCode(ReaderThread()); // Main cycle: reading data and sending
                              // compression jobs to other threads
  // *****************************************************************************************************
...
// Thread performing jobs from queue
template <class Job, typename Worker>
void MTCompressor<Job, Worker>::WorkerThread() {
  SetCompressionThreadPriority(); // Понизить приоритет треда (рас)паковки чтоб
                                  // не завешивать машину
  Worker local;
  SetErrCode(WorkerInit(local));
  for (;;) {
    Job *job = WorkerJobs.Get(); // Acquire next (de)compression job
    if (job == NULL || ErrCode)
      break;                           // Quit on EOF or error in other thread
    ...
    if (ImmediatelyFreeInbuf)
      InputBuffers.Put(
          job->InBuf);          // Input buffer is no more required for this job
...
template <class Job, typename Worker>
void MTCompressor<Job, Worker>::WriterThread() {
  for (;;) {
    ...
    if (SetErrCode(job->result) < 0)
      break; // Break on error (after we've wrote data - assuming they were
             // produced before error)
    if (!ImmediatelyFreeInbuf)
      InputBuffers.Put(
          job->InBuf); // Input buffer is no more required for this job
    OutputBuffers.Put(
        job->OutBuf);  // Output buffer is no more required for this job
