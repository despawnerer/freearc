...
#pragma pack(1)
static _THREAD1 struct SEE2_CONTEXT { // SEE-contexts for PPM-contexts with
                                      // masked symbols
  WORD Summ;
  BYTE Shift, Count;
...
static _THREAD1 PPM_CONTEXT::STATE *_THREAD
    FoundState; // found next state transition
static _THREAD1 int _THREAD InitEsc, _THREAD OrderFall, _THREAD RunLength,
    _THREAD InitRL, _THREAD MaxOrder;
static _THREAD1 BYTE _THREAD CharMask[256], _THREAD NumMasked,
    _THREAD PrevSuccess, _THREAD EscCount, _THREAD PrintCount;
...
void _STDCALL EncodeFile(_PPMD_FILE *EncodedFile, _PPMD_FILE *DecodedFile,
  ...
  for (PPM_CONTEXT *MinContext;;) {
    ...
    ARI_ENC_NORMALIZE(EncodedFile);
    if (_PPMD_ERROR_CODE(DecodedFile) < 0 || _PPMD_ERROR_CODE(EncodedFile) < 0)
      return;
  }
STOP_ENCODING:
