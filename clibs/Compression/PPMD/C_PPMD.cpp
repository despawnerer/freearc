...
int ppmd_dispatch(int ENCODE, int order, MemSize mem, int MRMethod,
                  MemSize chunk, CALLBACK_FUNC *callback, void *auxdata) {
  ...
  FARPROC f =
      compress_all_at_once
          ? (de_compress ? de_compress
                         : (FARPROC)PPMD_de_compression::ppmd_de_compress)
          : (FARPROC)NULL;
  if (!f)
    f = ENCODE ? (FARPROC)compress : (FARPROC)decompress;
  if (!f)
    f = ENCODE ? (FARPROC)PPMD_compression::ppmd_compress2
               : (FARPROC)ppmd_decompress2;
  if (!f)
    return FREEARC_ERRCODE_INVALID_COMPRESSOR;
  return ((int (*)(int, int, MemSize, int, MemSize, CALLBACK_FUNC *, void *))f)(
      ENCODE, order, mem, MRMethod, chunk, callback, auxdata);
}
...
void PPMD_METHOD::SetCompressionMem(MemSize _mem) {
  ...
  order = mymax(MIN_O, mymin(MAX_O, order + int(log(double(_mem) / mem) /
                                                log(double(2)) * 4)));
  mem = _mem;
}
...
// параметры (функция, обратная к parse_PPMD)
void PPMD_METHOD::ShowCompressionMethod(char *buf, bool purify) {
  char MemStr[100], ChunkStr[100];
  showMem(mem, MemStr);
  showMem(chunk, ChunkStr);
  sprintf(buf, "ppmd:%d:%s%s%s%s", order, MemStr,
          MRMethod == 2 ? ":r2" : (MRMethod == 1 ? ":r" : ""),
          chunk ? ":c" : "", chunk ? ChunkStr : "");
}
...
COMPRESSION_METHOD *parse_PPMD(char **parameters) {
  if (strcmp(parameters[0], "ppmd") == 0) {
    // Если название метода (нулевой параметр) - "ppmd", то разберём остальные
    // параметры
    ...
    while (*++parameters && !error) {
      ...
      if (strlen(param) == 1)
        ...
        case 'r':
          ...
          continue;
        case 'c':
          p->chunk = parseMem(param + 1, &error);
          continue;
        }
      ...
      if (!error)
        ...
        error = 0, p->mem = parseMem(param, &error);
    }
    if (error || p->mem < MIN_MEM || p->order < MIN_O || p->order > MAX_O) {
      delete p;
      return NULL;
    } // Ошибка при парсинге параметров метода или значение параметра за
...
static int PPMD_x =
    AddCompressionMethod(parse_PPMD); // Зарегистрируем парсер метода PPMD
