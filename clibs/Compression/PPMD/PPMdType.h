...
class PRIME_STREAM {
public:
  PRIME_STREAM(CALLBACK_FUNC *_callback, void *_auxdata, int _bufsize)
      : Error(0), StrPos(0), Count(0) {
    callback = _callback;
    auxdata = _auxdata;
    BufSize = _bufsize;
    p = Buf = (BYTE *)BigAlloc(BufSize);
    if (!p)
      Error = FREEARC_ERRCODE_NOT_ENOUGH_MEMORY;
  }
  void free() {
    BigFree(Buf);
    Buf = NULL;
  ...
  int get() {
    return (--Count >= 0) ? (*p++) : (fill(), (--Count >= 0) ? (*p++) : EOF);
  }
  int put(int c) {
    return (--Count >= 0) ? (*p++ = c)
                          : (flush(), (--Count >= 0) ? (*p++ = c) : EOF);
  }
  ...
  void flush() { // it must remove (p-Buf) bytes
    if (Error >= 0 && p > Buf) {
      int i = callback("write", Buf, p - Buf, auxdata);
      if (i < 0)
        Error = i;
    }
    p = Buf;
