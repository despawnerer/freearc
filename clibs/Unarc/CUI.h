...
public:
  ...
  bool ProgressFile(bool isdir, const char *operation, FILENAME filename,
                    uint64 filesize);
  void EndProgress();
  bool AllowProcessing(char cmd, int silent, FILENAME arcname, char *comment,
                       int cmtsize, FILENAME outdir);
  FILENAME GetOutDir();
...
void CUI::DisplayHeader(char *header) { printf("%s", header); }

bool CUI::ProgressFile(bool isdir, const char *operation, FILENAME filename,
                       uint64 filesize) {
...
void CUI::EndProgress() { printf("All OK"); }

FILENAME CUI::GetOutDir() { return outdir; }

bool CUI::AllowProcessing(char cmd, int silent, FILENAME arcname, char *comment,
                          int cmtsize, FILENAME _outdir) {
...
char CUI::AskOverwrite(FILENAME filename, uint64 size, time_t modified) {
  char help[] = "Valid answers: Y - yes, N - no, A - overwrite all, S - skip "
                "all, Q - quit\n";
again:
  ...
  *answer = tolower(*answer);
  if (strlen(answer) != 1 || !strchr("ynasq", *answer)) {
    printf(help);
    goto again;
  }
  if (*answer == 'q') {
...
void CUI::ListHeader(COMMAND &command) {
  if (command.cmd == 'l')
    ...
    printf("Date/time              Attr            Size          Packed      "
           "CRC Filename\n"
           "-------------------------------------------------------------------"
           "----------\n");
...
void CUI::ListFooter(COMMAND &command) {
  ...
  printf("%.0lf files, %.0lf bytes, %.0lf compressed\n", double(total_files),
         double(total_bytes), double(total_packed));
}
...
void CUI::ListFiles(DIRECTORY_BLOCK *dirblock, COMMAND &command) {
  ...
  iterate_var(i, dirblock->total_files) {
    // Увеличим номер солид-блока если мы вышли за последний принадлежащий ему
    // файл
    if (i >= dirblock->block_end(b))
      b++;
    // Если это первый файл в солид-блоке - соберём block-related информацию
    if (i ==
        dirblock->block_start(
            b)) { // Запишем на первый файл в блоке весь его упакованный размер
      packed = dirblock->data_block[b].compsize;
      // Запомним информацию о солид-блоке для использования её со всеми файлами
      // из этого солид-блока
      char *c = dirblock->data_block[b].compressor;
      Encrypted =
          strstr(c, "+aes-") != NULL || strstr(c, "+serpent-") != NULL ||
          strstr(c, "+blowfish-") != NULL || strstr(c, "+twofish-") != NULL;
    }

    if (command.accept_file(dirblock,
                            i)) { //   Если этот файл требуется обработать
      unsigned long long filesize = dirblock->size[i];
      char timestr[100];
      FormatDateTime(timestr, 100, dirblock->time[i]);

      if (command.cmd == 'l')
        printf(dirblock->isdir[i] ? "%s       -dir-" : "%s %11.0lf", timestr,
               double(filesize));
      else
        printf("%s %s %15.0lf %15.0lf %08x", timestr,
               dirblock->isdir[i] ? ".D....." : ".......", double(filesize),
               double(packed), dirblock->crc[i]);
      printf("%c", Encrypted ? '*' : ' ');
