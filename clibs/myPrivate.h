// This file replaces p7zip's myPrivate.h that cannot be used when multiple .cpp
// files are compiled as one unit (using #include)
...
#define nameWindowToUnix(lpFileName)                                           \
  ((lpFileName)[0] == 'c' && (lpFileName)[1] == ':' ? (lpFileName) + 2         \
                                                    : (lpFileName))
// #endif
#endif
