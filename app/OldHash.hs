...
recordNew :: IO ()
recordNew = instrument rec_
  where rec_ hd@HD{ tables=t, totBuckets=b } =
               hd{ tables=t+1, totBuckets=b+fromIntegral tABLE_MIN }

recordIns :: Int32 -> Int32 -> [a] -> IO ()
recordIns i sz bkt = instrument rec_
  where rec_ hd@HD{ insertions=ins, maxEntries=mx, maxChain=mc } =
               hd{ insertions=ins+fromIntegral i, maxEntries=mx `max` sz,
                   maxChain=mc `max` length bkt }

recordResize :: Int32 -> Int32 -> IO ()
recordResize older newer = instrument rec_
  where rec_ hd@HD{ totBuckets=b, maxBuckets=mx } =
...
expandHashTable hash table@HT{ buckets=bkts, bmask=mask } = do
   ...
   --
    ...
    Control.Monad.mapM_ splitBucket [0..mask]
